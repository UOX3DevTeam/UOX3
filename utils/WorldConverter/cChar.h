...
class CChar : public cBaseObject
{
	protected:
		...
		SI16		npcaitype;
		SI32		making; // skill number of skill using to make item, 0 if not making anything, used for house building, reduce at later date, for changeover
...
//		CItem *		itemLayers[MAXLAYERS];
	...
	public:
		...
		void		SetBankBoxOpen( bool newValue );

		void		SetSerial( UI32 newSerial , CHARACTER c );
		void		SetSerial( UI08 newSerial, UI08 newPart, CHARACTER c );
		void		SetRobe( UI08 value, UI08 part );
		void		SetRobe( UI32 newValue );
		void		SetOwner( UI08 value, UI08 part );
		...
		void		SetOldX( SI16 oldXToSet );
		void		SetOldY( SI16 oldYToSet );
		void		SetOldZ( SI08 oldZToSet );
		...
		void		SetLocation( SI16 newX, SI16 newY, SI08 newZ, SI08 newDispZ );
		void		SetLocation( SI16 newX, SI16 newY, SI08 newZ );
		void		SetLocation( struct point3 &targ );

		SI32		GetPackItem( void ) const;	// index into items[]
		CItem *		GetPackItemObj( void ) const;
		SI32		GetTarg( void ) const;
		SI32		GetAttacker( void ) const;
		SI32		GetFTarg( void ) const;
		SI32		GetSmeltItem( void ) const;
		SI32		GetTailItem( void ) const;
		SI32		GetAdvObj( void ) const;
		SI32		GetEnvokeItem( void ) const;
		SI32		GetSwingTarg( void ) const;
		UI32		GetTrackingTarget( void ) const;
		UI32		GetTrackingTargets( UI08 targetNum ) const;
		RACEID		GetRaceGate( void ) const;

		void		SetPackItem( SI32 newVal );
		void		SetTarg( SI32 newTarg );
		void		SetAttacker( SI32 newValue );
		void		SetFTarg( SI32 newTarg );
		void		SetTailItem( SI32 newValue );
		void		SetSmeltItem( SI32 newValue );
		void		SetAdvObj( SI32 newValue );
		void		SetEnvokeItem( SI32 newValue );
		void		SetSwingTarg( SI32 newValue );
		void		SetTrackingTarget( UI32 newValue );
		void		SetRaceGate( RACEID newValue );
		void		SetTrackingTargets( UI32 newValue, UI08 targetNum );
		...
		SI32		GetPoison( void ) const;
		SI32		GetPoisoned( void ) const;

		void				SetPoison( SI32 newValue );
		void				SetPoisoned( SI32 newValue );

							CChar( CHARACTER c, bool zeroSer = false );
		virtual		~CChar();

		void		Dupe( CChar& target, CHARACTER c );
		void		RemoveFromSight( void );
		void		Teleport( CHARACTER c );
		void		ExposeToView( CHARACTER c );
		void		HideFromView( CHARACTER c );
		void		Update( CHARACTER c );

		SI32		GetSpeechItem( void ) const;
		UI08		GetSpeechMode( void ) const;

		void		SetSpeechItem( SI32 newValue );
		...
		bool		TakeOffItem( UI08 Layer, CItem *toTakeOff );

		SI32		FirstItem( void ) const;
		SI32		NextItem( void ) const;
		bool		FinishedItems( void ) const;
		SI32		NumItems( void ) const;
