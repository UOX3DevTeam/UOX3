...
function onUse( pUser, iUsed ) 
{
	...
	if( iUsed.container != null )
	{
		...
		if( iPackOwner.serial != pUser.serial )
		{
			...
			srcSock.CustomTarget( 0, "What do you want to spin the cotton on?" );
	}
	else
		pUser.SysMessage( "This has to be in your backpack!" );
}
...
function onCallback0( tSock, targSerial )
{
	...
	{ //Target is a Maptile/Character
		pUser.SysMessage("You cannot spin your cotton on that!");
	}
	...
	{ // In case its a spinning wheel
		...
		var iMakeResource =  pUser.ResourceCount( 0x0df9 );
		if( iMakeResource < 1 )
		{
			pUser.SysMessage( "You dont seem to have any cotton!" );
			return;
		}
		pUser.UseResource( 1, 0x0df9 ); // uses up a resource 
		pUser.SoundEffect( 0x021b, true );
		// add spools of thread
		var itemMade = CreateDFNItem( pUser.socket, pUser, "0x0fa0", false, 6, true, true ); //makes some spools of thread		
