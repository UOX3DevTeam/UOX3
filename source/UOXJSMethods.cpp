...
JSBool CMisc_ReplaceTreeStatic( JSContext *cx, uintN argc, jsval *vp )
{
	...
	SI08 z = static_cast<SI08>( JSVAL_TO_INT( argv[2] ));
	UI08 worldNum = static_cast<UI08>( JSVAL_TO_INT( argv[3] ));
	UI16 oldTileID = static_cast<UI16>( JSVAL_TO_INT( argv[4] ));
	UI16 newTileID = static_cast<UI16>( JSVAL_TO_INT( argv[5] ));
	UI16 newHue = static_cast<UI16>( JSVAL_TO_INT( argv[6] ));

	bool result = false;

	if( LiveStatics != nullptr )
	{
		result = LiveStatics->ReplaceTreeStatic(
			x,
			y,
			z,
			worldNum,
