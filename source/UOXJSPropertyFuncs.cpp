...
JSBool CItemProps_getProperty( JSContext *cx, JSObject *obj, jsval id, jsval *vp )
{
	...
	if( JSVAL_IS_INT( id ))
	{
		...
		switch( JSVAL_TO_INT( id ))
		{
			...
			case CIP_SPDINCREASE:	*vp = INT_TO_JSVAL( gPriv->GetSwingSpeedIncrease() );			break;
...
JSBool CItemProps_setProperty( JSContext *cx, JSObject *obj, jsval id, jsval *vp )
{
	...
	if( JSVAL_IS_INT( id ))
	{
		...
		switch( JSVAL_TO_INT( id ))
		{
			...
			case CIP_SPDINCREASE:	gPriv->SetSwingSpeedIncrease( static_cast<SI16>( encaps.toInt() ));	break;
