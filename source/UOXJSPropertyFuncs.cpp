...
JSBool CItemProps_getProperty( JSContext *cx, JSObject *obj, jsid id, jsval *vp )
{
	...
	if( propID != 0xFFFF )
	{
		...
		switch( propID )
		{
			...
			case CIP_DAMAGEPOISONPERCENT: *vp = INT_TO_JSVAL( gPriv->GetDamageType( POISON )); break;
			case CIP_DAMAGEENERGYPERCENT: *vp = INT_TO_JSVAL( gPriv->GetDamageType( LIGHTNING )); break;
			case CIP_DAMAGECHAOSPERCENT: *vp = INT_TO_JSVAL( gPriv->GetDamageType( CHAOS )); break;
			case CIP_DAMAGELIGHTPERCENT: *vp = INT_TO_JSVAL( gPriv->GetDamageType( LIGHT )); break;
			case CIP_DAMAGERAINPERCENT: *vp = INT_TO_JSVAL( gPriv->GetDamageType( RAIN )); break;
...
JSBool CItemProps_setProperty( JSContext* cx, JSObject* obj, jsid id, JSBool strict, jsval* vp )
{
	...
	if( propID != 0xFFFF )
	{
		...
		switch( propID )
		{
			...
			case CIP_DAMAGEPOISONPERCENT: gPriv->SetDamageType( static_cast<UI08>( std::clamp( encaps.toInt(), 0, 100 )), POISON ); break;
			case CIP_DAMAGEENERGYPERCENT: gPriv->SetDamageType( static_cast<UI08>( std::clamp( encaps.toInt(), 0, 100 )), LIGHTNING ); break;
			case CIP_DAMAGECHAOSPERCENT: gPriv->SetDamageType( static_cast<UI08>( std::clamp( encaps.toInt(), 0, 100 )), CHAOS ); break;
			case CIP_DAMAGELIGHTPERCENT: gPriv->SetDamageType( static_cast<UI08>( std::clamp( encaps.toInt(), 0, 100 )), LIGHT ); break;
			case CIP_DAMAGERAINPERCENT: gPriv->SetDamageType( static_cast<UI08>( std::clamp( encaps.toInt(), 0, 100 )), RAIN ); break;
