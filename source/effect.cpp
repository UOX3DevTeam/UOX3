...
void cEffects::DeathAction(CChar *s, CItem *x, std::uint8_t fallDirection) {
    ...
    for (auto &mSock : FindNearbyPlayers(s)) {
        if (mSock->CurrcharObj() !=
            s) // Death action screws up corpse display for the person who died.
        {
            mSock->Send(&toSend);
        }
    }
...
void cEffects::PlaySpellCastingAnimation(CChar *mChar, std::uint16_t actionId, bool monsterCast, bool monsterAreaCastAnim) {
    if (!monsterCast && (mChar->GetBodyType() == BT_GARGOYLE || ServerConfig::shared().enabled(ServerSwitch::FORECENEWANIMATIONPACKET))) {
        ...
        return;
             ...
             mChar->GetMounted()) // Players and NPC are mounted differently...
    {
        if (actionId == 0x10) {
        ...
        if (!monsterCast) {
            ...
            PlayCharacterAnimation(mChar, (castAnim != 0 ? castAnim : 0x04), 0,
...
void cEffects::PlayStaticAnimation(std::int16_t x, std::int16_t y, std::int8_t z, std::uint16_t effect, std::uint8_t speed, std::uint8_t loop,
    ...
    std::for_each(Network->connClients.begin(), Network->connClients.end(),
                  [&toSend](CSocket *tSock) { tSock->Send(&toSend); });
}
        ...
        for (auto &Cell : MapRegion->PopulateList(nItem)) {
            ...
            for (const auto &tempItem : regItems->collection()) {
                if (!ValidateObject(tempItem) ||
                    tempItem->GetInstanceId() != nItem->GetInstanceId())
                    continue;
...
auto cEffects::CheckTempeffects() -> void {
    ...
    for (auto &Effect : collection) {
        ...
        case 9: // Grind potion (also used for necro stuff)
            if (validChar) {
                ...
                case 0:
                    if (Effect->More2() != 0) {
                        s->TextMessage(nullptr, 1270, EMOTE, 1,
                                       s->GetName().c_str()); // *%s continues grinding*
                    }
                    PlaySound(s, 0x0242);
                    break;
                default:
        ...
        case 17: // Explosion effect (JS and code)
            src = CalcCharObjFromSer(Effect->Source());
            ExplodeItem(src->GetSocket(),
                        static_cast<CItem *>(Effect->ObjPtr())); // explode this item
            break;
                ...
                if (lockType == IT_LOCKEDDOOR || lockType == IT_LOCKEDCONTAINER ||
                    ...
                    if (temp.Effect() != INVALIDID) {
                        ...
                        if (ValidateObject(iCont) && iCont->CanBeObjType(CBaseObject::OT_CHAR)) {
                            ...
                            Effects->PlayStaticAnimation(lockedItem, temp.Effect(), temp.Speed(),
                                                         temp.Loop());
                        }
                    }
                }
    ...
    std::for_each(removeEffects.begin(), removeEffects.end(),
...
void cEffects::TempEffect(CChar *source, CChar *dest, std::uint8_t num, std::uint16_t more1, std::uint16_t more2, std::uint16_t more3,
    ...
    std::for_each(removeEffect.begin(), removeEffect.end(),
                  [](CTEffect *effect) { cwmWorldState->tempEffects.Remove(effect, true); });
    ...
    case 40: // Used by JS timers
        toAdd->ExpireTime(
            BuildTimeValue((static_cast<R32>(more1) + static_cast<R32>(more2) / 1000.0f)));
        toAdd->More1(more3);
        break;
...
bool CTEffect::Save(std::ostream &effectDestination) const {
    ...
    effectDestination << "Expire=" +
                             std::to_string(ExpireTime() - cwmWorldState->GetUICurrentTime()) +
                             newLine;
