...
// object to target location
// o------------------------------------------------------------------------------------------------o
void cEffects::PlayMovingAnimation(std::int16_t srcX, std::int16_t srcY, std::int8_t srcZ, std::int16_t x, std::int16_t y, std::int8_t z, std::uint16_t effect, std::uint8_t speed, std::uint8_t loop, bool explode, std::uint32_t hue, std::uint32_t renderMode) { // 0x0f 0x42 = arrow 0x1b 0xfe=bolt
...
// action/anim
// o------------------------------------------------------------------------------------------------o
void cEffects::PlayCharacterAnimation(CChar *mChar, std::uint16_t actionId, std::uint8_t frameDelay, std::uint8_t frameCount, bool playBackwards) {
...
// o------------------------------------------------------------------------------------------------o
void cEffects::PlayNewCharacterAnimation(CChar *mChar, std::uint16_t actionId, std::uint16_t subActionId, std::uint8_t subSubActionId) {
...
void cEffects::PlaySpellCastingAnimation(CChar *mChar, std::uint16_t actionId, bool monsterCast, bool monsterAreaCastAnim) {
    if (!monsterCast && (mChar->GetBodyType() == BT_GARGOYLE || ServerConfig::shared().enabled(ServerSwitch::FORECENEWANIMATIONPACKET))) {
        ...
        if (!monsterCast) {
            ...
            // frames if no cast anim was defined in creatures.dfn
...
// object
// o------------------------------------------------------------------------------------------------o
void cEffects::PlayStaticAnimation(CBaseObject *target, std::uint16_t effect, std::uint8_t speed, std::uint8_t loop, bool explode) {
...
// location
// o------------------------------------------------------------------------------------------------o
void cEffects::PlayStaticAnimation(std::int16_t x, std::int16_t y, std::int8_t z, std::uint16_t effect, std::uint8_t speed, std::uint8_t loop, bool explode) {
...
// the item
// o------------------------------------------------------------------------------------------------o
auto ExplodeItem(CSocket *mSock, CItem *nItem, std::uint32_t damage = 0, [[maybe_unused]] std::uint8_t damageType = 0, bool explodeNearby = true) -> void {
        ...
        for (auto &Cell : worldMapHandler.PopulateList(nItem)) {
            ...
            for (const auto &tempChar : regChars->collection()) {
                if (!ValidateObject(tempChar) ||
                    tempChar->GetInstanceId() != nItem->GetInstanceId())
                    continue;
        ...
        switch (Effect->Number()) {
            ...
            case 50: {// Unlock a magically locked object
                ...
                if (ValidateObject(myObj)) {
                    ...
                    if (lockType == IT_LOCKEDDOOR || lockType == IT_LOCKEDCONTAINER ||
                        lockType == IT_LOCKEDSPAWNCONT) {
                        switch (lockType) {
                            case IT_LOCKEDDOOR:
                                lockedItem->SetType(IT_DOOR);
                                break;
                            case IT_LOCKEDCONTAINER:
                                lockedItem->SetType(IT_CONTAINER);
                                break;
                            case IT_LOCKEDSPAWNCONT:
                                lockedItem->SetType(IT_SPAWNCONT);
                                break;
                            default:
                                break;
                        }
...
void ReverseEffect(CTEffect *Effect) {
    ...
    if (ValidateObject(s)) {
        switch (Effect->Number()) {
            ...
            case 3:
                s->IncDexterity2(Effect->More1());
                break; // Clumsy Spell (JS)
            case 4:
                s->IncIntelligence2(Effect->More1());
                break; // Feeblemind Spell (JS)
            case 5:
                s->IncStrength2(Effect->More1());
                break; // Weaken Spell
            case 6:
                s->IncDexterity2(-Effect->More1());
                break; // Agility Potion (JS) and Spell (code)
            case 7:
                s->IncIntelligence2(-Effect->More1());
                break; // Cunning Spell
            case 8:
                s->IncStrength2(-Effect->More1());
                break; // Strength Potion (JS) and Spell (code)
            case 11:   // Bless Spell
                s->IncStrength2(-Effect->More1());
                s->IncDexterity2(-Effect->More2());
                s->IncIntelligence2(-Effect->More3());
...
// o------------------------------------------------------------------------------------------------o
void cEffects::TempEffect(CChar *source, CChar *dest, std::uint8_t num, std::uint16_t more1, std::uint16_t more2, std::uint16_t more3, [[maybe_unused]] CItem *targItemPtr) {
    // if( !ValidateObject( source ) || !ValidateObject( dest ))
    ...
    switch (num) {
        ...
        case 2: { // Nightsight Potion (JS) and Spell (code)
            ...
            worldbrightlevel = ServerConfig::shared().ushortValues[UShortValue::BRIGHTLEVEL] ;
        ...
        case 16: // Explosion potion messages (JS)
            ...
            break;
        case 18: // Polymorph Spell
            toAdd->ExpireTime(BuildTimeValue(ServerConfig::shared().timerSetting[TimerSetting::POLYMORPH]));
            toAdd->Dispellable(false);
            
            std::uint16_t k;
            // Grey flag when polymorphed
            dest->SetTimer(tCHAR_CRIMFLAG, BuildTimeValue(ServerConfig::shared().timerSetting[TimerSetting::POLYMORPH]));
            if (dest->IsOnHorse()) {
        ...
        case 26: // Temporarily disable the usage of potions
            toAdd->ExpireTime(BuildTimeValue(ServerConfig::shared().timerSetting[TimerSetting::POTION]));
...
// o------------------------------------------------------------------------------------------------o
void cEffects::TempEffect(CChar *source, CItem *dest, std::uint8_t num, std::uint16_t more1, std::uint16_t more2, [[maybe_unused]] std::uint16_t more3) {
    if (!ValidateObject(dest))
        return;
                    ...
                    while (tag != "o---o") {
                        ...
                        if (tag != "o---o") {
                            ...
                            switch ((UTag.data()[0])) {
                                case 'A':
                                    if (UTag == "ASSOCSCRIPT") {
                                        toLoad->AssocScript(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    break;
                                case 'D':
                                    if (UTag == "DEST") {
                                ...
                                case 'I':
                                    if (UTag == "ITEMPTR") {
                                        auto objSer = static_cast<serial_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0));
                                ...
                                case 'M':
                                    if (UTag == "MORE1") {
                                        toLoad->More1(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    if (UTag == "MORE2") {
                                        toLoad->More2(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    if (UTag == "MORE3") {
                                        toLoad->More3(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    break;
                                case 'N':
                                    if (UTag == "NUMBER") {
                                        toLoad->Number(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    break;
                                case 'O':
                                    if (UTag == "OBJPTR") {
                                ...
                                case 'S':
                                    if (UTag == "SOURCE") {
                                        toLoad->Source(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                                    }
                                    break;
                                default:
