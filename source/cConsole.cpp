...
//o-----------------------------------------------------------------------------------------------o
//| Function	-	void Log( const char *toLog, const char *filename, ... )
//o-----------------------------------------------------------------------------------------------o
...
//o-----------------------------------------------------------------------------------------------o
//| Function	-	void Log( const char *toLog, ... )
//o-----------------------------------------------------------------------------------------------o
...
//o-----------------------------------------------------------------------------------------------o
//| Function	-	void Error( const char *toWrite, ... )
//o-----------------------------------------------------------------------------------------------o
...
//|	Date		-	24th December, 2001
//|	Programmer	-	DarkStorm
//|	Changes		-	Maarc - 7th February, 2003 - Made it use PrintSpecial
//o-----------------------------------------------------------------------------------------------o
...
//o-----------------------------------------------------------------------------------------------o
//|	Function	-	void Warning( const char *toWrite, ... )
...
//o-----------------------------------------------------------------------------------------------o
//|	Function	-	void PrintSpecial( UI08 colour, const char *toPrint, ... )
//|	Date		-	2/7/2003
//|	Programmer	-	Maarc
//|	Team		-	UOX3 DevTeam
//o-----------------------------------------------------------------------------------------------o
	...
	else 
	{
		...
		case 'Y':
			...
			case 'M':
				...
				messageLoop << "     Cache:";
                sprintf( temp, "        Tiles: %zu bytes", Map->GetTileMem() );
				messageLoop << temp;
                sprintf( temp, "        Multis: %zu bytes", Map->GetMultisMem() );
				messageLoop << temp;
				UI32 m, n;
				...
				messageLoop << temp;
                sprintf( temp, "        You save I: %lu & C: %lu bytes!", m * sizeof(CItem) - ObjectFactory::getSingleton().CountOfObjects( OT_ITEM ), m * sizeof( CChar ) - ObjectFactory::getSingleton().CountOfObjects( OT_CHAR ) );
				total += tmp = 69 * sizeof( SpellInfo );
				sprintf( temp, "     Spells: %i bytes", tmp );
				messageLoop << "     Sizes:";
                sprintf( temp, "        CItem  : %lu bytes", sizeof( CItem ) );
				messageLoop << temp;
                sprintf( temp, "        CChar  : %lu bytes", sizeof( CChar ) );
				messageLoop << temp;
                sprintf( temp, "        TEffect: %lu bytes %lui total)", sizeof( CTEffect ), sizeof( CTEffect ) * cwmWorldState->tempEffects.Num() );
				messageLoop << temp;
...
//|									
//|	Changes		-	10/21/2002	-	EviLDeD - Xuri found the bug in one spot, just
