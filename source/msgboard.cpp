            ...
            while (file) {
                ...
                for (serial_t postAck = mSock->FirstPostAck(); !mSock->FinishedPostAck();) {
                    if (msgBoardPost.serial == postAck) {
                        ...
                        if (buffer[0] == 0x05) {
                            file.read(buffer, 4);
                            msgBoardPost.parentSerial =
                            CalcSerial(buffer[0], buffer[1], buffer[2], buffer[3]);
                        }
                        else {
...
void MsgBoardPost(CSocket *tSock) {
    ...
    for (lIter = msgBoardPost.msgBoardLine.begin(); lIter != msgBoardPost.msgBoardLine.end();
        ...
        if (msgBoardPost.msgBoardLine[j][msgBoardPost.msgBoardLine[j].size()] > 0) {
            auto checkForNullTerminator =
            msgBoardPost.msgBoardLine[j][msgBoardPost.msgBoardLine[j].size() - 1];
...
bool MsgBoardReadPost(std::istream &file, MsgBoardPost_st &msgBoardPost,
    ...
    if (!file.fail() && (msgBoardPost.serial == msgSerial || msgSerial == INVALIDSERIAL)) {
        ...
        for (std::uint8_t i = 0; i < msgBoardPost.lines; ++i) {
            ...
            // Stuff contents of tmpLine into msgBoardLine std::vector<std::string> in msgBoardPost
            // struct, then add null terminator
    ...
    // If Toggle value has been set to 0x00 for a post, return false so it won't be included when
    // writing posts back to file later
    return false;
}
...
void MsgBoardRemovePost(CSocket *mSock) {
    ...
    if (file.is_open()) {
        ...
        while (!file.eof()) {
            ...
            // Read first 2 characters from filestream into rBuffer, starting from first line in
            // file
            file.read(rBuffer, 2);
            ...
            // Read the next 4 characters from filestream into rBuffer, and use this to attempt
            // calculating post serial
            file.read(rBuffer, 4);
            ...
            if (tmpSerial == msgSer) {
                ...
                // Move stream's "set" position to the start of the post in question, and overwrite
                // first 2 bytes with 0x00 to mark it for deletion This will hide it from view, but
                // not actually delete it from the file itself; that's handled by message board
                // maintenance
            ...
            if (removeReply || !foundPost) {
                // Move stream's "get" position relative to current position, to... what? Look for a
                // reply?
                file.seekg(tmpSize - 11, std::ios::cur);
                
                // Read another serial candidate into the buffer, and try to calculate post serial
                // from buffer
                file.read(rBuffer, 4);
                tmpSerial = CalcSerial(rBuffer[0], rBuffer[1], rBuffer[2], rBuffer[3]);
                
                if (removeReply) {
                    // Send packet to remove item (each post is an item in a container - the
                    // container being the message board) visually from message board
                    CPRemoveItem toRemove;
                    toRemove.Serial((tmpSerial + BASEITEMSERIAL));
                    mSock->Send(&toRemove);
                }
                else if (tmpSerial == msgSer) {
                    // Mark post as found, but don't break out of the loop, in order to get all
                    // replies
                    foundPost = true;
                    
                    // Move stream's "set" position to the start of the post in question, and
                    // overwrite first 2 bytes with 0x00 to mark it for deletion This will hide it
                    // from view, but not actually delete it from the file itself; that's handled by
                    // message board maintenance
                    file.seekp(totalSize + 2, std::ios::beg);
                    file.put(0x00);
            ...
            // Make sure we don't get stuck in an infinite loop... don't allow seeking past the end
            // of the file!
            totalSize += tmpSize;
    ...
    // replies to it have been removed already
    if (foundPost) {
        CPRemoveItem toRemove;
...
auto MsgBoardPostQuest(CChar *mNPC, const QuestTypes questType) -> bool {
    ...
    auto fileName = "region"s + util::ntos(mNPC->GetQuestOrigRegion()) + ".bbf"s;
    ...
    switch (questType) {
        case QT_ESCORTQUEST:
            ...
            for (const auto &sec : EscortData->collection()) {
                ...
                while (position != std::string::npos) {
                    fullLine.replace(position, 2, util::format("%d %d", mNPC->GetX(), mNPC->GetY()));
                    position = fullLine.find("%l");
                }
            ...
            if (entry.path().extension() == ".bbf") {
                ...
                if (file.is_open()) {
                    ...
                    while (!file.eof()) {
                        ...
                        // If post had Toggle value other than 0x00, add it to the list of posts to
                        // write back out to file later
