...
auto CBaseObject::describeBody(ObjectDescription &descrip) -> void {
    ...
    if (ValidateObject(multis)){
        ...
        try{
            serial = oldstrutil::number(multis->GetSerial(),16);
        }
        catch(...){
    ...
    auto temp_in2 = in2;
    if(objType == OT_CHAR){
        auto myChar = static_cast<CChar *>(this);

        // For every equipped item
        // We need to reduce Str2+Dex2+Int2
        for( auto myItem = myChar->FirstItem(); !myChar->FinishedItems(); myItem = myChar->NextItem() ){
