...
void FileArchive(void) {
    ...
    if (makeResult) {
        Console::shared()
            << "NOTICE: Accounts not backed up. Archiving will change. Sorry for the trouble."
            << myendl;
        ...
        Console::shared() << "Cannot create backup directory, please check available disk space"
                          << myendl;
    }
