...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetOldTargLocX(void) const { return oldTargLocX; }
void CBaseObject::SetOldTargLocX(SI16 newValue) { oldTargLocX = newValue; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetOldTargLocY(void) const { return oldTargLocY; }
void CBaseObject::SetOldTargLocY(SI16 newValue) { oldTargLocY = newValue; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetX(void) const { return x; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetY(void) const { return y; }
...
// o------------------------------------------------------------------------------------------------o
SI08 CBaseObject::GetZ(void) const { return z; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CBaseObject::GetResist(WeatherType damage) const { return resistances[damage]; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CBaseObject::GetId(void) const { return id; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CBaseObject::GetColour(void) const { return colour; }
...
// o------------------------------------------------------------------------------------------------o
SI32 CBaseObject::GetWeight(void) const { return weight; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetStrength(void) const { return strength; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetDexterity(void) const { return dexterity; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetIntelligence(void) const { return intelligence; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetHP(void) const { return hitpoints; }
...
// o------------------------------------------------------------------------------------------------o
void CBaseObject::IncHP(SI16 amtToChange) { SetHP(hitpoints + amtToChange); }
...
// o------------------------------------------------------------------------------------------------o
UI08 CBaseObject::GetDir(void) const { return dir; }
...
UI08 CBaseObject::GetSpawn(UI08 part) const {
    ...
    switch (part) {
    case 1:
        rValue = static_cast<UI08>(spawnSerial >> 24);
        break;
    case 2:
        rValue = static_cast<UI08>(spawnSerial >> 16);
        break;
    case 3:
        rValue = static_cast<UI08>(spawnSerial >> 8);
        break;
    case 4:
        rValue = static_cast<UI08>(spawnSerial % 256);
        break;
    }
    return rValue;
}
...
UI08 CBaseObject::GetSerial(UI08 part) const {
    switch (part) {
    case 1:
        return static_cast<UI08>(serial >> 24);
    case 2:
        return static_cast<UI08>(serial >> 16);
    case 3:
        return static_cast<UI08>(serial >> 8);
    case 4:
        return static_cast<UI08>(serial % 256);
    }
    return 0;
}
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetLoDamage(void) const { return loDamage; }
...
// o------------------------------------------------------------------------------------------------o
SI32 CBaseObject::GetFilePosition(void) const { return FilePosition; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetStamina(void) const { return stamina; }
void CBaseObject::SetStamina(SI16 stam) { stamina = stam; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetMana(void) const { return mana; }
void CBaseObject::SetMana(SI16 mn) { mana = mn; }
...
auto CBaseObject::GetOrigin() const -> UI08 { return origin; }
...
// o------------------------------------------------------------------------------------------------o
std::vector<UI16> CBaseObject::GetScriptTriggers(void) { return scriptTriggers; }
...
// o------------------------------------------------------------------------------------------------o
void CBaseObject::IncStrength(SI16 toInc) { SetStrength(strength + toInc); }
...
// o------------------------------------------------------------------------------------------------o
void CBaseObject::IncDexterity(SI16 toInc) { SetDexterity(dexterity + toInc); }
...
// o------------------------------------------------------------------------------------------------o
void CBaseObject::IncIntelligence(SI16 toInc) { SetIntelligence(intelligence + toInc); }
...
bool CBaseObject::HandleLine(std::string &UTag, std::string &data) {
    ...
    case 'D':
        if (UTag == "DAMAGE") {
            if (csecs.size() >= 2) {
                loDamage = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0));
                hiDamage = static_cast<SI16>(
            ...
            SetDamageable(util::ston<UI08>(data) == 1);
        }
            ...
            if (csecs.size() >= 2) {
                dexterity = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0));
                dx2 = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0));
            }
            else {
                dexterity =
            ...
            dx2 = static_cast<SI16>(std::stoi(util::trim(util::strip(data, "//")), nullptr, 0));
        }
            ...
            SetDisabled(util::ston<SI16>(data) == 1);
        }
        else {
            rValue = false;
        }
    ...
    case 'H':
        if (UTag == "HITPOINTS") {
            hitpoints = util::ston<SI16>(data);
        }
        else if (UTag == "HIDAMAGE") {
            hiDamage = util::ston<SI16>(data);
        }
        else {
            rValue = false;
        }
    ...
    case 'I':
        if (UTag == "ID") {
            id = util::ston<SI16>(data);
        }
        else if (UTag == "INTELLIGENCE") {
            if (data.find(",") != std::string::npos) {
                intelligence = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0));
                in2 = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0));
            }
            else {
                intelligence = util::ston<SI16>(data);
            }
        }
        else if (UTag == "INTELLIGENCE2") {
            in2 = util::ston<SI16>(data);
        }
        else {
            rValue = false;
        }
    ...
    case 'K':
        if (UTag == "KARMA") {
            SetKarma(util::ston<SI16>(data));
        }
        else if (UTag == "KILLS") {
            SetKills(util::ston<SI16>(data));
        }
        else {
            rValue = false;
        }
    ...
    case 'L':
        if (UTag == "LOCATION") {
            x = static_cast<SI16>(std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0));
            y = static_cast<SI16>(std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0));
            z = static_cast<SI08>(std::stoi(util::trim(util::strip(csecs[2], "//")), nullptr, 0));
            worldNumber =
                static_cast<UI08>(std::stoi(util::trim(util::strip(csecs[3], "//")), nullptr, 0));

            // Backwards compatibility with pre-instanceId worldfiles
            if (csecs.size() >= 5) {
                instanceId = static_cast<SI16>(
    ...
    case 'M':
        if (UTag == "MANA") {
            mana = util::ston<SI16>(data);
        }
        else if (UTag == "MULTIID") {
            tempMulti = (util::ston<UI32>(data));
            multis = nullptr;
        }
        else {
            rValue = false;
    ...
    case 'R':
        if (UTag == "RACE") {
            race = util::ston<UI16>(data);
        }
        else if (UTag == "REPUTATION") {
            if (csecs.size() == 3) {
                SetFame(static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                SetKarma(static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                SetKills(static_cast<SI16>(
    ...
    case 'S':
        ...
        if (UTag == "SECTIONID") {
            ...
            stamina = util::ston<SI16>(data);
        }
        else if (UTag == "SPAWNERID") {
            spawnSerial = util::ston<UI32>(data);
        }
        else if (UTag == "SERIAL") {
            serial = util::ston<UI32>(data);
        }
        else if (UTag == "STRENGTH") {
            if (csecs.size() >= 2) {
                strength = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0));
                st2 = static_cast<SI16>(
                    std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0));
            }
            else {
                strength = util::ston<SI16>(data);
            }
        }
        else if (UTag == "STRENGTH2") {
            st2 = util::ston<SI16>(data);
        }
        else if (UTag == "SCPTRIG") {
            // scriptTrig	= util::ston<UI16>(data);
            UI16 scriptId = util::ston<UI16>(data);
            if (scriptId != 0 && scriptId != 65535) {
                ...
                if (toExecute == nullptr) {
                    ...
                    this->AddScriptTrigger(util::ston<UI16>(data));
                }
            }
        }
        else {
    ...
    case 'W':
        if (UTag == "WEIGHT") {
            SetWeight(util::ston<SI32>(data));
        }
        else if (UTag == "WIPE") {
            SetWipeable(util::ston<UI08>(data) == 1);
        }
        else if (UTag == "WORLDNUMBER") {
            worldNumber = util::ston<UI08>(data);
        }
        else {
            rValue = false;
        }
...
// o------------------------------------------------------------------------------------------------o
UI08 CBaseObject::WorldNumber(void) const { return worldNumber; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CBaseObject::GetInstanceId(void) const { return instanceId; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CBaseObject::GetSubRegion(void) const { return subRegion; }
void CBaseObject::SetSubRegion(UI16 value) { subRegion = value; }
...
// o------------------------------------------------------------------------------------------------o
UI08 CBaseObject::GetPoisoned(void) const { return poisoned; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetCarve(void) const { return carve; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetKarma(void) const { return karma; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetFame(void) const { return fame; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CBaseObject::GetKills(void) const { return kills; }
