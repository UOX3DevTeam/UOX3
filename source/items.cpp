	...
	for( const auto &sec : toApply->collection2() )
	{
		...
		switch( tag )
		{
			...
			case DFNTAG_SPELLBOOKDATA:
				if( ssecs.size() == 3 )
				{
					applyTo->SetSpellBookFirstSpell( static_cast<SI32>( std::stol( oldstrutil::trim( oldstrutil::removeTrailing( ssecs[0], "//" )), nullptr, 0 )));
					applyTo->SetSpellBookSpellCount( static_cast<UI16>( std::stoul( oldstrutil::trim( oldstrutil::removeTrailing( ssecs[1], "//" )), nullptr, 0 )));
					applyTo->SetSpellBookFirstScroll( static_cast<UI16>( std::stoul( oldstrutil::trim( oldstrutil::removeTrailing( ssecs[2], "//" )), nullptr, 0 )));
				}
				else
				{
