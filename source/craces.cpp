...
bool cRaces::InvalidRace(RACEID x) const { return (x >= races.size()); }
    ...
    if (CombatMods != nullptr) {
        ...
        if (tag.empty()) // location didn't exist!!!
        {
            ...
            if (util::upper(tag) != "MODCOUNT") {
                ...
                UI32 modifierCount =
                    static_cast<UI32>(std::stoul(CombatMods->GrabData(), nullptr, 0));
                if (modifierCount < 4) {
    ...
    if (s->GetRaceGate() == 65535 || always) {
        ...
        if (pRace->GenderRestriction() != 0) {
            if (pRace->GenderRestriction() != FEMALE &&
                (s->GetId() == 0x0191 || s->GetId() == 0x025E || s->GetId() == 0x029B ||
                 s->GetId() == 0x02EF || s->GetId() == 0x00B8 || s->GetId() == 0x00BA)) {
                if (mSock) {
                    mSock->SysMessage(370); // You are not of the right gender!
                }
                return;
            }
            if (pRace->GenderRestriction() != MALE &&
                (s->GetId() == 0x0190 || s->GetId() == 0x025D || s->GetId() == 0x029A ||
                 s->GetId() == 0x02EE || s->GetId() == 0x00B7 || s->GetId() == 0x00B9)) {
                if (mSock) {
                    mSock->SysMessage(370); // You are not of the right gender!
                }
                return;
        ...
        if (pRace->RequiresBeard() && (s->GetId() == 0x0190 || s->GetId() == 0x025D) &&
            !ValidateObject(beardobject)) {
...
//|	Purpose		-	Gets/Sets skill bonus associated with race (?)
// o------------------------------------------------------------------------------------------------o
SKILLVAL CRace::Skill(SI32 skillNum) const { return iSkills[skillNum]; }
void CRace::Skill(SKILLVAL newValue, SI32 iNum) { iSkills[iNum] = newValue; }
...
// o------------------------------------------------------------------------------------------------o
LIGHTLEVEL CRace::LightLevel(void) const { return lightLevel; }
void CRace::LightLevel(LIGHTLEVEL newValue) { lightLevel = newValue; }
...
// o------------------------------------------------------------------------------------------------o
COLDLEVEL CRace::ColdLevel(void) const { return coldLevel; }
void CRace::ColdLevel(COLDLEVEL newValue) { coldLevel = newValue; }
...
// o------------------------------------------------------------------------------------------------o
HEATLEVEL CRace::HeatLevel(void) const { return heatLevel; }
void CRace::HeatLevel(HEATLEVEL newValue) { heatLevel = newValue; }
...
// o------------------------------------------------------------------------------------------------o
LIGHTLEVEL CRace::NightVision(void) const { return nightVision; }
void CRace::NightVision(LIGHTLEVEL newValue) { nightVision = newValue; }
...
// o------------------------------------------------------------------------------------------------o
ARMORCLASS CRace::ArmourClassRestriction(void) const { return armourRestrict; }
void CRace::ArmourClassRestriction(ARMORCLASS newValue) { armourRestrict = newValue; }
...
//|	Purpose		-	Gets/Sets ???
// o------------------------------------------------------------------------------------------------o
RANGE CRace::VisibilityRange(void) const { return visDistance; }
void CRace::VisibilityRange(RANGE newValue) { visDistance = newValue; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CRace::GetHungerRate(void) const { return hungerRate; }
void CRace::SetHungerRate(UI16 newValue) { hungerRate = newValue; }
...
// o------------------------------------------------------------------------------------------------o
UI16 CRace::GetThirstRate(void) const { return thirstRate; }
void CRace::SetThirstRate(UI16 newValue) { thirstRate = newValue; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CRace::GetHungerDamage(void) const { return hungerDamage; }
void CRace::SetHungerDamage(SI16 newValue) { hungerDamage = newValue; }
...
void CRace::NumEnemyRaces(SI32 iNum) { racialEnemies.resize(iNum); }
...
// o------------------------------------------------------------------------------------------------o
void CRace::RaceRelation(RaceRelate value, RACEID race) { racialEnemies[race] = value; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CRace::HPModifier(void) const { return HPMod; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CRace::ManaModifier(void) const { return ManaMod; }
...
// o------------------------------------------------------------------------------------------------o
SI16 CRace::StamModifier(void) const { return StamMod; }
    ...
    for (tag = RacialPart->First(); !RacialPart->AtEnd(); tag = RacialPart->Next()) {
        ...
        case 'A': {
            if (UTag == "ALLOWEQUIPLIST") {
                ...
                std::string subSect = std::string("EQUIPLIST ") +
                                      util::ntos(static_cast<UI08>(std::stoul(data, nullptr, 0)));
                    ...
                    case 'I':
                        if (subUTag == "ITEMS") {
                            ...
                            for (size_t i = 0; i < csecs.size() - 1; i++) {
                                UI16 temp = static_cast<UI16>(std::stoul(
        ...
        case 'B':
            if (UTag == "BANEQUIPLIST") {
                ...
                beardColours.push_back(
                    ColourPair_st(beardMin, static_cast<UI16>(std::stoul(data, nullptr, 0))));
            }
        ...
        case 'H':
            if (UTag == "HAIRMIN") {
                ...
                hairColours.push_back(
                    ColourPair_st(hairMin, static_cast<UI16>(std::stoul(data, nullptr, 0))));
            }
                ...
                if (csecs.size() > 1) {
                    SetHungerRate(static_cast<SI16>(
                        std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    SetHungerDamage(static_cast<SI16>(
        ...
        case 'R':
            ...
            else if (UTag == "RAINSECS") // how often light affects in secs
            {
                ...
                if (raceDiff > static_cast<SI32>(racialEnemies.size())) {
                    Console::shared() << "Error in race " << static_cast<UI32>(sectNum)
        ...
        case 'S':
            if (UTag == "STRCAP") {
                ...
                skinColours.push_back(
                    ColourPair_st(skinMin, static_cast<UI16>(std::stoul(data, nullptr, 0))));
            }
        ...
        case 'T':
            if (UTag == "THIRST") {
                ...
                auto csecs = oldstrutil::sections(data, ",");
                if (csecs.size() > 1) {
                    SetThirstRate(static_cast<SI16>(
                        std::stoi(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    SetThirstDrain(static_cast<SI16>(
                        std::stoi(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                }
                else {
                    SetThirstRate(0);
