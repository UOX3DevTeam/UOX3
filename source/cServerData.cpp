...
auto CServerData::Directory( CSDDirectoryPaths dp, std::string value ) -> void
{
	...
	if( dp != CSDDP_COUNT )
	{
		...
		else
		{
			...
			sText = oldstrutil::fixDirectory( std::string(sText) );
			...
			if( error )
			{
				Console.Error( util::format( "%s %s does not exist", verboseDirectory.c_str(), std::string(sText).c_str() ));
				Shutdown( FATAL_UOX3_DIR_NOT_FOUND );
			}
			else
			{
...
//o------------------------------------------------------------------------------------------------o
auto CServerData::EraStringToEnum( const std::string_view eraString, bool useDefault, bool inheritCore ) -> ExpansionRuleset
{
	auto rValue = ER_CORE;
	if( useDefault )
	{
	...
	if( input.is_open() )
	{
		...
		while( input.good() && !input.eof() )
		{
			...
			if( input.gcount() > 0 )
			{
				...
				auto line = std::string(util::trim( util::strip( std::string( input_buffer ), "//" )));
				if( !line.empty() )
				{
					switch( static_cast<int>( state ))
					{
						...
						case static_cast<int>( search_t::endsection ):
						{
							if( line[0] != '}' )
							{
								...
								try
								{
									if( HandleLine( std::string(key), std::string(value) ))
									{
										rValue = true;
									}
								}
								catch( const std::exception &e )
								{
									...
									Console.Error( util::format( "Entry was: %s = %s", std::string(key).c_str(), std::string(value).c_str() ));
...
auto CServerData::ServerLocation( std::string toSet ) -> void
{
	...
	if( csecs.size() >= 7 )
	{
		...
		toAdd.x 		= static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[2], "//" ))), nullptr, 0 ));
		toAdd.y 		= static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[3], "//" ))), nullptr, 0 ));
		toAdd.z 		= static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[4], "//" ))), nullptr, 0 ));
		toAdd.worldNum 	= static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[5], "//" ))), nullptr, 0 ));
		if( csecs.size() == 7 )
		{
			toAdd.clilocDesc	= static_cast<UI32>( std::stoul( std::string(util::trim( util::strip( csecs[6], "//" ))), nullptr, 0 ));
		}
		else
		{
			toAdd.instanceId	= static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[6], "//" ))), nullptr, 0 ));
			toAdd.clilocDesc	= static_cast<UI32>( std::stoul( std::string(util::trim( util::strip( csecs[7], "//" ))), nullptr, 0 ));
		}
	...
	while( tag != "o---o" )
	{
		...
		if( tag != "o---o" )
		{
			...
			else if( UTag == "MOON" )
			{
				...
				if( csecs.size() > 1 )
				{
					ServerMoon( 0, static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[0], "//" ))), nullptr, 0 )));
					ServerMoon( 1, static_cast<SI16>( std::stoi( std::string(util::trim( util::strip( csecs[1], "//" ))), nullptr, 0 )));
				}
			}
		}
	}
