...
const std::uint32_t BIT_DYEABLE = 3;
const std::uint32_t BIT_CORPSE = 4;
const std::uint32_t BIT_HELDONCURSOR = 5;
const std::uint32_t BIT_GUARDED = 6;
const std::uint32_t BIT_SPAWNERLIST = 7;

const std::int32_t DEFITEM_RANDVALUE = 0;
const std::int16_t DEFITEM_GOOD = -1;
...
const std::uint8_t DEFITEM_MAXRANGE = 0;
const std::uint8_t DEFITEM_BASERANGE = 0;
const std::uint16_t DEFITEM_USESLEFT = 0;
const std::uint16_t DEFITEM_MAXUSES = 0;
const std::uint16_t DEFITEM_REGIONNUM = 255;
const std::uint16_t DEFITEM_TEMPLASTTRADED = 0;
const std::int8_t DEFITEM_STEALABLE = 1;
...
//|	Purpose		-	Constructor
// o------------------------------------------------------------------------------------------------o
CItem::CItem() : CBaseObject(), contObj(nullptr), glowEffect(DEFITEM_GLOWEFFECT), glow(DEFITEM_GLOW), glowColour(DEFITEM_GLOWCOLOUR), madeWith(DEFITEM_MADEWITH), rndValueRate(DEFITEM_RANDVALUE), good(DEFITEM_GOOD), rank(DEFITEM_RANK), armorClass(DEFITEM_ARMORCLASS), restock(DEFITEM_RESTOCK), movable(DEFITEM_MOVEABLE), tempTimer(DEFITEM_TEMPTIMER), decayTime(DEFITEM_DECAYTIME), spd(DEFITEM_SPEED), maxHp(DEFITEM_MAXHP), amount(DEFITEM_AMOUNT), layer(DEFITEM_LAYER), type(DEFITEM_TYPE), offspell(DEFITEM_OFFSPELL), entryMadeFrom(DEFITEM_ENTRYMADEFROM), creator(DEFITEM_CREATOR), gridLoc(DEFITEM_GRIDLOC), weightMax(DEFITEM_WEIGHTMAX), baseWeight(DEFITEM_BASEWEIGHT), maxItems(DEFITEM_MAXITEMS), maxRange(DEFITEM_MAXRANGE), baseRange(DEFITEM_BASERANGE), maxUses(DEFITEM_MAXUSES), usesLeft(DEFITEM_USESLEFT), regionNum(DEFITEM_REGIONNUM), tempLastTraded(DEFITEM_TEMPLASTTRADED), stealable(DEFITEM_STEALABLE) {
    ...
    if (ValidateObject(newCont)) {
        ...
        if (newCont->GetObjType() == OT_CHAR) {
            ...
            if (ValidateObject(charWearing)) {
                if (charWearing->IsDead() &&
                    (this->GetLayer() == IL_HAIR || this->GetLayer() == IL_FACIALHAIR)) {
                    // if charWaring is dead, it means we're setting the cont on a duped item
            ...
            CTownRegion *tRegion =
            CalcRegionFromXY(charWearing->GetX(), charWearing->GetY(), charWearing->WorldNumber(), charWearing->GetInstanceId(), this);
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsDoorOpen() const -> bool { return bools.test(BIT_DOOROPEN); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsPileable() const -> bool { return bools.test(BIT_PILEABLE); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsMarkedByMaker() const -> bool { return bools.test(BIT_MAKERSMARK); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsDyeable() const -> bool { return bools.test(BIT_DYEABLE); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsCorpse() const -> bool { return bools.test(BIT_CORPSE); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsHeldOnCursor() const -> bool { return bools.test(BIT_HELDONCURSOR); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsGuarded() const -> bool { return bools.test(BIT_GUARDED); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetName2() const -> const std::string & { return name2; }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IncZ(std::int16_t newValue) -> void {
    SetZ(z + newValue);
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IncLocation(std::int16_t xInc, std::int16_t yInc) -> void {
...
// o------------------------------------------------------------------------------------------------o
void CItem::SetOldLocation(std::int16_t newX, std::int16_t newY, std::int8_t newZ) {
    oldLocX = newX;
    oldLocY = newY;
    oldLocZ = newZ;
    UpdateRegion();
...
// o------------------------------------------------------------------------------------------------o
void CItem::SetLocation(std::int16_t newX, std::int16_t newY, std::int8_t newZ) {
...
// instance
// o------------------------------------------------------------------------------------------------o
void CItem::SetLocation(std::int16_t newX, std::int16_t newY, std::int8_t newZ, std::uint8_t world, std::uint16_t instanceId) {
...
auto CItem::GetRegionNum() const -> std::uint16_t { return regionNum; }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetOffSpell() const -> std::int8_t { return offspell; }
auto CItem::SetOffSpell(std::int8_t newValue) -> void {
    offspell = newValue;
    UpdateRegion();
}
...
// value
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmount() const -> std::uint16_t { return amount; }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IncAmount(std::int32_t incValue, bool noDelete) -> bool {
    bool iDeleted = false;
    std::int32_t newAmt = (amount + incValue);
    if (newAmt >= 0) {
        if (newAmt > MAX_STACK) {
            SetAmount(MAX_STACK);
        }
        else {
            SetAmount(static_cast<std::uint16_t>(newAmt));
        }
    }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetMaxUses() const -> std::uint16_t { return maxUses; }
auto CItem::SetMaxUses(std::uint16_t newValue) -> void {
    maxUses = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetUsesLeft() const -> std::uint16_t { return usesLeft; }
auto CItem::SetUsesLeft(std::uint16_t newValue) -> void {
    usesLeft = newValue;
    Dirty(UT_UPDATE);
    UpdateRegion();
}
...
// speed
// o------------------------------------------------------------------------------------------------o
auto CItem::GetSpeed() const -> std::uint8_t { return spd; }
auto CItem::SetSpeed(std::uint8_t newValue) -> void {
    spd = newValue;
    UpdateRegion();
}
...
// weapons
// o------------------------------------------------------------------------------------------------o
auto CItem::GetMaxRange() const -> std::uint8_t { return maxRange; }
auto CItem::SetMaxRange(std::uint8_t newValue) -> void {
    maxRange = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetBaseRange() const -> std::uint8_t { return baseRange; }
auto CItem::SetBaseRange(std::uint8_t newValue) -> void {
    baseRange = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetMovable() const -> std::int8_t { return movable; }
auto CItem::SetMovable(std::int8_t newValue) -> void {
    movable = newValue;
    UpdateRegion();
}
...
// sell it for
// o------------------------------------------------------------------------------------------------o
auto CItem::GetSellValue() const -> std::uint32_t { return value[1]; }
auto CItem::SetSellValue(std::uint32_t newValue) -> void {
    value[1] = newValue;
    UpdateRegion();
}
...
// it
// o------------------------------------------------------------------------------------------------o
auto CItem::GetBuyValue() const -> std::uint32_t { return value[0]; }
auto CItem::SetBuyValue(std::uint32_t newValue) -> void {
    value[0] = newValue;
    UpdateRegion();
}
...
// it
// o------------------------------------------------------------------------------------------------o
auto CItem::GetVendorPrice() const -> std::uint32_t { return value[2]; }
auto CItem::SetVendorPrice(std::uint32_t newValue) -> void {
    value[2] = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetRestock() const -> std::uint16_t { return restock; }
auto CItem::SetRestock(std::uint16_t newValue) -> void {
    restock = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetRank() const -> std::int8_t { return rank; }
auto CItem::SetRank(std::int8_t newValue) -> void {
    rank = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetGood() const -> std::int16_t { return good; }
auto CItem::SetGood(std::int16_t newValue) -> void {
    good = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetRndValueRate() const -> std::int32_t { return rndValueRate; }
auto CItem::SetRndValueRate(std::int32_t newValue) -> void { rndValueRate = newValue; }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetMadeWith() const -> std::int8_t { return madeWith; }
auto CItem::SetMadeWith(std::int8_t newValue) -> void {
    madeWith = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmmoId() const -> std::uint16_t { return ammo[0]; }
auto CItem::SetAmmoId(std::uint16_t newValue) -> void {
    ammo[0] = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmmoHue() const -> std::uint16_t { return ammo[1]; }
auto CItem::SetAmmoHue(std::uint16_t newValue) -> void {
    ammo[1] = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmmoFX() const -> std::uint16_t { return ammoFX[0]; }
auto CItem::SetAmmoFX(std::uint16_t newValue) -> void {
    ammoFX[0] = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmmoFXHue() const -> std::uint16_t { return ammoFX[1]; }
auto CItem::SetAmmoFXHue(std::uint16_t newValue) -> void {
    ammoFX[1] = newValue;
    UpdateRegion();
}
...
// weapons
// o------------------------------------------------------------------------------------------------o
auto CItem::GetAmmoFXRender() const -> std::uint16_t { return ammoFX[2]; }
auto CItem::SetAmmoFXRender(std::uint16_t newValue) -> void {
    ammoFX[2] = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetWeightMax() const -> std::int32_t { return weightMax; }
auto CItem::SetWeightMax(std::int32_t newValue) -> void {
    weightMax = newValue;
    UpdateRegion();
}
...
// container weight
// o------------------------------------------------------------------------------------------------o
auto CItem::GetBaseWeight() const -> std::int32_t { return baseWeight; }
auto CItem::SetBaseWeight(std::int32_t newValue) -> void {
    baseWeight = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetMaxItems() const -> std::uint16_t { return maxItems; }
auto CItem::SetMaxItems(std::uint16_t newValue) -> void {
    maxItems = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsLockedDown() const -> bool { return (movable == 3); }
...
auto CItem::IsShieldType() const -> bool {
    return (id >= 0x1B72 && id <= 0x1B7B) || (id >= 0x1BC3 && id <= 0x1BC7) || (id >= 0x4200 && id <= 0x420B) || (id >= 0x4228 && id <= 0x422C) || (id == 0xA649 || id == 0xA64A) || (id == 0xA831 || id == 0xA832);
}
...
bool CItem::DumpBody(std::ostream &outStream) const {
    ...
    outStream << "Layer=0x" << static_cast<std::int16_t>(GetLayer()) << newLine;
...
bool CItem::HandleLine(std::string &UTag, std::string &data) {
    ...
    if (!rValue) {
        ...
        switch ((UTag.data()[0])) {
            case 'A':
                if (UTag == "AMMO") {
                    if (csecs.size() == 2) { SetAmmoId(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetAmmoHue(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                    }
                    else {
                        SetAmmoId(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                        SetAmmoHue((0));
                    }
                    rValue = true;
                }
                    ...
                    if (csecs.size() == 2) {
                        SetAmmoFX(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetAmmoFXHue(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetAmmoFXRender(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                    }
                    else {
                        SetAmmoFX(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                        SetAmmoFXHue((0));
                        SetAmmoFXRender((0));
                    }
                    rValue = true;
                    ...
                    amount = static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0));
                    rValue = true;
                }
                else if (UTag == "AC") {
                    SetArmourClass(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'B':
            ...
            case 'C':
                if (UTag == "CONT") {
                    ...
                    SetCreator(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "CORPSE") {
                    SetCorpse(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)) == 1);
                    rValue = true;
                }
                else if (UTag == "COLD") {
            ...
            case 'D':
                ...
                if (UTag == "DIR") {
                    ...
                    SetDye(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)) == 1);
                    rValue = true;
                }
                break;
            case 'E':
                if (UTag == "ENTRYMADEFROM") {
                    EntryMadeFrom(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "EVENT") {
                    SetEvent(data.c_str());
            ...
            case 'G':
                if (UTag == "GRIDLOC") {
                    SetGridLocation(static_cast<std::int8_t>(std::stoi(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "GLOWTYPE") {
                    SetGlowEffect(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "GLOWBC") {
                    SetGlowColour(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "GLOW") {
                    SetGlow(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "GOOD") {
                    SetGood(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'H':
            ...
            case 'L':
                if (UTag == "LAYER") {
                    layer = static_cast<itemlayers_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0));
                    rValue = true;
                }
                else if (UTag == "LIGHT") {
            ...
            case 'M':
                if (UTag == "MAXITEMS") {
                    ...
                    if (csecs.size() >= 4) {
                        SetTempVar(CITV_MORE, 1, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetTempVar(CITV_MORE, 2, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetTempVar(CITV_MORE, 3, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                        SetTempVar(CITV_MORE, 4, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[3], "//")),  nullptr, 0)));
                    }
                    else {
                        SetTempVar(CITV_MORE, static_cast<std::uint32_t>(std::stoul( util::trim(util::strip(data, "//")), nullptr, 0)));
                    }
                    rValue = true;
                }
                else if (UTag == "MORE2") // Depreciated
                {
                    ...
                    SetTempVar(CITV_MOREX, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    SetTempVar(CITV_MOREY, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                    SetTempVar(CITV_MOREZ, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                    rValue = true;
                }
                    ...
                    if (csecs.size() >= 4) {
                        SetTempVar(CITV_MOREX, 1, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREX, 2, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREX, 3, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREX, 4, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[3], "//")), nullptr, 0)));
                    }
                    else {
                        SetTempVar(CITV_MOREX, static_cast<std::uint32_t>(std::stoul( util::trim(util::strip(data, "//")), nullptr, 0)));
                    }
                    rValue = true;
                }
                else if (UTag == "MOREY") {
                    if (csecs.size() >= 4) {
                        SetTempVar(CITV_MOREY, 1, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREY, 2, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREY, 3, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREY, 4, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[3], "//")), nullptr, 0)));
                    }
                    else {
                        SetTempVar(CITV_MOREY, static_cast<std::uint32_t>(std::stoul( util::trim(util::strip(data, "//")), nullptr, 0)));
                    }
                    rValue = true;
                }
                else if (UTag == "MOREZ") {
                    if (csecs.size() >= 4) {
                        SetTempVar(CITV_MOREZ, 1, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")),  nullptr, 0)));
                        SetTempVar(CITV_MOREZ, 2, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREZ, 3, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                        SetTempVar(CITV_MOREZ, 4,  static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[3], "//")),  nullptr, 0)));
                    }
                    else {
                        SetTempVar(CITV_MOREZ, static_cast<std::uint32_t>(std::stoul( util::trim(util::strip(data, "//")), nullptr, 0)));
                    }
                    rValue = true;
                }
                    ...
                    SetMovable(static_cast<std::int8_t>(std::stoi(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "MAXHP") {
                    SetMaxHP(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "MAXUSES") {
                    SetMaxUses(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'N':
            ...
            case 'O':
                if (UTag == "OFFSPELL") {
                    SetOffSpell(static_cast<std::int8_t>(std::stoi(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'P':
                if (UTag == "PRIV") {
                    SetPriv(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "PILEABLE") {
                    SetPileable(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)) == 1);
                    rValue = true;
                }
                break;
            case 'R':
                if (UTag == "RESTOCK") {
                    SetRestock(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                    ...
                    if (csecs.size() > 1) {
                        SetBaseRange(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetMaxRange(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                    }
                    else {
                        auto val = static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0));
                        SetBaseRange(val);
                        SetMaxRange(val / 2);
                    }
                    rValue = true;
            ...
            case 'S':
                if (UTag == "SPEED") {
                    SetSpeed(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                    ...
                    SetSpell(0, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    SetSpell(1, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                    SetSpell(2, static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "STEALABLE") {
                    SetStealable(static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'T':
                if (UTag == "TYPE") {
                    if (csecs.size() != 1) {
                        SetType(static_cast<itemtypes_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    }
                    else {
                        SetType(static_cast<itemtypes_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    }
                    rValue = true;
                }
                else if (UTag == "TYPE2") {
            ...
            case 'U':
                if (UTag == "USESLEFT") {
                    SetUsesLeft(static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
            case 'V':
                if (UTag == "VALUE") {
                    if (csecs.size() > 2) {
                        SetBuyValue(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                        SetSellValue(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[1], "//")), nullptr, 0)));
                        SetVendorPrice(static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(csecs[2], "//")), nullptr, 0)));
                    }
                        ...
                        auto val = static_cast<std::uint32_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0));
                        SetBuyValue(val);
                        SetSellValue(val / 2);
                    }
                    rValue = true;
            ...
            case 'W':
                if (UTag == "WEIGHTMAX") {
                    SetWeightMax(static_cast<std::int32_t>(std::stoi(util::trim(util::strip(data, "//")), nullptr, 0)));
                    rValue = true;
                }
                break;
        }
...
void CItem::CheckItemIntegrity() {
    ...
    if (type == IT_CONTAINER && GetLayer() == IL_PACKITEM &&
        ...
        if (GetMaxItems() == 0) {
            SetMaxItems(ServerConfig::shared().ushortValues[UShortValue::MAXPLAYERPACKITEM]);
        ...
        if (GetWeightMax() == 0) {
            SetWeightMax(ServerConfig::shared().uintValues[UIntValue::MAXPLAYERPACKWEIGHT]);
...
const std::uint32_t BIT_DECAYABLE = 0;
const std::uint32_t BIT_NEWBIE = 1;
const std::uint32_t BIT_DISPELLABLE = 2;
const std::uint32_t BIT_DIVINELOCK = 3;
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsDecayable() const -> bool { return priv.test(BIT_DECAYABLE); }
...
// o------------------------------------------------------------------------------------------------o
auto CItem::IsDivineLocked() const -> bool { return priv.test(BIT_DIVINELOCK); }
...
// be stolen
// o------------------------------------------------------------------------------------------------o
auto CItem::GetStealable() const -> std::uint8_t { return stealable; }
auto CItem::SetStealable(std::uint8_t newValue) -> void {
    stealable = newValue;
    UpdateRegion();
}
...
// o------------------------------------------------------------------------------------------------o
auto CItem::EntryMadeFrom() const -> std::uint16_t { return entryMadeFrom; }
auto CItem::EntryMadeFrom(std::uint16_t newValue) -> void {
...
// o------------------------------------------------------------------------------------------------o
void CItem::SetWeight(std::int32_t newVal, bool doWeightUpdate) {
...
inline bool operator==(const CItem &x, const CItem &y) { return (x.GetSerial() == y.GetSerial()); }
...
void CItem::Update([[maybe_unused]] CSocket *mSock, [[maybe_unused]] bool drawGamePlayer, [[maybe_unused]] bool sendToSelf) {
    ...
    if (iCont == nullptr) {
        ...
        std::for_each(nearbyChars.begin(), nearbyChars.end(), [this](CSocket *tSock) { SendToSocket(tSock); });
        return;
    }
...
void CItem::SendToSocket(CSocket *mSock, [[maybe_unused]] bool drawGamePlayer) {
    ...
    if (mChar != nullptr) {
        if (!mChar->IsGM()) {
            ...
            if ((iVisible != VT_VISIBLE && iVisible != VT_TEMPHIDDEN) || (iVisible == VT_TEMPHIDDEN && (!ValidateObject(ownerObj) || (ValidateObject(ownerObj) && ownerObj->GetSerial() != mChar->GetSerial())))) {
                return;
            }
        }
    ...
    if (iCont == nullptr || oType == OT_ITEM) {
        ...
        if (mSock != nullptr) {
            ...
            for (const auto &oSock : contOpenedByList->collection()) {
                ...
                //  A) opened the container at some point, and are still on the container's list of
                //  players who opened it B) are within range of the container
...
// o------------------------------------------------------------------------------------------------o
auto CItem::GetSpell(std::uint8_t part) const -> std::uint32_t {
    std::uint32_t rValue = 0;
    if (part < 3) {
        rValue = spells[part];
    }
    return rValue;
}
auto CItem::SetSpell(std::uint8_t part, std::uint32_t newValue) -> void {
    if (part < 3) {
...
auto CItem::GetContainsList() -> GenericList<CItem *> * { return &Contains; }

auto CItem::GetContOpenedByList() -> GenericList<CSocket *> * { return &contOpenedBy; }
...
// o------------------------------------------------------------------------------------------------o
auto CSpawnItem::GetInterval(std::uint8_t part) const -> std::uint8_t {
...
auto CSpawnItem::SetInterval(std::uint8_t part, std::uint8_t newVal) -> void {
    if (part < 2) {
...
// o------------------------------------------------------------------------------------------------o
auto CSpawnItem::GetSpawnSection() const -> std::string { return spawnSection; }
...
// o------------------------------------------------------------------------------------------------o
auto CSpawnItem::IsSectionAList() const -> bool { return isSectionAList; }
...
bool CSpawnItem::DumpBody(std::ostream &outStream) const {
    ...
    outStream << "Interval=" << static_cast<std::uint16_t>(GetInterval(0)) << "," << static_cast<std::uint16_t>(GetInterval(1)) << '\n';
...
bool CSpawnItem::HandleLine(std::string &UTag, std::string &data) {
    ...
    if (!rValue) {
        ...
        switch ((UTag.data()[0])) {
            case 'I':
                if (UTag == "INTERVAL") {
                    SetInterval(0, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[0], "//")), nullptr, 0)));
                    SetInterval(1, static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(csecs[1], "//")),  nullptr, 0)));
                    rValue = true;
                }
                else if (UTag == "ISSECTIONALIST") {
                    IsSectionAList((static_cast<std::uint8_t>(std::stoul(util::trim(util::strip(data, "//")), nullptr, 0)) == 1));
                    rValue = true;
                }
                break;
            case 'S':
            ...
            if (itemList != nullptr) {
                ...
                if (itemListSize > 0) {
                    ...
                    for (size_t i = 0; i < itemListSize; i++) {
                        ...
                        // tag
                        auto listObj = oldstrutil::sections(util::trim(util::strip(itemList->moveTo(i), "//")), ",");
                        if (!listObj.empty()) {
                            std::uint16_t amountToSpawn = 1;
                            std::vector<std::string> itemListData;
                            if (util::upper(listObj[0]) == "ITEMLIST" || util::upper(listObj[0]) == "LOOTLIST") {
                                bool useLootList = util::upper(listObj[0]) == "LOOTLIST";
                                
                                // Itemlist/Lootlist
                                itemListData = oldstrutil::sections(util::trim(util::strip(itemList->GrabData(), "//")), ",");
                                ...
                                if (itemListData.size() > 1) {
                                    ...
                                    if (tsects.size() > 1) { // check if the second part of the tag-data contains two
                                        ...
                                        auto first = static_cast<std::uint16_t>(std::stoul(util::trim(util::strip(tsects[0], "//")), nullptr, 0));
