		...
		if( i > 0 )
		{
			...
			if( !tag.empty() )
			{
				...
				if( util::upper( tag ) == "LOOTLIST" )
				{
					...
					{
						iAmount = static_cast<UI16>( std::stoul( util::trim( util::strip( csecs[1], "//" )), nullptr, 0 ));
						[[maybe_unused]] CItem *retItemNested = nullptr;
						for( UI16 iCount = 0; iCount < iAmount; ++iCount )
						{
							retItemNested = AddRandomLoot( s, util::trim( util::strip( csecs[0], "//" )), shouldSave );
						}
					}
					else
					{
				...
				else
				{
					...
					{
						iAmount = static_cast<UI16>( std::stoul( util::trim( util::strip( tcsecs[1], "//" )), nullptr, 0 ));
						retItem = Items->CreateBaseScriptItem( s, util::trim( util::strip( tcsecs[0], "//" )), s->WorldNumber(), iAmount, s->GetInstanceId(), OT_ITEM, 0xFFFF, shouldSave );
						if( retItem )
						{
							retItem->SetCont( s );
							retItem->PlaceInPack();
...
auto CCharStuff::NpcListLookup( const std::string &npclist ) -> std::string
{
	...
	for( size_t i = 0; i < npcListSize; i++ )
	{
		...
		if( csecs.size() > 1 )
		{
			sectionWeight = static_cast<UI16>( std::stoul( util::trim( util::strip( csecs[0], "//" )), nullptr, 0 ));
		}
			...
			case DFNTAG_VALUE:
				if( retItem )
				{
					if( !cdata.empty() )
					{
						...
						if( ssecs.size() > 1 )
						{
							retItem->SetBuyValue(  static_cast<UI32>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 0 )));
							retItem->SetSellValue( static_cast<UI32>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 )));
							break;
						}
					}
	...
	for( const auto &sec : NpcCreation->collection2() )
	{
		...
		switch( tag )
		{
			...
			case DFNTAG_ELEMENTRESIST:
				if( ssects.size() >= 4 )
				{
					applyTo->SetResist(( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[0], "//" )), nullptr, 0 ))), HEAT );
					applyTo->SetResist(( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[1], "//" )), nullptr, 0 ))), COLD );
					applyTo->SetResist(( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[2], "//" )), nullptr, 0 ))), LIGHTNING );
					applyTo->SetResist(( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[3], "//" )), nullptr, 0 ))), POISON );
				}
				break;
			case DFNTAG_ERBONUS:
				if( ssects.size() >= 4 )
				{
					applyTo->SetResist( applyTo->GetResist( HEAT ) + static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[0], "//" )), nullptr, 0 )), HEAT );
					applyTo->SetResist( applyTo->GetResist( COLD ) + static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[1], "//" )), nullptr, 0 )), COLD );
					applyTo->SetResist( applyTo->GetResist( LIGHTNING ) + static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[2], "//" )), nullptr, 0 )), LIGHTNING );
					applyTo->SetResist( applyTo->GetResist( POISON ) + static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[3], "//" )), nullptr, 0 )), POISON );
				}
				break;
			...
			case DFNTAG_ID:
				...
				if( ssects.size() == 1 )
				{
					applyTo->SetId( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[0], "//" )), nullptr, 0 )));
					applyTo->SetOrgId( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[0], "//" )), nullptr, 0 )));
				}
				else
				{
					SI32 rndEntry = RandomNum( 0, static_cast<SI32>( ssects.size() - 1 ));
					applyTo->SetId( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[rndEntry], "//" )), nullptr, 0 )));
					applyTo->SetOrgId( static_cast<UI16>( std::stoul( util::trim( util::strip( ssects[rndEntry], "//" )), nullptr, 0 )));
				}
				break;
