...
// o------------------------------------------------------------------------------------------------o
auto DoHouseTarget(CSocket *mSock, std::uint16_t houseEntry) -> void {
    ...
    if (House) {
        ...
        if (!houseId) {
            ...
            if (houseId >= 0x4000) {
                mSock->mtarget(static_cast<std::uint16_t>(houseId - 0x4000),
                               576); // Select a location for the building.
            }
            else {
                mSock->SendTargetCursor(0, TARGET_BUILDHOUSE, 0,
                                        9184); // Select a location for the house addon.
            }
        }
    }
}
...
// o------------------------------------------------------------------------------------------------o
void CreateHouseKey(CSocket *mSock, CChar *mChar, CMultiObj *house, std::uint16_t houseId) {
    if (houseId >= 0x4000) {
...
// o------------------------------------------------------------------------------------------------o
auto CreateHouseItems(CChar *mChar, std::vector<std::string> houseItems, CItem *house, std::uint16_t houseId,
                      std::int16_t x, std::int16_t y, std::int8_t z, std::uint8_t worldNum) -> void {
    ...
    for (const auto &entry : houseItems) {
        ...
        if (HouseItem) {
            ...
            std::int8_t hiZ = z;
            std::uint8_t hWorld = house->WorldNumber();
            std::uint16_t hInstanceId = house->GetInstanceId();
            ...
            for (const auto &sec : HouseItem->collection()) {
                            ...
                            if (hItem->GetTempVar(CITV_MOREZ) != 0) {
                                ...
                                if (wallFound) {
                                    ...
                                    if (northWallFound) {
                                        ...
                                        // Found a wall west of addon, but none north! Let's rotate
                                        // addon to face east:
                                        hItem->SetId(hItem->GetTempVar(CITV_MOREZ));
                                    }
                                }
                ...
                {
                    hiX += static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
                }
                else if (UTag == "Y") {
                    hiY += static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
                }
                else if (UTag == "Z") {
                    hiZ += static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
                }
            }
            if (ValidateObject(hItem)) {
...
auto CheckForValidHouseLocation(CSocket *mSock, CChar *mChar, std::int16_t x, std::int16_t y, std::int8_t z, std::int16_t spaceX,
    ...
    if (!isMulti) {
        // House addon
        for (std::int16_t k = 0; k <= spaceX - 1; ++k) {
            curX = x + k;
            for (std::int16_t l = 0; l <= spaceY - 1; ++l) {
                curY = y + l;
        ...
        // Multi
        // Loop through each tile in the multi's affected area, check if it contains any blocking
        ...
        for (std::int16_t k = -spaceX; k <= spaceX; ++k) {
            ...
            for (std::int16_t l = (-spaceY + 1); l <= spaceY; ++l) {
                ...
                if ((k < (-spaceX + 1)) && (l > -spaceY + 3 && l < spaceY - 3)) {
                    ...
                    // and in the 4 tiles furthest in front of the house; we don't care about
                    // blocking statics like trees in this area
...
// o------------------------------------------------------------------------------------------------o
CMultiObj *BuildHouse(CSocket *mSock, std::uint16_t houseEntry, bool checkLocation = true, std::int16_t xLoc = -1,
    ...
    std::uint16_t maxLockdowns = 425;
    std::uint16_t maxSecureContainers = 3;
    std::uint16_t maxFriends = 50;
    std::uint16_t maxGuests = 50;
    std::uint16_t maxBans = 50;
    std::uint16_t maxOwners = 8;
    std::uint16_t maxVendors = 10;
    std::uint16_t maxTrashContainers = 1;
    std::uint16_t scriptTrigger = 0;
    std::uint16_t multiColour = 0;
    std::int32_t weightMax = 400; // Default
    ...
    for (const auto &sec : House->collection()) {
        ...
        if (UTag == "ID") {
            ...
            sx = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "SPACEY") {
            sy = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "CHARX") {
            cx = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "CHARY") {
            cy = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "CHARZ") {
            cz = static_cast<std::int8_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "BANX") {
            bx = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "BANY") {
            by = static_cast<std::int16_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "Z") // to allow offsetting houses in Z to fix multis like Farmer's Cabin
        {
            z = z + static_cast<std::int8_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "ITEMSDECAY") {
            itemsWillDecay = (static_cast<std::int16_t>(std::stoi(data, nullptr, 0)) == 1);
        }
            ...
            isDamageable = (static_cast<std::int16_t>(std::stoi(data, nullptr, 0)) == 1);
        }
        else if (UTag == "MAXITEMS") {
            maxItems = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "WEIGHTMAX") {
            weightMax = static_cast<std::int32_t>(std::stoi(data, nullptr, 0));
        }
        else if (UTag == "MAXBANS") {
            maxBans = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXFRIENDS") {
            maxFriends = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXGUESTS") {
            maxGuests = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXLOCKDOWNS") {
            maxLockdowns = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXTRASHCONTAINERS") {
            maxTrashContainers = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXOWNERS") {
            maxOwners = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXSECURECONTAINERS") {
            maxSecureContainers = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "MAXVENDORS") {
            maxVendors = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
        else if (UTag == "SCRIPT") {
            scriptTrigger = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
            if (scriptTrigger > 0) {
                scriptIds.push_back(scriptTrigger);
            }
        }
        else if (UTag == "COLOUR" || UTag == "COLOR") {
            multiColour = static_cast<std::uint16_t>(std::stoul(data, nullptr, 0));
        }
    ...
    if (isMulti) {
        ...
        for (auto &ourChar : FindNearbyChars(x, y, worldNumber, instanceId, std::max(sx, sy))) {
            if ((ourChar->GetX() >= multiX1 && ourChar->GetX() <= multiX2) &&
                ...
                // Move character (NPC or PC, online or offline) to corner of multi so they don't
                // get stuck in house!
    ...
    if (ValidateObject(mChar) && (isMulti || isBoat)) {
        ...
        for (const auto &follower : myFollowers->collection()) {
            if (ValidateObject(follower) && follower->GetNpcAiType() != AI_PLAYERVENDOR) {
                // No need to check if they're following player, just teleport them. Don't let them
                // get stuck under house!
...
CMultiObj *BuildBaseMulti(std::uint16_t multiId, std::int16_t xLoc = -1, std::int16_t yLoc = -1, std::int8_t zLoc = 127,
    ...
    for (auto &ourChar : FindNearbyChars(xLoc, yLoc, worldNumber, instanceId, std::max(sx, sy))) {
        if ((ourChar->GetX() >= multiX1 && ourChar->GetX() <= multiX2) &&
            ...
            // Move character (NPC or PC, online or offline) to corner of multi so they don't get
            // stuck in multi!
    ...
    if (ValidateObject(a) && a->CanBeObjType(OT_ITEM)) {
        ...
        if (!charSerial) {
            ...
            else if (i->GetId() >= 0x1769 && i->GetId() <= 0x176b) // Keyrings
            {
                ...
                std::int32_t keyCount = i->GetTag("keys").m_IntValue;
                if (keyCount > 0) {
            ...
            if (i->GetType() == IT_KEY && i->GetTempVar(CITV_MORE) == targSerial) {
                ...
                else if (ValidateObject(i->GetCont()) &&
                         FindRootContainer(i)->GetOwner() == charSerial) {
            ...
            {
                CChar *itemPackOwner = FindItemOwner(i);
                // If In backpack or bank, or locked down in character's house, or inside a
                // container in character's house, or on the ground of character's house but not
                // locked down
                if ((ValidateObject(itemPackOwner) && itemPackOwner->GetSerial() == charSerial) ||
                    (i->GetMovable() == 3 && ValidateObject(i->GetMultiObj()) &&
                     i->GetMultiObj()->GetOwnerObj()->GetSerial() == charSerial) ||
                    (ValidateObject(i->GetCont()) &&
                     FindRootContainer(i)->GetOwner() == charSerial) ||
...
void KillKeys(serial_t targSerial, serial_t charSerial = INVALIDSERIAL) {
    if (charSerial == INVALIDSERIAL) {
        ...
        // matches target serial
        std::uint32_t toPass[2];
        toPass[0] = targSerial;
        toPass[1] = charSerial;
        std::uint32_t b = 0;
        ObjectFactory::shared().IterateOver(OT_ITEM, b, toPass, &KillKeysFunctor);
