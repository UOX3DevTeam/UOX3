...
auto DoHouseTarget(CSocket *mSock, UI16 houseEntry) -> void {
    ...
    if (House) {
        ...
        if (!houseId) {
            ...
            if (houseId >= 0x4000) {
                mSock->mtarget(static_cast<UI16>(houseId - 0x4000),
...
// o------------------------------------------------------------------------------------------------o
auto CreateHouseItems(CChar *mChar, std::vector<std::string> houseItems, CItem *house, UI16 houseId,
                      SI16 x, SI16 y, SI08 z, UI08 worldNum) -> void {
                ...
                else if (UTag == "LOCK" && houseId >= 0x4000) // lock it with the house key
                {
                    ...
                    if (ValidateObject(hItem)) {
                        ...
                        frontDoorTag.m_IntValue =
                            static_cast<SI32>(frontDoorTag.m_StringValue.size());
...
// o------------------------------------------------------------------------------------------------o
auto CheckForValidHouseLocation(CSocket *mSock, CChar *mChar, SI16 x, SI16 y, SI08 z, SI16 spaceX,
                                SI16 spaceY, UI08 worldNum, UI16 instanceId, bool isBoat,
                                bool isMulti) -> bool {
...
// o------------------------------------------------------------------------------------------------o
CMultiObj *BuildHouse(CSocket *mSock, UI16 houseEntry, bool checkLocation = true, SI16 xLoc = -1,
