...
void CNetworkStuff::Disconnect(UOXSOCKET s) {
    ...
    Console::shared() << "Client " << static_cast<UI32>(s) << " disconnected. [Total:"
                      << static_cast<SI32>(cwmWorldState->GetPlayersOnline() - 1) << "]" << myendl;
            ...
            if ((FD_ISSET(connClients[i]->CliSocket(), &all)) &&
                ...
                try {
                    ...
                    if (blah.ErrorNumber() == WSAECONNRESET) {
                        ...
                        Console::shared()
                            << "Socket error: " << static_cast<SI32>(blah.ErrorNumber()) << myendl;
                    }
#endif
        ...
        if (mSock->Receive(1, false) > 0) {
            ...
            if (mSock->FirstPacket() && packetId != 0x80 && packetId != 0x91) {
                ...
                if (cwmWorldState->ServerData()->ServerUOGEnabled()) {
                    Console::shared() << "UOG Stats request completed. Disconnecting client. ["
                                      << static_cast<SI32>(mSock->ClientIP4()) << "."
                                      << static_cast<SI32>(mSock->ClientIP3()) << "."
                                      << static_cast<SI32>(mSock->ClientIP2()) << "."
                                      << static_cast<SI32>(mSock->ClientIP1()) << "]" << myendl;
                }
                else {
                    Console::shared()
                        << "Encrypted client attempting to cut in, disconnecting them: IP "
                        << static_cast<SI32>(mSock->ClientIP1()) << "."
                        << static_cast<SI32>(mSock->ClientIP2()) << "."
                        << static_cast<SI32>(mSock->ClientIP3()) << "."
                        << static_cast<SI32>(mSock->ClientIP4()) << myendl;
                }
                Disconnect(s);
                return;
            }
                ...
                case 0x12: // Ext. Command
                    ...
                    if (buffer[3] == 0xC7) // Action
                    {
                        ...
                        if (std::string(reinterpret_cast<char *>(&buffer[4])) == "bow") {
                            ...
                            else if ((ourChar->GetBodyType() == BT_HUMAN ||
                                      ourChar->GetBodyType() == BT_ELF) &&
                                     !cwmWorldState->ServerData()->ForceNewAnimationPacket()) {
                                // If human or elf, and new animation packet is disabled
                                Effects->PlayCharacterAnimation(ourChar, ACT_EMOTE_BOW, 0,
                                                                5); // 0x20
                            }
                            else {
                                // If polymorphed to other creatures
                                Effects->PlayCharacterAnimation(ourChar, 0x12, 0,
                                                                5); // Monster fidget 1
                            }
                        }
                        if (std::string(reinterpret_cast<char *>(&buffer[4])) == "salute") {
                            if (ourChar->GetBodyType() == BT_GARGOYLE ||
                                ...
                                Effects->PlayNewCharacterAnimation(ourChar, N_ACT_EMOTE,
                                     ...
                                     !cwmWorldState->ServerData()->ForceNewAnimationPacket()) {
                                // If human or elf, and new animation packet is disabled
                                Effects->PlayCharacterAnimation(ourChar, ACT_EMOTE_SALUTE, 0,
                                                                5); // 0x21
                            }
                            else {
                                // If polymorphed to other creatures
                                Effects->PlayCharacterAnimation(ourChar, 0x11, 0,
                                                                5); // Monster fidget 2
                            }
                        }
                        break;
                    ...
                    else if (buffer[3] == 0x24) // Skill
                    {
                        ...
                        buffer[j] = 0;
                        Skills->SkillUse(
                            mSock, static_cast<UI08>(std::stoul(std::string((char *)&buffer[4]))));
                        break;
                    }
    ...
    if (mSock->NewClient()) {
        ...
        if (mSock->Receive(1, false) > 0) {
            ...
            if (mSock->FirstPacket() && packetId != 0x80 && packetId != 0x91 && packetId != 0xE4 &&
                ...
                if (cwmWorldState->ServerData()->ServerUOGEnabled()) {
                    messageLoop << util::format(
                        "UOG Stats Sent or Encrypted client detected. [%i.%i.%i.%i]",
                        mSock->ClientIP4(), mSock->ClientIP3(), mSock->ClientIP2(),
                        mSock->ClientIP1());
                }
                else {
                    messageLoop << util::format("Encrypted client detected. [%i.%i.%i.%i]",
                                                mSock->ClientIP4(), mSock->ClientIP3(),
                                                mSock->ClientIP2(), mSock->ClientIP1());
                }
                LoginDisconnect(s);
                return;
            }
                ...
                messageLoop << util::format(
                    "Buffer is empty, no packets to read. Disconnecting client. [%i.%i.%i.%i]",
                    mSock->ClientIP4(), mSock->ClientIP3(), mSock->ClientIP2(), mSock->ClientIP1());
                LoginDisconnect(s);
                return;
            }
            bool doSwitch = true;
            ...
            if (doSwitch) {
                ...
                default:
                    ...
                    messageLoop << util::format("Unknown message from client: 0x%02X - Ignored",
                                                packetId);
                    break;
                }
            }
...
void CNetworkStuff::RegisterPacket(UI08 packet, [[maybe_unused]] UI08 subCmd, UI16 scriptId) {
    // UI16 packetId = static_cast<UI16>(( subCmd << 8 ) + packet ); // Registration of subCmd
