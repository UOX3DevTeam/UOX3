...
bool CHandleCombat::StartAttack(CChar *cAttack, CChar *cTarget) {
    ...
    if (cTarget->WorldNumber() != cAttack->WorldNumber() ||
        cTarget->GetInstanceId() != cAttack->GetInstanceId())
        return false;
    ...
    if (cAttack->IsNpc() && cTarget->IsNpc() && (cAttack->GetOwner() == INVALIDSERIAL && cTarget->GetOwner() == INVALIDSERIAL)) {
        ...
        if (!cwmWorldState->creatures[cAttack->GetId()].IsHuman() && !cwmWorldState->creatures[cTarget->GetId()].IsHuman()) {
            ...
            // Don't allow human NPC to human NPC combat if they both have same NPC Flag (evil,
            // innocent, neutral)
    ...
    // Only unhide the defender, if they're going to return the attack (otherwise they're doing
    // nothing!)
    if (returningAttack) {
        ...
        // if the target is an npc, and not a guard, make sure they're in war mode and update their
        // movement time ONLY IF THEY'VE CHANGED ATTACKER
...
void CHandleCombat::PlayerAttack(CSocket *s) {
    ...
    if (ourChar->IsDead()) {
        ...
        if (ourChar->GetRegion()->IsSafeZone() || i->GetRegion()->IsSafeZone()) {
            // Either attacker or target is in a safe zone where all aggressive actions are
            // forbidden, disallow
        ...
        // Send attacker message to all nearby players, IF player is attacking someone who wasn't
        ...
        if (i->GetNpcAiType() != AI_GUARD && !ValidateObject(i->GetTarg())) {
            // Set ourChar as attacker of target, if target has no target? Should this check if it
            // has an Attacker instead?
            i->SetAttacker(ourChar);
        }
        
        // Add ourChar as an aggressor to the target, if target is not already an aggressor to
        // ourChar!
        ...
        for (auto &tSock : FindNearbyPlayers(cAttack)) {
            if (tSock) {
                ...
                if (ValidateObject(witness)) {
                    ...
                    // Send an emote about attacking target to nearby witness
                    cAttack->TextMessage(tSock, 334, EMOTE, 0, attackerName.c_str(),
                                         targetName.c_str()); // You see %s attacking target
                }
            }
        }
...
/*
 ...
 Purpose:
 To emulate OSI's armor calculation methods for specific hit locations and total AR
 level
 
 How we do this:
 We need to first determine what we are calculating, total AR (hitLoc 0) or the armor
 level of a specific locations (hitLoc 1-6). Next we need to loop through the items a player has
 equipped searching for items that match the following criteria: Defence > 0 On a specific layer
 (depending on the hit location) The item with the most AR covering that location Lastly we have
 to return this information. If we are returning the AR of a specific hit location, then we return
 the actual Def value of the item if we are returning the total AR of a character, we need to find
 the AR / the divisor used for that specific location.
...
void CHandleCombat::CombatAnimsNew(CChar *i) {
    ...
    if (i->IsFlying() || i->IsOnHorse()) {
        ...
        switch (GetWeaponType(j)) {
            case BOWS:
                subAnimToPlay = S_ACT_BOW;
                break; // 0x01
            case XBOWS:
            case BLOWGUNS:
                subAnimToPlay = S_ACT_XBOW;
                break; // 0x02
            case THROWN:
                subAnimToPlay = S_ACT_THROW;
                break; // 0x09
            case DEF_SWORDS:
            case SLASH_SWORDS:
            case ONEHND_LG_SWORDS:
...
void CHandleCombat::CombatOnFoot(CChar *i) {
    ...
    switch (GetWeaponType(j)) {
        case BOWS:
            animToPlay = ACT_ATT_BOW;
            break; // 0x12
        case XBOWS:
        case BLOWGUNS:
            animToPlay = ACT_ATT_XBOW;
            break; // 0x13
        case DEF_SWORDS:
        case SLASH_SWORDS:
        case ONEHND_LG_SWORDS:
        ...
        case DUAL_FENCING_STAB:
        case DEF_FENCING:
            animToPlay = ACT_ATT_1H_PIERCE;
            break; // 0x0A
        case TWOHND_FENCING:
            animToPlay = ACT_ATT_2H_PIERCE;
            break; // 0x0E
        case WRESTLING:
        default:
            switch (RandomNum(0, 2)) {
                case 0:
                    ...
                    break; // 0x0B
                case 1:
                    animToPlay = ACT_ATT_1H_SLASH;
                    break; // 0x09
                default:
                    animToPlay = ACT_ATT_WRESTLE;
                    break; // 0x1F
            }
            break;
    }
    ...
    if (ServerConfig::shared().enabled(ServerSwitch::DISPLAYHITMSG) && targSock != nullptr) {
        ...
        switch (hitLoc) {
            ...
            case 2: // Arms
                switch (randHit) {
                    case 1:
                        targSock->SysMessage(290, attackerName.c_str());
                        break; // %s hits you in your Left Arm!
                    case 2:
                        targSock->SysMessage(291, attackerName.c_str());
                        break; // %s hits you in your Right Arm!
                    default:
                        targSock->SysMessage(292, attackerName.c_str());
                        break; // %s hits you in your Right Arm!
                }
                break;
            case 3: // Head
            ...
            case 4: // Legs
                switch (randHit) {
                    case 1:
                        targSock->SysMessage(299, attackerName.c_str());
                        break; // %s hits you in the Left Thigh!
                    case 2:
                        targSock->SysMessage(300, attackerName.c_str());
                        break; // %s hits you in the Right Thigh!
                    default:
                        targSock->SysMessage(301, attackerName.c_str());
                        break; // %s hits you in the Groin!
                }
                break;
            case 5:                                              // Neck
            ...
            case 6: // Hands
                switch (randHit) {
                    case 1:
                        targSock->SysMessage(303, attackerName.c_str());
                        break; // %s hits you in the Left Hand!
                    case 2:
                        targSock->SysMessage(304, attackerName.c_str());
                        break; // s hits you in the Right Hand!
                    default:
                        targSock->SysMessage(305, attackerName.c_str());
                        break; // %s hits you in the Right Hand!
                }
                break;
        }
...
std::int16_t CHandleCombat::ApplyDamageBonuses(Weather::type_t damageType, CChar *mChar, CChar *ourTarg, std::uint8_t getFightSkill, std::uint8_t hitLoc, std::int16_t baseDamage) {
    ...
    switch (damageType) {
        ...
        case Weather::PHYSICAL:
            ...
            // The lumberjacking skill will provide a bonus to damage when the player is using one of
            // the following axes. The higher the characters lumberjacking skill the more damage they
            // will do up to a 25% bonus for 99.9 lumberjacking. At Grandmaster lumberjacking, the
            // damage bonus is 35%.
            //	Axe
            //	Battle Axe
            //	Double Axe
            ...
            // Publish 13 (Aug 19, 2001, during Third Dawn era) had some changes to combat damage:
            // https://www.uoguide.com/Publish_13 Hit points for players: (str/2) + 50 Anatomy had a bug
            // prior to this where damage bonus against players was doubled. After fixing this, the
            // bonus from Anatomy was changed to 20% of Anatomy skill (against both monsters and
            // players) for players with less than 100.0 Anatomy, and 30% of skill for players who reach
            // 100.0 Anatomy. Lumberjack damage bonus for axe weapons using same formula as anatomy; 20%
            // bonus up until 99.9 skillpoints, 30% for 100.0 skillpoints Special hits work on monsters
            // as well as players NPCs can do special hits if they have anatomy and two-handed melee
            // weapon equipped All special hits with 2-h weapons now base chance to go off on anatomy
            // skill: Anatomy / 4 Pre-casting re-enabled: Cannot equip anything while casting, after you
            // cast a spell it will "time out" after 30 seconds, and can no longer target anything.
            // During those 30 seconds, you can equip a weapon, attack with it, and still keep the
            // stored spell. Upon releasing the spell you automatically unequip the weapon
            ...
            // Energy Bolt, Explosion, Chain Lightning, Meteor Swarm and Flamestrike spells given small
            // damage increase Lower reagent cost: Magic Arrow (1 Sulfurous Ash), Fireball (1 Black
            // Pearl), Lightning (1 Mandrake Root, 1 Sulfurous Ash), Explosion (1 Bloodmoos, 1 Mandrake
            // Root) Harm spell damage now scales inversely with distance to target; the closer to the
            // target, the higher the damage
            
            // From at least ML forward
            // (https://web.archive.org/web/20080616071554/https://uo.stratics.com/content/arms-armor/combat.php)
            ...
            // Final Damage Bonus% = Tactics Bonus + Anatomy Bonus + Lumber Bonus + Strangth Bonus +
            // Damage Increase Items* Final Damage = Base Damage + (Base Damage * Final Damage Bonus%)
            ...
            if (offensiveWeight > rndNum) {
                ...
                switch ((RandomNum(static_cast<std::int16_t>(0), spattacks) + 1)) {
                    case 1: // Spell Circle 1 - Offensive Spells
                        switch (RandomNum(1, 4)) {
                            case 1:
                                ...
                                break; // Clumsy
                            case 2:
                                CastSpell(npcAttack, cDefend, 3);
                                break; // Feeble Mind
                            case 3:
                                CastSpell(npcAttack, cDefend, 5);
                                break; // Magic Arrow
                            case 4:
                                CastSpell(npcAttack, cDefend, 8);
                                break; // weaken
                        }
                        break;
                    case 2:
                        ...
                        switch (RandomNum(1, 2)) {
                            case 1:
                                if (npcAttack->GetIntelligence() > cDefend->GetIntelligence()) {
                                    ...
                                    // Avoid dumb caster NPCs from killing themselves, and fallthrough to
                                    // next spell in list instead
                                    [[fallthrough]];
                                }
                            case 2:
                    ...
                    case 6:
                        ...
                        switch (RandomNum(1, 2)) {
                            case 1:
                                ...
                                break; // Energy Bolt
                            case 2:
                                CastSpell(npcAttack, cDefend, 43);
                                break; // Explosion
                        }
                        break;
                    case 7:
                        switch (RandomNum(1, 3)) {
                            case 1:
                                ...
                                break; // Chain Lightning
                            case 2:
                                CastSpell(npcAttack, cDefend, 51);
                                monsterAreaCastAnim = true;
                                break; // Flamestrike
                            case 3:
                                CastSpell(npcAttack, cDefend, 55);
                                monsterAreaCastAnim = true;
                                break; // Meteor Swarm
                        }
                        break;
                    case 8:
                        ...
                        monsterAreaCastAnim = true;
                        // case 2:		break; //CastSpell( npcAttack, cDefend, 58 ); 	// Energy
                        // Vortex
                        break;
                        ...
                        case 2:
                            switch (RandomNum(1, 4)) {
                                ...
                                case 2:
                                    ...
                                    break; // Feeble Mind
                                case 3:
                                    CastSpell(npcAttack, cDefend, 7);
                                    break; // Reactive Armor (personal...switch target temporarily to self
                                    // and then back after casting)
                                case 4:
                                    CastSpell(npcAttack, cDefend, 8);
                                    break; // weaken
                            }
                        ...
                        case 4:
                            switch (RandomNum(1, 2)) {
                                case 1:
                                    ...
                                    break; // Curse
                                case 2:
                                    CastSpell(npcAttack, cDefend, 31);
                                    break; // Mana Drain
                            }
                            break;
                        case 5:
...
void CHandleCombat::InvalidateAttacker(CChar *mChar) {
    ...
    if (ValidateObject(ourTarg) && ourTarg->GetTarg() == mChar) {
        ...
        if (ourTarg->IsAtWar() && ourTarg->IsNpc()) {
            ourTarg->ToggleCombat(); // What if character is fighting OTHER characters than mChar as
            // well?
        }
    }
