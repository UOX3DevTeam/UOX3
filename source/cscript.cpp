...
//|					The basic event model is in, along with some fairly simple
//error handling |					Note that parameters for each event is NOT
//correct at all, it was just to get the basic layout right
//|					Support for CS type stuff with IsFiring(), Firing(), and
// Stop()
//|
//|					1.1		6th September, 2000
//|					Each function has params and passes it along into the the script
//as required
//|
...
//|					Addition of a number of events.  Caching of function
//presence
// added |					All functions assumed present, only failure on a
// call will flag not present
//|
//|					1.4		1st July, 2004
//|					Added private property of SCRIPTTYPE to store which type of
//script we've got |					Preparation for JS scriptable spells
// o------------------------------------------------------------------------------------------------o
...
// Global error message variable used to pass error message from MethodError() to the custom JSError
// callback function
...
//| Notes		-	Relies on global variable g_errorMessage to pass in error message
// from |					MethodError function.
// o------------------------------------------------------------------------------------------------o
const JSErrorFormatString *ScriptErrorCallback([[maybe_unused]] void *userRef,
                                               [[maybe_unused]] const char *locale,
                                               [[maybe_unused]] const uintN errorNumber) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Spit out a custom error message related to JS Methods or their
// parameters
//| Notes		-	Piggybacks on the internal error reporting mechanism in SpiderMonkey
//to
// get |					the filename and relevant line number from associated
// script
// o------------------------------------------------------------------------------------------------o
void ScriptError(JSContext *cx, const char *txt, ...) {
    // Combine error message with any potential additional arguments provided, store in
    // g_errorMessage
    va_list argptr;
    va_start(argptr, txt);
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Allows scripters to check if a particular JS event (or function)
// exists in a |					script before attempting to call it via
// TriggerEvent
// o------------------------------------------------------------------------------------------------o
...
//|	Notes		-	Checks for the presence of onCreateDFN by default, but onCreateTile
// can also |					be used to intercept creation of items directly from
// tiles/harditems.dfn
// o------------------------------------------------------------------------------------------------o
bool cScript::OnCreate(CBaseObject *thingCreated, bool dfnCreated, bool isPlayer) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, functionName.c_str(), 2, params, &rval);
    if (retVal == JS_FALSE) {
        if (isPlayer) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers when a character talks in range of the character with event
// attached
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers when an object comes in range of a character with the event
// attached
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	A third parameter is provided to let script know whether object that
// comes into |					range is a character or an item (Multi not supported
// yet)
// o------------------------------------------------------------------------------------------------o
...
std::int8_t cScript::OnMoveDetect(CBaseObject *sourceObj, CChar *charInRange, std::uint8_t rangeToChar,
    ...
    if (JS_CallFunctionName(targContext, targObject, "onMoveDetect", 5, params, &rval) ==
        JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	If used WITH RegisterSkill() in a script listed under
// SKILLUSE_SCRIPTS in
//|					JSE_FILEASSOCIATIONS.SCP, event will trigger for only the
//specified skill. |					If used WITHOUT RegisterSkill() in a general
// purpose script listed under
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for objects which server is about to send tooltip to client
// for
// o------------------------------------------------------------------------------------------------o
...
std::string cScript::OnNameRequest(CBaseObject *myObj, CChar *nameRequester, std::uint8_t requestSource) {
    ...
    try {
        ...
        JSBool retVal =
        JS_CallFunctionName(targContext, targObject, "onNameRequest", 3, params, &rval);
        if (retVal == JS_FALSE) {
            SetEventExists(seOnNameRequest, false);
        }
        
        // If rval is negative, it's possible some other function/method called from within
        ...
        // If no string was returned from the event, make sure we return an empty string instead of
...
//|					Will also trigger the onDefense event for the character
//being attacked
// o------------------------------------------------------------------------------------------------o
...
std::int8_t cScript::OnVirtueGumpPress(CChar *mChar, CChar *tChar, std::uint16_t buttonId) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onVirtueGumpPress", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnWarModeToggle(CChar *mChar) {
    ...
    params[0] = OBJECT_TO_JSVAL(charObj);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onWarModeToggle", 1, params, &rval);
...
std::int8_t cScript::OnDropItemOnItem(CItem *item, CChar *dropper, CItem *dest) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onDropItemOnItem", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
bool cScript::OnContRemoveItem(CItem *contItem, CItem *item, CChar *itemRemover) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onContRemoveItem", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnEquipAttempt(CChar *equipper, CItem *equipping) {
    ...
    params[1] = OBJECT_TO_JSVAL(itemObj);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onEquipAttempt", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnUnequipAttempt(CChar *equipper, CItem *equipping) {
    ...
    params[1] = OBJECT_TO_JSVAL(itemObj);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onUnequipAttempt", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers (after hardcoded checks) for item with event attached, when
// used
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers (before hardcoded checks) for item with event attached,
// when used
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	The function returns 3 possible values
//|					-1	=> No such function or bad call
...
std::int8_t cScript::OnUseUnChecked(CChar *user, CItem *iUsing) {
    ...
    params[1] = OBJECT_TO_JSVAL(itemObj);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onUseUnChecked", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for item with event attached when dropped on an NPC, and
// for NPC with
//|					event attached if item is dropped on it and doesn't bounce
//in item's script
// o------------------------------------------------------------------------------------------------o
//|	Changes		-	V2 -
//|						Returns
...
std::int8_t cScript::OnDropItemOnNpc(CChar *srcChar, CChar *dstChar, CItem *item) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onDropItemOnNpc", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for character with event attached when objects go out of
// range |	Notes		-	A third parameter in the event can be used to determine
// whether the object |					going out of range is an ITEM or a CHARACTER
// o------------------------------------------------------------------------------------------------o
...
//|	Date		-	10/06/2002
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for character with event attached when logging out of
// server
// o------------------------------------------------------------------------------------------------o
...
bool cScript::OnSystemSlice() {
    ...
    jsval rval;
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSystemSlice", 0, nullptr, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
bool cScript::OnWeatherChange(CBaseObject *tObject, Weather::type_t element) {
    if (!ValidateObject(tObject))
        return false;
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onWeatherChange", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
bool cScript::DoCallback(CSocket *tSock, serial_t targeted, std::uint8_t callNum) {
    ...
    try {
        ...
        JSBool retVal =
        JS_CallFunctionName(targContext, targObject,
...
std::int8_t cScript::OnLoyaltyChange(CChar *pChanging, std::int8_t newStatus) {
    ...
    params[1] = INT_TO_JSVAL(newStatus);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onLoyaltyChange", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnHungerChange(CChar *pChanging, std::int8_t newStatus) {
    ...
    params[1] = INT_TO_JSVAL(newStatus);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onHungerChange", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
bool cScript::OnThirstChange(CChar *pChanging, std::int8_t newStatus) {
    ...
    params[1] = INT_TO_JSVAL(newStatus);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onThirstChange", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnSnoopAttempt(CChar *snooped, CItem *pack, CChar *snooper) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSnoopAttempt", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
void cScript::HandleGumpInput(CPIGumpInput *pressing) {
    ...
    [[maybe_unused]] JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onGumpInput", 3, params, &rval);
}
...
std::int8_t cScript::OnScrollingGumpPress(CSocket *tSock, std::uint16_t gumpId, std::uint16_t buttonId) {
    ...
    params[2] = INT_TO_JSVAL(buttonId);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onScrollingGumpPress", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
std::int8_t cScript::OnSpellTargetSelect(CChar *caster, CBaseObject *target, std::uint8_t spellNum) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSpellTargetSelect", 3, params, &rval);
    if (retVal == JS_FALSE) {
...
bool cScript::CallParticularEvent(const char *eventToCall, jsval *params, std::int32_t numParams,
    ...
    JS_SetGlobalObject(targContext, targObject);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, eventToCall, numParams, params, eventRetVal);
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers when character with event attached casts a spell through a
// scroll |	Notes		-	Return value table
//|						-2: use NORMAL non-JS casting
//|						-1: CANCEL spellcasting
//|						0->inf: Spell delay in ms
// o------------------------------------------------------------------------------------------------o
...
std::int8_t cScript::OnSpellSuccess(CChar *tChar, std::uint8_t SpellId) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSpellSuccess", 2, params, &rval);
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers after player with event attached inputs speech after a
// request for |					speech input has been made. |	Notes
// - This function is called out of network.cpp if a speechmode(9) was previously set
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
bool cScript::AreaObjFunc(char *funcName, CBaseObject *srcObject, CBaseObject *tmpObject,
                          CSocket *s) {
...
bool cScript::ScriptRegistration(std::string scriptType) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, scriptType.c_str(), 0, params, &rval);
    return (retVal == JS_TRUE);
}
...
bool cScript::executeCommand(CSocket *s, std::string funcName, std::string executedString) {
    ...
    JS_SetGlobalObject(targContext, targObject);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, funcName.c_str(), 2, params, &rval);
...
bool cScript::OnIterateSpawnRegions(CSpawnRegion *a, std::uint32_t &b) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onIterateSpawnRegions", 1, params, &rval);
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	"Packet hook" event that allows interception of incoming network
// packets |	Notes		-	Requires OVERLOADPACKETS ini setting to be enabled, and
...
std::int8_t cScript::OnCharDoubleClick(CChar *currChar, CChar *targChar) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onCharDoubleClick", 2, params, &rval);
...
std::int8_t cScript::OnUseBandageMacro(CSocket *mSock, CChar *targChar, CItem *bandageItem) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onUseBandageMacro", 3, params, &rval);
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for characters with event attached when selecting a target
// as part of |					default AI behaviour
...
std::int8_t cScript::OnAICombatTarget(CChar *attacker, CChar *target) {
    ...
    params[1] = OBJECT_TO_JSVAL(targObj);
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onAICombatTarget", 2, params, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for characters with event attached when performing a death
// blow in combat |	Notes		-	Returning FALSE will also run hard code for this
// scenario
//|					Returning TRUE will override code's default handling of this
// scenario
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Triggers for characters with event attached every time combat damage
// is calculated |	Notes		-	Returning -1 will default to hard code handling of
...
std::int16_t cScript::OnCombatDamageCalc(CChar *attacker, CChar *defender, std::uint8_t getFightSkill,
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onCombatDamageCalc", 4, params, &rval);
    if (retVal == JS_FALSE) {
...
// o------------------------------------------------------------------------------------------------o
std::int8_t cScript::OnDamage(CChar *damaged, CChar *attacker, std::int16_t damageValue, Weather::type_t damageType) {
    const std::int8_t RV_NOFUNC = -1;
...
// o------------------------------------------------------------------------------------------------o
std::int8_t cScript::OnDamageDeal(CChar *attacker, CChar *damaged, std::int16_t damageValue, Weather::type_t damageType) {
    const std::int8_t RV_NOFUNC = -1;
    if (!ValidateObject(damaged))
        return RV_NOFUNC;
...
//|	Date		-	26th November, 2011
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Allows determining what happens when an item is in the process of
// being bought
...
std::int8_t cScript::OnBuyFromVendor(CSocket *tSock, CChar *objVendor, CBaseObject *objItemBought,
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onBuyFromVendor", 4, params, &rval);
...
std::int8_t cScript::OnSellToVendor(CSocket *tSock, CChar *objVendor, CBaseObject *objItemSold,
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSellToVendor", 4, params, &rval);
...
std::int8_t cScript::OnBoughtFromVendor(CSocket *tSock, CChar *objVendor, CBaseObject *objItemBought,
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onBoughtFromVendor", 4, params, &rval);
...
std::int8_t cScript::OnSoldToVendor(CSocket *tSock, CChar *objVendor, CBaseObject *objItemSold,
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onSoldToVendor", 4, params, &rval);
...
std::int8_t cScript::OnHouseCommand(CSocket *tSock, CMultiObj *objMulti, std::uint8_t cmdId) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onHouseCommand", 3, params, &rval);
...
std::int8_t cScript::OnEnterEvadeState(CChar *npc, CChar *enemy) {
    ...
    JSBool retVal =
    JS_CallFunctionName(targContext, targObject, "onEnterEvadeState", 2, params, &rval);
    if (retVal == JS_FALSE) {
