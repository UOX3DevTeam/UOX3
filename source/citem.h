...
class CItem : public CBaseObject {
  protected:
    ...
    SI08 rank; // for rank system, this value is the LEVEL of the item from 1 to 10.  Simply
    ...
    SI08 movable; // 0=Default as stored in client, 1=Always movable, 2=Never movable, 3=Owner
                  // movable.
    TIMERVAL tempTimer;
    TIMERVAL decayTime;
    ...
    SI32 baseWeight; // Base weight of item. Applied when item is created for the first time, based
    ...
    UI08 stealable; // 0=Not stealable, 1=Stealable (default, most items), 2=Special Stealable (town
                    // rares, etc)
    ...
    UI32 value[3];  // Price a shopkeep buys and sells items for, with price on player vendor as
  ...
  public:
    ...
    auto TextMessage(CSocket *s, SI32 dictEntry, R32 secsFromNow = 0.0f, UI16 Colour = 0x005A)
        -> void;
