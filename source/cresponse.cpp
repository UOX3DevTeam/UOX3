...
CEscortResponse::CEscortResponse(bool newVal) { findDest = newVal; }
// o------------------------------------------------------------------------------------------------o
...
void CEscortResponse::Handle([[maybe_unused]] CSocket *mSock, CChar *mChar) {
    ...
    for (auto &nearbyNpc : FindNearbyNPCs(mChar, DIST_NEARBY)) {
        if (nearbyNpc->GetQuestType() == QT_ESCORTQUEST) {
            // I WILL TAKE THEE
            // If this is a request for hire and the PC is within range of the NPC and the NPC is
...
CBasePetResponse::CBasePetResponse(const std::string &text) { ourText = text; }
// o------------------------------------------------------------------------------------------------o
