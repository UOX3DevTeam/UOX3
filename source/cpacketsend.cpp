...
//|						BYTE[7] unknown5
//|
//|					Newer, more up to date version of the packet. When did it
// change? |						BYTE cmd |
// BYTE[4] player id |						BYTE[4] unknown1 |
// BYTE[2] bodyType |						BYTE[2] xLoc |
// BYTE[2] yLoc |						BYTE Unknown (0x0) |
// BYTE zLoc |						BYTE direction
//|						BYTE[4] unknown2 (0x0, Described as X in Jerrith's
//old docs) |						BYTE[4] unknown3 (0x0, Described as Y in
// Jerrith's old docs) |						BYTE unknown4 (0x0) |
...
void CPCharLocBody::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Z Loc            : " << static_cast<std::uint16_t>(pStream.GetByte(16)) << std::endl;
    outStream << "Direction        : " << static_cast<std::uint16_t>(pStream.GetByte(17)) << std::endl;
...
void CPCharLocBody::InternalReset() {
    ...
    for (std::uint8_t k = 5; k < 9; ++k) {
        pStream.WriteByte(k, 0x00);
    }
    for (std::uint8_t j = 33; j < 37; ++j) {
        pStream.WriteByte(j, 0x00);
    }
    
    HighlightColour(0);
}
CPCharLocBody::CPCharLocBody() { InternalReset(); }
...
void CPCharLocBody::Flag(std::uint8_t toPut) { pStream.WriteByte(31, toPut); }
void CPCharLocBody::HighlightColour(std::uint8_t color) { pStream.WriteByte(32, color); }
...
void CPacketSpeech::GhostIt([[maybe_unused]] std::uint8_t method) {
...
void CPacketSpeech::Speech(const std::string &toPut) {
    ...
    pStream.WriteShort(1, static_cast<std::uint16_t>(newLen));
}
...
void CPWalkDeny::SequenceNumber(char newValue) { pStream.WriteByte(1, newValue); }
void CPWalkDeny::X(std::int16_t newValue) { pStream.WriteShort(2, newValue); }
void CPWalkDeny::Y(std::int16_t newValue) { pStream.WriteShort(4, newValue); }
void CPWalkDeny::Z(std::int8_t newValue) { pStream.WriteByte(7, newValue); }
void CPWalkDeny::Direction(char newValue) { pStream.WriteByte(6, newValue); }
...
void CPWalkOK::SequenceNumber(char newValue) { pStream.WriteByte(1, newValue); }
void CPWalkOK::FlagColour(std::uint8_t newValue) { pStream.WriteByte(2, newValue); }
...
void CPExtMove::FlagColour(std::uint8_t newValue) { pStream.WriteByte(16, newValue); }
...
void CPExtMove::SetFlags(CChar &toCopy) {
    ...
    if (ServerConfig::shared().enableClients.enableClient7000() ||  ServerConfig::shared().enableClients.enableClient7090() || ServerConfig::shared().enableClients.enableClient70160() || ServerConfig::shared().enableClients.enableClient70240() || ServerConfig::shared().enableClients.enableClient70300() || ServerConfig::shared().enableClients.enableClient70331() || ServerConfig::shared().enableClients.enableClient704565() || ServerConfig::shared().enableClients.enableClient70610() ) {
        ...
        const std::uint8_t BIT__FLYING = 2; //	0x04, flying (post 7.0.0.0)
        const std::uint8_t BIT__GOLDEN = 3; //	0x08, yellow healthbar
        // const std::uint8_t BIT__IGNOREMOBILES = 4;	// 0x10, ignore other mobiles?
        ...
        const std::uint8_t BIT__INVUL = 0;  //	0x01
        const std::uint8_t BIT__DEAD = 1;   //	0x02
        const std::uint8_t BIT__POISON = 2; //	0x04, poison
        const std::uint8_t BIT__GOLDEN = 3; //	0x08, yellow healthbar
        // const std::uint8_t BIT__IGNOREMOBILES = 4;	// 0x10, ignore other mobiles?
    ...
    const std::uint8_t BIT__ATWAR = 6; // 0x40
    const std::uint8_t BIT__DEAD = 7;  // 0x80, dead or hidden
    flag.set(BIT__ATWAR, toCopy.IsAtWar());
    flag.set(BIT__DEAD, (toCopy.IsDead() || toCopy.GetVisible() != VT_VISIBLE));
    
    pStream.WriteByte(15, static_cast<std::uint8_t>(flag.to_ulong()));
}
...
//| Function	-	CPAttackOK()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet informing client if it's ok or not to attack
// target
// o------------------------------------------------------------------------------------------------o
...
//|							ID is set to 00 00 00 00 when attack is
// refused.
// o------------------------------------------------------------------------------------------------o
void CPAttackOK::CopyData(CChar &toCopy) { Serial(toCopy.GetSerial()); }
...
//|						BYTE[4] item/char id
// o------------------------------------------------------------------------------------------------o
void CPRemoveItem::CopyData(CBaseObject &toCopy) { Serial(toCopy.GetSerial()); }
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet telling client to update season visuals of
// world
// o------------------------------------------------------------------------------------------------o
...
//|
//|					Note: Server message
//|					Note: if id2 = 1, then this is a season change. Although,
//this could also just
//|							be a flag to play a sound FX on season change
//or not...
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet updating client with current overall light
// level
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet updating client with skill values and lock
// states
// o------------------------------------------------------------------------------------------------o
...
void CPUpdIndSkill::CopyData(CChar &i, std::uint8_t sNum) {
    SkillNum(sNum);
...
void CPUpdIndSkill::Character(CChar &i, std::uint8_t sNum) { CopyData(i, sNum); }
...
//|							BYTE[2] # bought
// o------------------------------------------------------------------------------------------------o
void CPBuyItem::CopyData(CBaseObject &i) { Serial(i.GetSerial()); }
CPBuyItem::CPBuyItem() { InternalReset(); }
...
// const std::uint32_t loopbackIP = (127<<24) + 1;
// o------------------------------------------------------------------------------------------------o
//| Function	-	CPRelay()
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE[4] new key
// o------------------------------------------------------------------------------------------------o
CPRelay::CPRelay() { InternalReset(); }
CPRelay::CPRelay(std::uint32_t newIP) {
    InternalReset();
    ServerIP(newIP);
}
CPRelay::CPRelay(std::uint32_t newIP, std::uint16_t newPort) {
    InternalReset();
    ServerIP(newIP);
    Port(newPort);
}
...
//| Function	-	CPWornItem()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to equip a single item for player and update
// paperdoll
// o------------------------------------------------------------------------------------------------o
...
//|							0x0c = swing (attack with sword over and
// side) |							0x0d = swing (attack with sword
// side) |							0x0e = stab with point of sword |
// 0x0f = ready stance |							0x10 = magic (butter
// churn!) |							0x11 = hands over head (balerina) |
//0x12 = bow shot |							0x13 = crossbow |
//0x14 = get hit |							0x15 = fall down and die
//(backwards) |							0x16 = fall down and die (forwards)
...
//|						BYTE[1] Frame Count
//|						BYTE[2] repeat (1 = once / 2 = twice / 0 = repeat
// forever) |						BYTE forward/backwards(0x00=forward,
// 0x01=backwards)
//|						BYTE repeat Flag (0 - Don't repeat / 1 repeat)
//|						BYTE frame Delay (0x00 - fastest / 0xFF - Too slow
//to watch)
// o------------------------------------------------------------------------------------------------o
...
CPCharacterAnimation::CPCharacterAnimation() { InternalReset(); }
...
void CPCharacterAnimation::DoBackwards(bool newValue) {
    pStream.WriteByte(11, static_cast<std::uint8_t>((newValue ? 1 : 0)));
}
void CPCharacterAnimation::RepeatFlag(bool newValue) {
    pStream.WriteByte(12, static_cast<std::uint8_t>((newValue ? 1 : 0)));
}
void CPCharacterAnimation::FrameDelay(std::uint8_t delay) { pStream.WriteByte(13, delay); }
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to play specific animation for character in
// clients 7.0.0.0+
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
void CPNewCharacterAnimation::CopyData(CChar &toCopy) { Serial(toCopy.GetSerial()); }
CPNewCharacterAnimation::CPNewCharacterAnimation() { InternalReset(); }
...
//|						BYTE zLoc
//|
//|						Note: Only used with the character being played by
//the client. |						Ancient client version Flags: | 0x01 =
// Unknown |							0x02 = CanAlterPaperdoll | 0x04 =
// Posioned |							0x08 = Yellow HealthBar | 0x10 =
...
void CPDrawGamePlayer::CopyData(CChar &toCopy) {
    ...
    if (ServerConfig::shared().enableClients.enableClient7000() ||  ServerConfig::shared().enableClients.enableClient7090() || ServerConfig::shared().enableClients.enableClient70160() || ServerConfig::shared().enableClients.enableClient70240() || ServerConfig::shared().enableClients.enableClient70300() || ServerConfig::shared().enableClients.enableClient70331() || ServerConfig::shared().enableClients.enableClient704565() || ServerConfig::shared().enableClients.enableClient70610() ) {
        ...
        const std::uint8_t BIT__FLYING = 2; //	0x04, flying (post 7.0.0.0)
        const std::uint8_t BIT__GOLDEN = 3; //	0x08, yellow healthbar
        // const std::uint8_t BIT__IGNOREMOBILES = 5;	// 0x10, ignore other mobiles?
    ...
    const std::uint8_t BIT__ATWAR = 6; //	0x40
    const std::uint8_t BIT__INVIS = 7; //	0x80
...
CPDrawGamePlayer::CPDrawGamePlayer() { InternalReset(); }
...
CPPersonalLightLevel::CPPersonalLightLevel() { InternalReset(); }
...
//|					Packet Build
//|						BYTE cmd
//|						BYTE mode (0x00=quiet, repeating, 0x01=single normally
//played sound effect) |						BYTE[2] SoundModel | BYTE[2]
// unknown3 (speed/volume modifier? Line of sight stuff?) | BYTE[2] xLoc |
// BYTE[2] yLoc | BYTE[2] zLoc / Or not z at all? Could be related to sequences when sent after
// packet 0x7B
// o------------------------------------------------------------------------------------------------o
CPPlaySoundEffect::CPPlaySoundEffect() { InternalReset(); }

void CPPlaySoundEffect::Mode(std::uint8_t mode) { pStream.WriteByte(1, mode); }
void CPPlaySoundEffect::Model(std::uint16_t newModel) { pStream.WriteShort(2, newModel); }
...
CPPaperdoll::CPPaperdoll() { InternalReset(); }
...
//|					Packet Build
//|						BYTE cmd
//|						BYTE type (0x00 – “It starts to rain”, 0x01 – “A fierce
//storm approaches.”, 0x02 – “It begins to snow”, 0x03 - “A storm is brewing.”, 0xFF – None (turns
// off sound effects), 0xFE (no effect?? Set temperature?)
...
//|					Note: If it is raining, you can add snow by setting the num to
//the
// num of rain |						currently going, plus the number of snow
// you want. |					Note: Weather messages are only displayed when
// weather starts. |					Note: Weather will end automatically after 6
// minutes without any weather change packets.
//|					Note: You can totally end weather (to display a new message)
//by teleporting.
//|						I think it’s either the 0x78 or 0x20 messages that reset
//it, though I
...
CPWeather::CPWeather() { InternalReset(); }
CPWeather::CPWeather(std::uint8_t nType) {
    InternalReset();
    Type(nType);
}
CPWeather::CPWeather(std::uint8_t nType, std::uint8_t nParts) {
    InternalReset();
    Type(nType);
    Particles(nParts);
}
CPWeather::CPWeather(std::uint8_t nType, std::uint8_t nParts, std::uint8_t nTemp) {
    InternalReset();
    Type(nType);
    Particles(nParts);
    Temperature(nTemp);
}
void CPWeather::Type(std::uint8_t nType) { pStream.WriteByte(1, nType); }
void CPWeather::Particles(std::uint8_t nParts) {
    if (nParts > 70) {
        nParts = 70;
    }
...
void CPWeather::Temperature(std::uint8_t nTemp) { pStream.WriteByte(3, nTemp); }
...
CPGraphicalEffect::CPGraphicalEffect(std::uint8_t effectType) {
    InternalReset();
    Effect(effectType);
}
...
void CPGraphicalEffect::Z(std::int8_t nZ) { pStream.WriteByte(16, nZ); }
void CPGraphicalEffect::XTrg(std::int16_t nX) { pStream.WriteShort(17, nX); }
void CPGraphicalEffect::YTrg(std::int16_t nY) { pStream.WriteShort(19, nY); }
void CPGraphicalEffect::ZTrg(std::int8_t nZ) { pStream.WriteByte(21, nZ); }
void CPGraphicalEffect::Speed(std::uint8_t nSpeed) { pStream.WriteByte(22, nSpeed); }
void CPGraphicalEffect::Duration(std::uint8_t nDuration) { pStream.WriteByte(23, nDuration); }
void CPGraphicalEffect::AdjustDir(bool nValue) {
    pStream.WriteByte(26, static_cast<std::uint8_t>((nValue ? 0 : 1)));
}
void CPGraphicalEffect::ExplodeOnImpact(bool nValue) {
    pStream.WriteByte(27, static_cast<std::uint8_t>((nValue ? 1 : 0)));
}

CPGraphicalEffect::CPGraphicalEffect(std::uint8_t effectType, CBaseObject &src, CBaseObject &trg) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
    TargetSerial(trg);
}
CPGraphicalEffect::CPGraphicalEffect(std::uint8_t effectType, CBaseObject &src) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
}
...
void CPGraphicalEffect::SourceLocation(std::int16_t x, std::int16_t y, std::int8_t z) {
    X(x);
    Y(y);
    Z(z);
}
void CPGraphicalEffect::TargetLocation(std::int16_t x, std::int16_t y, std::int8_t z) {
    XTrg(x);
    YTrg(y);
    ZTrg(z);
}
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet(s) to update player's health, mana and
// stamina |					Also used to update the health of items/multis with
// damageable flag set
// o------------------------------------------------------------------------------------------------o
...
CPUpdateStat::CPUpdateStat(CBaseObject &toUpdate, std::uint8_t statNum, bool normalizeStats) {
    InternalReset();
...
CPPlayMusic::CPPlayMusic(std::int16_t musicId) {
    InternalReset();
    MusicId(musicId);
}
CPPlayMusic::CPPlayMusic() { InternalReset(); }
void CPPlayMusic::MusicId(std::int16_t musicId) { pStream.WriteShort(1, musicId); }
...
//|						BYTE[2] model-Gump
//|							0x003c = backpack
//|						BYTE[2] container type // added in UO:HS clients, 0x00
...
CPDrawContainer::CPDrawContainer() { InternalReset(); }
CPDrawContainer::CPDrawContainer(CItem &toCopy) {
    InternalReset();
    CopyData(toCopy);
}
void CPDrawContainer::Model(std::uint16_t newModel) {
    pStream.WriteShort(5, newModel); //-1?
}
void CPDrawContainer::ContType(std::uint16_t contType) {
    pStream.ReserveSize(9);
...
//then
// always 0x00 as msb) |							BYTE[2] color of shown
...
void CPOpenGump::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Question Length  : " << std::dec << static_cast<std::int16_t>(pStream.GetByte(9))
    << std::endl;
    outStream << "Question         : ";
    
    for (std::uint32_t i = 0; i < pStream.GetByte(9); ++i) {
    ...
    for (std::uint32_t j = 0; j < pStream.GetByte(responseBaseOffset); ++j) {
        outStream << "    Response " << j << std::endl;
        outStream << "      Model  : " << std::hex << pStream.GetUShort(offsetCount + 0)
        << std::endl;
        outStream << "      Colour : " << pStream.GetUShort(offsetCount + 2) << std::endl;
        outStream << "      Length : " << std::dec << pStream.GetByte(offsetCount + 4) << std::endl;
        outStream << "      Text   : ";
        
        for (std::uint32_t k = 0; k < pStream.GetByte(offsetCount + 4); ++k) {
...
void CPOpenGump::Question(std::string toAdd) {
    pStream.ReserveSize(
                        10 + toAdd.length() +
                        2); // 10 for start of string, length of string + nullptr, plus spot for # responses
...
#endif
    pStream.WriteByte(9, static_cast<std::uint8_t>(toAdd.length() + 1));
    responseBaseOffset = (pStream.GetSize() - 1);
    responseOffset = responseBaseOffset + 1;
}
void CPOpenGump::AddResponse(std::uint16_t modelNum, std::uint16_t colour, std::string responseText) {
    pStream.WriteByte(responseBaseOffset,
...
#endif
    std::uint16_t toAdd = static_cast<std::uint16_t>(5 + responseText.length());
    ...
    pStream.WriteByte((responseOffset + 4), static_cast<std::uint8_t>(responseText.length()));
...
CPOpenGump::CPOpenGump() { InternalReset(); }
CPOpenGump::CPOpenGump(CChar &toCopy) {
    InternalReset();
    CopyData(toCopy);
}
void CPOpenGump::Length(std::int32_t totalLines) { pStream.WriteShort(1, totalLines); }
void CPOpenGump::GumpIndex(std::int32_t index) { pStream.WriteShort(7, index); }
...
//map/landscape tile) |							Note: the model # shouldn’t
// be trusted.
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE[2] maxHitpoints
//|						BYTE name change flag (0x1 = allowed, 0 = not
// allowed) |						BYTE Status Flag/Supported Features |
// Status Flag
//|							0x00: no more data following (end of packet
// here). |							0x01: T2A Extended Info |
...
//|
//|					Note: For characters other than the player, currentHitpoints
//and maxHitpoints
//|					are not the actual values.  MaxHitpoints is a fixed value,
//and currentHitpoints |					works like a percentage.
// o------------------------------------------------------------------------------------------------o
void CPStatWindow::SetCharacter(CChar &toCopy, CSocket &target) {
    ...
    if (toCopy.GetSerial() != mChar->GetSerial()) {
        ...
        Name(charName);
        std::int16_t currentHP = toCopy.GetHP();
        std::uint16_t maxHP = toCopy.GetMaxHP();
        ...
        if (toCopy.IsTamed() && ValidateObject(toCopy.GetOwnerObj()) &&
            toCopy.GetOwner() == mChar->GetSerial()) {
            NameChange(true);
        }
        else {
            NameChange(false);
...
CPStatWindow::CPStatWindow(CBaseObject &toCopy, CSocket &target) {
    ...
    if (toCopy.CanBeObjType(CBaseObject::OT_CHAR)) {
        ...
        std::int16_t currentHP = itemObj->GetHP();
        std::uint16_t maxHP = itemObj->GetMaxHP();
        std::int16_t percentHP = 0;
        if (currentHP > 0) {
            percentHP = static_cast<std::int16_t>(
                                                  ceil(100 * (static_cast<float>(currentHP) / static_cast<float>(maxHP))));
...
void CPStatWindow::CurrentHP(std::int16_t nValue) { pStream.WriteShort(37, nValue); }
void CPStatWindow::MaxHP(std::int16_t nValue) { pStream.WriteShort(39, nValue); }
void CPStatWindow::NameChange(bool nValue) {
    pStream.WriteByte(41, static_cast<std::uint8_t>((nValue ? 0xFF : 0)));
}
void CPStatWindow::Flag(std::uint8_t nValue) { pStream.WriteByte(42, nValue); }
void CPStatWindow::Sex(std::uint8_t nValue) { pStream.WriteByte(43, nValue); }
void CPStatWindow::Strength(std::int16_t nValue) { pStream.WriteShort(44, nValue); }
void CPStatWindow::Dexterity(std::int16_t nValue) { pStream.WriteShort(46, nValue); }
void CPStatWindow::Intelligence(std::int16_t nValue) { pStream.WriteShort(48, nValue); }
void CPStatWindow::Stamina(std::int16_t nValue) { pStream.WriteShort(50, nValue); }
void CPStatWindow::MaxStamina(std::int16_t nValue) { pStream.WriteShort(52, nValue); }
void CPStatWindow::Mana(std::int16_t nValue) { pStream.WriteShort(54, nValue); }
void CPStatWindow::MaxMana(std::int16_t nValue) { pStream.WriteShort(56, nValue); }
void CPStatWindow::Gold(std::uint32_t gValue) { pStream.WriteLong(58, gValue); }
void CPStatWindow::AC(std::uint16_t nValue) { pStream.WriteShort(62, nValue); }
void CPStatWindow::Weight(std::uint16_t nValue) {
    pStream.WriteShort(64, nValue);
    byteOffset = 66;
}
// extended5
void CPStatWindow::MaxWeight(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::Race(std::uint8_t value) {
    pStream.WriteByte(byteOffset, value);
    byteOffset += 1;
}
// extended5 end
// extended3 start
void CPStatWindow::StatCap(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::CurrentPets(std::uint8_t value) {
    pStream.WriteByte(byteOffset, value);
    byteOffset += 1;
}
void CPStatWindow::MaxPets(std::uint8_t value) {
    pStream.WriteByte(byteOffset, value);
    byteOffset += 1;
}
// extended3 end
// extended4 start
void CPStatWindow::FireResist(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::ColdResist(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::PoisonResist(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::EnergyResist(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::Luck(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::DamageMin(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::DamageMax(std::uint16_t value) {
    pStream.WriteShort(byteOffset, value);
    byteOffset += 2;
}
void CPStatWindow::TithingPoints(std::uint32_t value) {
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to show idle warning or login rejection
// message
// o------------------------------------------------------------------------------------------------o
...
//|								"Another character from this account is
...
CPTime::CPTime(std::uint8_t hour, std::uint8_t minute, std::uint8_t second) {
    InternalReset();
    Hour(hour);
    Minute(minute);
    Second(second);
...
//|						BYTE[2] blockSize
//|						BYTE[2] unknown1
//|					The client sends two of these independent of the color chosen.
//It sends two of
//|						them in quick succession as part of the "same" packet.
//The
// unknown1 is |						0x00 0x01 in the first and 0x00 0x02 in
// the second.
//|					Note, this message has been removed.  It is no longer used.
// o------------------------------------------------------------------------------------------------o
CPTextEmoteColour::CPTextEmoteColour() { BlockSize(5); }
void CPTextEmoteColour::BlockSize(std::int16_t newValue) {
    pStream.ReserveSize(newValue);
    pStream.WriteByte(0, 0x69);
    pStream.WriteShort(1, newValue);
}
void CPTextEmoteColour::Unknown(std::int16_t newValue) { pStream.WriteShort(3, newValue); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CPWarMode()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to respond to request for war mode/set war
// mode status
// o------------------------------------------------------------------------------------------------o
...
//|
//|					Server also replies with 0x77 packet to update player after
// changing war mode
// o------------------------------------------------------------------------------------------------o
...
CPWarMode::CPWarMode(std::uint8_t nFlag) {
    InternalReset();
    Flag(nFlag);
}
...
CPPauseResume::CPPauseResume() { InternalReset(); }
CPPauseResume::CPPauseResume(std::uint8_t mode) {
    InternalReset();
    Mode(mode);
}
void CPPauseResume::Mode(std::uint8_t mode) { pStream.WriteByte(1, mode); }

bool CPPauseResume::ClientCanReceive(CSocket *mSock) {
    if (mSock == nullptr)
        return false;
    
    switch (mSock->clientType().type) {
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to open a specific URL in player's default
// browser
// o------------------------------------------------------------------------------------------------o
...
CPWebLaunch::CPWebLaunch() { InternalReset(); }
CPWebLaunch::CPWebLaunch(const std::string &txt) {
    InternalReset();
    Text(txt);
}
void CPWebLaunch::Text(const std::string &txt) {
    const std::int16_t tLen = static_cast<std::int16_t>(txt.length()) + 4;
    SetSize(tLen);
    pStream.WriteString(3, txt, txt.size());
}

void CPWebLaunch::SetSize(std::int16_t newSize) {
...
CPTrackingArrow::CPTrackingArrow() { InternalReset(); }
CPTrackingArrow::CPTrackingArrow(std::int16_t x, std::int16_t y) {
    InternalReset();
    Location(x, y);
}
void CPTrackingArrow::Location(std::int16_t x, std::int16_t y) {
...
CPBounce::CPBounce() { InternalReset(); }
CPBounce::CPBounce(std::uint8_t mode) {
    InternalReset();
    Mode(mode);
}
void CPBounce::Mode(std::uint8_t mode) { pStream.WriteByte(1, mode); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CPDyeVat()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to resond to client request to show dye
// window
// o------------------------------------------------------------------------------------------------o
...
CPDyeVat::CPDyeVat() { InternalReset(); }
...
void CPMultiPlacementView::InternalReset() {
    ...
    for (std::uint8_t k = 20; k < 26; ++k) {
        pStream.WriteByte(k, 0);
    }
}
void CPMultiPlacementView::CopyData(CItem &target) { DeedSerial(target.GetSerial()); }
CPMultiPlacementView::CPMultiPlacementView() { InternalReset(); }
...
//|						0x08	Enable LBR features: skills, maps, mp3s, LBR
// monsters in 2D client |						0x10	Enable AOS update
//(Necromancer/Paladin skills, malas map/AOS monsters if AOS installation present) |
// 0x20	Enable Sixth Character Slot
//|						0x40	Enable SE features: Ninja/Samurai, spells,
//skills, map, housing tiles
//|						0x80	Enable ML features: elven race, spells,
//skills, housing tiles |						0x100	Enable the Eight Age
// splash screen |						0x200	Enable the Ninth Age splash
// screen and crystal/shadow housing tiles
...
//|						0x8000	Live (non-trial) Account. Since client 4.0
//this bit has to be set, otherwise bits 3..14 are ignored.
//|						0x10000	Enable SA features: gargoyle race, spells,
// skills, housing tiles |						0x20000	Enable HS features
//|						0x40000	Enable Gothing housing tiles
//|						0x80000	Enable Rustic housing tiles
//|
//|						Example
//|						0	neither T2A NOR LBR, equal to not sending it
//at all, |						1	is T2A, chatbutton, | 2	is LBR
// without chatbutton, |						3	is LBR with
// chatbutton… |						8013	LBR + chatbutton + AOS
// enabled
//|
//|						Note1: this message is send immediately after login.
//|						Note2: on OSI  servers this controls features OSI
// enables/disables via “upgrade codes.” |						Note3: a 3
// doesn’t seem to “hurt” older (NON LBR) clients.
// o------------------------------------------------------------------------------------------------o
CPEnableClientFeatures::CPEnableClientFeatures(CSocket *mSock) {
    if (mSock->clientType() <= ClientType::KR3D) {
...
void CPEnableClientFeatures::log(std::ostream &outStream, bool fullHeader) {
    ...
    std::uint32_t lastByte = pStream.GetUShort(1);
    outStream << "Flags          : " << std::hex << static_cast<std::uint32_t>(lastByte) << std::dec
    << std::endl;
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with feature negotiation request for
//assistant tools |					like Razor and AssistUO
// o------------------------------------------------------------------------------------------------o
...
//|
//|					At the same time as this is sent in pcmanage.cpp, a timer is
// started if
//|					KICKONASSISTANTSILENCE is enabled in uox.ini. If no valid
//response
// is received |					within 30 seconds, player is disconnected from
// the server.
// o------------------------------------------------------------------------------------------------o
...
CPAddItemToCont::CPAddItemToCont() { InternalReset(); }
...
void CPKickPlayer::CopyData(CChar &toCopy) { Serial(toCopy.GetSerial()); }
CPKickPlayer::CPKickPlayer() { InternalReset(); }
...
CPResurrectMenu::CPResurrectMenu() { InternalReset(); }
CPResurrectMenu::CPResurrectMenu(std::uint8_t action) {
    InternalReset();
    Action(action);
}
void CPResurrectMenu::Action(std::uint8_t action) { pStream.WriteByte(1, action); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CPFightOccurring()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet update client about combat taking place
// nearby
// o------------------------------------------------------------------------------------------------o
...
CPFightOccurring::CPFightOccurring() { InternalReset(); }
...
//|							0x02 - Full List of skills with skill cap for
//each skill |							0xDF - Single skill update with
// skill cap for skill |						Repeat for each skill: |
// BYTE[2] Skill ID number
//|							BYTE[2] Skill Value * 10
//|							BYTE[2] Unmodified Value * 10
//|							BYTE Skill Lock
...
void CPSkillsValues::SetCharacter(CChar &toCopy) {
    std::uint8_t numSkills = NumSkills();
    for (std::int8_t i = 0; i < numSkills; ++i) {
        SkillEntry(i, toCopy.GetSkill(i), toCopy.GetBaseSkill(i), toCopy.GetSkillLock(i));
    }
}

void CPSkillsValues::NumSkills(std::uint8_t numSkills) {
...
void CPSkillsValues::BlockSize(std::int16_t newValue) {
    pStream.ReserveSize(newValue);
...
CPSkillsValues::CPSkillsValues() { InternalReset(); }
...
void CPMapMessage::UpperLeft(std::int16_t x, std::int16_t y) {
    pStream.WriteShort(7, x);
    pStream.WriteShort(9, y);
}
void CPMapMessage::LowerRight(std::int16_t x, std::int16_t y) {
    pStream.WriteShort(11, x);
    pStream.WriteShort(13, y);
}
void CPMapMessage::Dimensions(std::int16_t width, std::int16_t height) {
    pStream.WriteShort(15, width);
    pStream.WriteShort(17, height);
}
void CPMapMessage::GumpArt(std::int16_t newArt) { pStream.WriteShort(5, newArt); }
void CPMapMessage::KeyUsed(std::uint32_t key) { pStream.WriteLong(1, key); }
...
//|						BYTE[30] author
//|
//|					NOTE: Client sends a 0x93 message on book close. Update packet
//for the server
//|						to handle changes. Write Flag through Page Count are all
//0's on client response |					NOTE: Replaced with 0xD4 in newer
// clients?
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE cancel (0=disable, 1=enable - if enabled, client is
//able to cancel query) |						BYTE style (0=disable,
// 1=normal, 2=numerical)
//|						BYTE[4] format (if style 1, max text len, if style2,
...
CPGumpTextEntry::CPGumpTextEntry() { InternalReset(); }
...
void CPGumpTextEntry::Style(std::uint8_t newVal) {
    std::int16_t t1Len = Text1Len();
...
void CPGumpTextEntry::Text1(const std::string &txt) {
    ...
    BlockSize(static_cast<std::int16_t>(sLen + 20)); // 11 + 1 + 8
    Text1Len(static_cast<std::int16_t>(sLen + 1));
    pStream.WriteString(11, txt, sLen);
}
void CPGumpTextEntry::Text2(const std::string &txt) {
    size_t sLen = txt.length();
    std::int16_t currentSize = CurrentSize();
    BlockSize(static_cast<std::uint16_t>(currentSize + sLen + 1));
    Text2Len(static_cast<std::int16_t>(sLen + 1));
    pStream.WriteString(currentSize, txt, sLen);
}
void CPGumpTextEntry::BlockSize(std::int16_t newVal) {
...
void CPGumpTextEntry::Text2Len(std::int16_t newVal) {
    std::int16_t t1Len = Text1Len();
...
void CPGodModeToggle::CopyData(CSocket *s) { pStream.WriteByte(1, s->GetByte(1)); }
...
CPCharDeleteResult::CPCharDeleteResult() { InternalReset(); }
CPCharDeleteResult::CPCharDeleteResult(std::int8_t result) {
    InternalReset();
    DeleteResult(result);
}
void CPCharDeleteResult::DeleteResult(std::int8_t result) { pStream.WriteByte(1, result); }
...
//|						0x05 - Bad request
// o------------------------------------------------------------------------------------------------o
CharacterListUpdate::CharacterListUpdate(std::uint8_t charCount) {
    numChars = charCount;
    InternalReset();
}
...
void CharacterListUpdate::AddCharName(std::uint8_t charNum, std::string charName) {
    std::uint16_t byteOffset = 4 + (charNum > 0 ? (charNum * 60) : 0);
...
//|
//|					Notes
//|						Unverified encryption method of keys is AES in CFB
// Mode
//|						As of 08/02/2009, this is the current encryption known for
//KR clients up to date: |							0xe3, | 0x4d00, |
// 0x03000000, | 0x02, 0x01, 0x03, |							0x13000000,
// | 0x02, 0x11, 0x00, 0xfc, 0x2f, 0xe3, 0x81, 0x93, 0xcb, 0xaf, 0x98, 0xdd, 0x83, 0x13, 0xd2, 0x9e,
// 0xea, 0xe4, 0x13, |							0x10000000, |
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet for krrios client special packet, used by
// ClassicUO
//|					for things like updating party/guild member locations in
// WorldMap
// o------------------------------------------------------------------------------------------------o
...
//|						Byte[2] length
//|						Byte type
//|							0x00 // krrios special packet accepted, enable
//WorldMap updates |							0x01 // custom party info |
...
//|								BYTE locations // bool, whether to
//update locations on map
//|								Loop for each party/guild member
// online |									BYTE[4] serial |
// BYTE[2] x |									BYTE[2] y |
// BYTE world
//|									BYTE healthpoint percentage of
//max hp |								BYTE[5] 0x00000 | 0x03 //
// runebook contents |							0x04 // guardline data |
// 0xf0 // ??? | 0xfe // Razor ack sent
// o------------------------------------------------------------------------------------------------o
CPKrriosClientSpecial::CPKrriosClientSpecial(CSocket *mSock, CChar *mChar, std::uint8_t type,
                                             bool locations) {
    pStream.ReserveSize(2);
    pStream.WriteByte(0, 0xF0);
    std::uint8_t byteOffset = 3;
    ...
    switch (type) {
        case 0x00: // Party
        {
            ...
            byteOffset += 1;
            Party *myParty = PartyFactory::shared().Get(mSock->CurrcharObj());
            if (myParty != nullptr) {
                ...
                if (mList != nullptr) {
                    for (size_t j = 0; j < mList->size(); ++j) {
                        ...
                        if (ObjInRange(mSock->CurrcharObj(), partyMember, DIST_INRANGE) &&
                            partyMember->GetVisible() == 0)
                            continue;
            ...
            if (mGuild != nullptr) {
                ...
                for (size_t i = 0; i < numRecruits; i++) {
                    ...
                    if (guildRecruit != nullptr && guildRecruit->GetSocket() != nullptr) {
                        ...
                        if (locations && ObjInRange(mChar, guildRecruit, DIST_SAMESCREEN) &&
                            guildRecruit->GetVisible() == 0)
                            continue;
                        ...
                        if (locations) {
                            ...
                            byteOffset += 1;
                            std::int16_t currentHP = guildRecruit->GetHP();
                            std::uint16_t maxHP = guildRecruit->GetMaxHP();
                            std::uint8_t percentHP = static_cast<std::uint8_t>(100 * (currentHP / maxHP));
                ...
                for (size_t i = 0; i < numMembers; i++) {
                    ...
                    if (guildMember != nullptr && guildMember->GetSocket() != nullptr) {
                        ...
                        if (locations && ObjInRange(mChar, guildMember, DIST_SAMESCREEN) &&
                            guildMember->GetVisible() == 0)
                            continue;
                        ...
                        if (locations) {
                            ...
                            std::int16_t currentHP = guildMember->GetHP();
                            std::uint16_t maxHP = guildMember->GetMaxHP();
                            std::uint8_t percentHP = static_cast<std::uint8_t>(100 * (currentHP / maxHP));
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to change huge of mouse cursor for Felucca
//and Trammel |					facets, and/or to change the map in the client
// o------------------------------------------------------------------------------------------------o
...
//|						Subcommand details
//|							BYTE hue
//|								0x00 = Felucca, unhued / BRITANNIA
// map.
//|								0x01 = Trammel, hued gold /
//BRITANNIA map |								0x02 = Ilshenar |
...
void CPMapChange::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Hue            : " << static_cast<std::int16_t>(pStream.GetByte(5)) << std::endl;
    switch (pStream.GetByte(5)) {
        case 0:
            outStream << " (Felucca)" << std::endl;
            break;
        case 1:
            outStream << " (Trammel)" << std::endl;
            break;
        case 2:
            outStream << " (Ilshenar)" << std::endl;
            break;
        case 3:
            outStream << " (Malas)" << std::endl;
            break;
        default:
...
CPMapChange::CPMapChange() { InternalReset(); }
CPMapChange::CPMapChange(std::uint8_t newMap) {
    InternalReset();
    SetMap(newMap);
}
...
void CPMapChange::SetMap(std::uint8_t newMap) { pStream.WriteByte(5, newMap); }
...
// o------------------------------------------------------------------------------------------------o
//| Function	-	CPCloseGump( std::uint32_t gumpId, std::uint32_t buttonId )
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to close a generic/custom gump with a
// specified ID, |					while also allowing to specify the button ID
// response to send after
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Packet: 0xBF (General Information Packet)
...
//|						Subcommand details
//|							BYTE[4] dialogID // which gump to destroy (second ID
//in 0xB0 packet)
//|							BYTE[4] buttonId // response buttonId for
//packet 0xB1
// o------------------------------------------------------------------------------------------------o
CPCloseGump::CPCloseGump(std::uint32_t gumpId, std::uint32_t buttonId) {
    _gumpId = gumpId;
    _buttonId = buttonId;
    InternalReset();
}
...
CPItemsInContainer::CPItemsInContainer() { InternalReset(); }

CPItemsInContainer::CPItemsInContainer(CSocket *mSock, CItem *container, std::uint8_t contType,
                                       bool isPVendor) {
...
void CPItemsInContainer::Type(std::uint8_t contType) {
...
void CPItemsInContainer::AddItem(CItem *toAdd, std::uint16_t itemNum, CSocket *mSock) {
    pStream.ReserveSize(pStream.GetSize() + (uokrFlag ? 20 : 19));
    std::uint16_t baseOffset = static_cast<std::uint16_t>(5 + itemNum * (uokrFlag ? 20 : 19));
...
void CPItemsInContainer::CopyData(CSocket *mSock, CItem &toCopy) {
    ...
    // tcCont->Reverse(); // This will reverse the order in which items are displayed in the
    // container
    for (const auto &ctr : tcCont->collection()) {
        if (ValidateObject(ctr) &&
            (!isCorpse || !itemIsCorpse || (itemIsCorpse && ctr->GetLayer()))) {
            if (!ctr->IsFree()) {
...
void CPItemsInContainer::log(std::ostream &outStream, bool fullHeader) {
    ...
    for (size_t x = 0; x < numItems; ++x) {
        outStream << "  ITEM " << x << "      ID: "
        << "0x" << std::hex << pStream.GetULong(baseOffset) << std::endl;
        outStream << "      Model     : "
        << "0x" << pStream.GetUShort(baseOffset += 4) << std::endl;
        outStream << "      Amount    : " << std::dec << pStream.GetUShort(baseOffset += 3)
        << std::endl;
        outStream << "      XY        : " << pStream.GetUShort(baseOffset += 2);
        outStream << "," << pStream.GetUShort(baseOffset += 2) << std::endl;
        outStream << "      Container : "
        << "0x" << std::hex << pStream.GetULong(baseOffset += 2) << std::endl;
        outStream << "      Color     : "
        << "0x" << pStream.GetUShort(baseOffset += 4) << std::endl;
        baseOffset += 2;
    }
...
//|
//|						NOTE: This packet is always preceded by a describe
//contents
// packet (0x3c) |							with the container id as the
//(vendorId | 0x40000000) and then an open
//|							container packet (0x24?) with the vendorId only and
//a model number of
//|							0x0030 (probably the model # for the buy
// screen)
//|						NOTE: The client displays items in the buy window from
//top left to bottom
...
CPOpenBuyWindow::CPOpenBuyWindow() { InternalReset(); }
...
void CPOpenBuyWindow::AddItem(CItem *toAdd, CTownRegion *tReg, std::uint16_t &baseOffset) {
    ...
    if (temp.substr(0, 1) == "#") {
        ...
        sLen = static_cast<std::uint8_t>(itemname.size() + 1);
    }
    else {
        sLen = static_cast<std::uint8_t>(GetTileName(
                                                     (*toAdd),
                                                     itemname)); // Item name length, don't strip the nullptr (3D client doesn't like it)
    }
...
auto CPOpenBuyWindow::CopyData(CItem &toCopy, CChar *vendorId, CPItemsInContainer &iic,
                               CSocket *mSock) -> void {
...
void CPOpenBuyWindow::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Number of Items: " << std::dec << static_cast<std::int16_t>(pStream.GetByte(7))
    << std::endl;
    std::int32_t baseOffset = 8;
    for (std::uint32_t x = 0; x < pStream.GetByte(7); ++x) {
        outStream << "  ITEM " << x << std::endl;
        outStream << "      Price: " << pStream.GetULong(baseOffset) << std::endl;
        baseOffset += 4;
        outStream << "      Len  : " << static_cast<std::int16_t>(pStream.GetByte(baseOffset)) << std::endl;
        outStream << "      Name : ";
        for (std::uint8_t y = 0; y < pStream.GetByte(baseOffset); ++y) {
...
//|						Repeat the following for as many locations as you
// have |							if client version >= 7.0.13.0 |
...
//|								BYTE[4] CIty Map ( Probably Map ID same as in
//mul files have to make sure ) |
...
//|							0x04	= single character (siege) (alternative
//seen, Limit Characters) |							0x08	= enable
//npcpopup/context menus |							0x10	= unknown,
//(alternative seen, single character)
//|							0x20	= enable common AOS features
//(tooltip thing/fight system book, but not AOS monsters/map/skills)
//|							0x40	= Sixth Character Slot?
//|							8x80	= Samurai Empire?
//|							0x100	= Elven race
//|							0x200	= KR support flag1
//|							0x400	= Send UO3D client type (client will
//send 0xE1 packet) |							0x800	= unknown | 0x1000
// = 7th character slot, 2D client | 0x2000	= unknown (SA?) |
...
void CPCharAndStartLoc::log(std::ostream &outStream, bool fullHeader) {
    if (fullHeader)
        ...
        << TimeStamp() << std::endl;
    outStream << "# Chars        : " << static_cast<std::int16_t>(pStream.GetByte(3)) << std::endl;
    outStream << "Characters --" << std::endl;
    
    std::uint32_t startLocOffset, realChars;
    ...
    for (std::uint8_t i = 0; i < realChars; ++i) {
        std::uint32_t baseOffset = 4 + i * 60;
        outStream << "    Character " << static_cast<std::uint16_t>(i) << ":" << std::endl;
        outStream << "      Name: ";
        for (std::uint8_t j = 0; j < 30; ++j) {
            if (pStream.GetByte(static_cast<size_t>(baseOffset) + j) != 0) {
                outStream << static_cast<std::int8_t>(
                                                      pStream.GetByte(static_cast<size_t>(baseOffset) + j));
            }
            else {
                break;
            }
        ...
        for (std::uint8_t k = 0; k < 30; ++k) {
            if (pStream.GetByte(static_cast<size_t>(baseOffset) + k + 30) != 0) {
                outStream << static_cast<std::int8_t>(
                                                      pStream.GetByte(static_cast<size_t>(baseOffset) + k + 30));
            }
            else {
                break;
            }
    ...
    outStream << "# Starts       : " << static_cast<std::int16_t>(pStream.GetByte(startLocOffset))
    << std::endl;
    outStream << "Starting locations --" << std::endl;
    for (std::uint8_t l = 0; l < pStream.GetByte(startLocOffset); ++l) {
        std::uint32_t baseOffset = startLocOffset + 1 + l * 63;
        outStream << "    Start " << static_cast<std::int16_t>(l) << std::endl;
        outStream << "      Index       : " << static_cast<std::int16_t>(pStream.GetByte(baseOffset))
        << std::endl;
        outStream << "      General Name: ";
        ++baseOffset;
        for (std::uint8_t m = 0; m < 31; ++m) {
    ...
    std::uint16_t lastByte = pStream.GetUShort(pStream.GetSize() - 2);
    outStream << "Flags          : " << std::hex << static_cast<std::uint32_t>(lastByte) << std::dec
    << std::endl;
    if ((lastByte & 0x02) == 0x02) {
...
CPCharAndStartLoc::CPCharAndStartLoc() { InternalReset(); }
CPCharAndStartLoc::CPCharAndStartLoc(AccountEntry &actbBlock, [[maybe_unused]] std::uint8_t numCharacters,
                                     std::uint8_t numLocations, CSocket *mSock) {
    InternalReset();
    
    std::uint8_t noLoopBytes = 9;
    if (mSock->clientType() == ClientType::SA3D || mSock->clientType() == ClientType::HS3D) {
        noLoopBytes = 11;
    }
    ...
    if (mSock->clientType() == ClientType::SA3D || mSock->clientType() == ClientType::HS3D) { // maybe 7.0.13+ too?
...
void CPCharAndStartLoc::CopyData(AccountEntry &toCopy) {
    ...
    for (std::uint8_t i = 0; i < pStream.GetByte(3); ++i) {
        baseOffset = static_cast<std::uint16_t>(4 + (i * 60));
...
void CPCharAndStartLoc::AddCharacter(CChar *toAdd, std::uint8_t charOffset) {
    std::uint16_t baseOffset = 0;
    baseOffset = static_cast<std::uint16_t>(4 + (charOffset * 60));
...
void CPCharAndStartLoc::NumberOfLocations(std::uint8_t numLocations, [[maybe_unused]] CSocket *mSock) {
    std::uint16_t byteOffset = 0;
    byteOffset = static_cast<std::uint16_t>(4 + (pStream.GetByte(3) * 60));
...
//| Function	-	CPKAccept()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to acknowledge login of custom (Krrios')
// client
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
CPKAccept::CPKAccept(std::uint8_t Response) {
    pStream.ReserveSize(5);
    pStream.WriteByte(0, 0xF0);
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to display tips/notice window with message
//on login
// o------------------------------------------------------------------------------------------------o
...
CPUpdScroll::CPUpdScroll() { InternalReset(); }
CPUpdScroll::CPUpdScroll(std::uint8_t tType) {
    InternalReset();
    TipType(tType);
}
CPUpdScroll::CPUpdScroll(std::uint8_t tType, std::uint8_t tNum) {
    InternalReset();
    TipType(tType);
    TipNumber(tNum);
}
...
void CPUpdScroll::SetLength(std::uint16_t len) {
...
//|							One 	 Darken
//|							Two 	 Lighten
//|							Three 	 Transparent to dark colors to
//emphasize the bright colors
//|							Four 	 Semi-transparent (high
// transparency) |							Five 	 Translucent (near
// the present) |							Six 	 "Negapoji" (???)
...
CPGraphicalEffect2::CPGraphicalEffect2(std::uint8_t effectType) : CPGraphicalEffect(effectType) {
    InternalReset();
}
CPGraphicalEffect2::CPGraphicalEffect2(std::uint8_t effectType, CBaseObject &src, CBaseObject &trg)
: CPGraphicalEffect(effectType, src, trg) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
    TargetSerial(trg);
}
CPGraphicalEffect2::CPGraphicalEffect2(std::uint8_t effectType, CBaseObject &src)
: CPGraphicalEffect(effectType, src) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
}
void CPGraphicalEffect2::Hue(std::uint32_t hue) { pStream.WriteLong(28, hue); }
void CPGraphicalEffect2::RenderMode(std::uint32_t mode) { pStream.WriteLong(32, mode); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CP3DGraphicalEffect()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to display 3D graphical/particle effect in
// client
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE[2] additional effect # that's only used for
//moving effects, 0 otherwise
//|						BYTE[4] if target is item (type 2) that's itemId, 0
//otherwise (EDIT: Packet logs indiciate this is target serial?) |
//BYTE[1] layer (of the character, e.g left hand, right hand,
//... 0-4, 0xff: moving effect or target is no char)
...
CP3DGraphicalEffect::CP3DGraphicalEffect(std::uint8_t effectType) : CPGraphicalEffect2(effectType) {
    InternalReset();
}
CP3DGraphicalEffect::CP3DGraphicalEffect(std::uint8_t effectType, CBaseObject &src, CBaseObject &trg)
: CPGraphicalEffect2(effectType, src, trg) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
    TargetSerial(trg);
}
CP3DGraphicalEffect::CP3DGraphicalEffect(std::uint8_t effectType, CBaseObject &src)
: CPGraphicalEffect2(effectType, src) {
    InternalReset();
    Effect(effectType);
    SourceSerial(src);
}
void CP3DGraphicalEffect::EffectId3D(std::uint16_t effectId3D) { pStream.WriteShort(36, effectId3D); }
void CP3DGraphicalEffect::ExplodeEffectId(std::uint16_t explodeEffectId) {
...
void CP3DGraphicalEffect::Unknown(std::uint16_t unknown) { pStream.WriteShort(47, unknown); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CPMapRelated()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet responding to client requests for map
// actions
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE map action
//|							1 = add map point,
//|							2 = add new pin with pin number. (insertion. other
//pins after the number are pushed back.) |							3 =
// change pin |							4 = remove pin
//|							5 = remove all pins on the map
//|							6 = toggle the 'editable' state of the map.
//|							7 = return msg from the server to the request 6 of
//the client.
//|						BYTE plotting state (1=on, 0=off, valid only if
//command 7)
//|						BYTE[2] x location (relative to upper left corner of the
//map, in pixels, for points)
//|						BYTE[2] y location (relative to upper left corner of the
//map, in pixels, for points)
// o------------------------------------------------------------------------------------------------o
...
void CPMapRelated::PlotState(std::uint8_t pState) { pStream.WriteByte(6, pState); }
void CPMapRelated::Location(std::int16_t x, std::int16_t y) {
    pStream.WriteShort(7, x);
    pStream.WriteShort(9, y);
}
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with instructions for client to draw an
// object
// o------------------------------------------------------------------------------------------------o
...
//|							DONE
//|						else loop this until above if statement is
// satisified |							BYTE[4] item serial |
// BYTE[2] model (item hex # - only 15 lsb) |							BYTE
// layer |							if (model & 0x8000) |
// BYTE[2] hue
// o------------------------------------------------------------------------------------------------o
//|					Notes
//|						Status Options
...
// o------------------------------------------------------------------------------------------------o
CPDrawObject::CPDrawObject() { InternalReset(); }
...
void CPDrawObject::SetLength(std::uint16_t len) {
    pStream.ReserveSize(len);
    pStream.WriteShort(1, len);
    curLen = len;
}
...
void CPDrawObject::AddItem(CItem *toAdd, bool alwaysAddItemHue) {
    ...
    pStream.WriteShort(cPos += 4, static_cast<std::int32_t>(toAdd->GetId()));
    pStream.WriteByte(cPos += 2, toAdd->GetLayer());
    
    if (alwaysAddItemHue) {
        // Always send color to clients 7.0.33.1 and above
        pStream.WriteShort(cPos += 1, static_cast<std::int32_t>(toAdd->GetColour()));
        SetLength(curLen + 9);
    }
    else {
        ...
        if (bColour) {
            ...
            pStream.WriteShort(++cPos, static_cast<std::int32_t>(toAdd->GetColour()));
        }
        else {
            SetLength(curLen + 7);
        }
...
void CPDrawObject::SetRepFlag(std::uint8_t value) { pStream.WriteByte(18, value); }
...
void CPDrawObject::CopyData(CChar &mChar) {
    ...
    pStream.WriteShort(15, static_cast<std::int32_t>(mChar.GetSkin()));
    ...
    if (ServerConfig::shared().enableClients.enableClient7000() ||  ServerConfig::shared().enableClients.enableClient7090() || ServerConfig::shared().enableClients.enableClient70160() || ServerConfig::shared().enableClients.enableClient70240() || ServerConfig::shared().enableClients.enableClient70300() || ServerConfig::shared().enableClients.enableClient70331() || ServerConfig::shared().enableClients.enableClient704565() || ServerConfig::shared().enableClients.enableClient70610() ) {
        ...
        // Clients below 7.0.0.0
        const std::uint8_t BIT__INVUL = 0;  //	0x01, invulnerable
        const std::uint8_t BIT__DEAD = 1;   //	0x02, dead
        const std::uint8_t BIT__POISON = 2; //	0x04, poison
        const std::uint8_t BIT__GOLDEN = 3; //	0x08, yellow healthbar
        // const std::uint8_t BIT__IGNOREMOBILES = 4;	// 0x10, ignore other mobiles?
    ...
    const std::uint8_t BIT__ATWAR = 6; //	0x40
    const std::uint8_t BIT__OTHER = 7; //	0x80
    flag.set(BIT__ATWAR, mChar.IsAtWar());
    flag.set(BIT__OTHER,
             ((!mChar.IsNpc() && !IsOnline(mChar)) || (mChar.GetVisible() != VT_VISIBLE) ||
              (mChar.IsDead() && !mChar.IsAtWar())));
    
    pStream.WriteByte(17, static_cast<std::uint8_t>(flag.to_ulong()));
}
...
CPCorpseClothing::CPCorpseClothing() { InternalReset(); }
...
void CPCorpseClothing::NumberOfItems(std::uint16_t numItems) {
    // knowing the number of items, set the packet size
    std::uint16_t packetSize = static_cast<std::uint16_t>((numItems * 5) + 8); // 7 for lead in, 1 for lead out
    pStream.ReserveSize(packetSize);
    pStream.WriteShort(1, packetSize);
    pStream.WriteByte(packetSize - 1, 0x00);
}
void CPCorpseClothing::AddItem(CItem *toAdd, std::uint16_t itemNum) {
    std::uint16_t baseOffset = static_cast<std::uint16_t>(7 + itemNum * 5);
...
//|						Byte DataType // 0x00 = Item , 0x01 = Character, 0x02
//= Multi |						Byte[4] Serial
//|						Byte[2] Object ID // for multi its same value as the
//multi has in multi.mul |						Byte Offset/Facing // 0x00
// if Multi |						Byte[2] Amount // 0x1 if Multi |
//Byte[2] Amount // 0x1 if Multi , no idea why Amount is sent 2 times |
// Byte[2] X | Byte[2] Y |						Byte Z
//|						Byte Layer // 0x00 if Multi / Light Level
//(TileData.Quality
// or 0 for Mobiles) |						Byte[2] Color // 0x00 if Multi |
// Byte Flag // 0x20 = Movable if normally not , 0x80 = Hidden , 0x00 if Multi
//|						IF ITEM
//|							Byte[2] Access (for items only, 0x01 = Player
//Item, 0x00 = World Item)
//|
//|						Notes
...
void CPNewObjectInfo::CopyItemData(CItem &mItem, CChar &mChar) {
    bool isInvisible = (mItem.GetVisible() != VT_VISIBLE ||
                        (mItem.GetVisible() == VT_TEMPHIDDEN && &mChar != mItem.GetOwnerObj()));
    bool isMovable = (mItem.GetMovable() == 1 || mChar.AllMove() ||
                      (mItem.IsLockedDown() && &mChar == mItem.GetOwnerObj()));
    ...
    if (mChar.GetSocket()->clientType() >= ClientType::HS2D) {
        pStream.ReserveSize(26);
...
void CPNewObjectInfo::CopyMultiData(CMultiObj &mMulti, CChar &mChar) {
    ...
    std::int32_t itemId = mMulti.GetId();
    itemId &= 0x3FFF;
    // itemId += 0x4000; // Old POL server forum post mentioned newer clients changing how they want
    // the multiID sent. Adding this to the ID (so a small house goes from 0x4064 to 0x8064) still
    // works, but so does the old value. Why the change, and what does it meean? When is it needed?
    ...
    pStream.WriteShort(
                       21, mMulti.GetColour()); // Central mast on classic boats can have a color, apparently!
    
    // Flags
    pStream.WriteByte(23, 0x00);
    
    if (mChar.GetSocket()->clientType() >= ClientType::HS2D) // required for boats to work properly?
    {
        pStream.ReserveSize(26);
        pStream.WriteShort(24, 0x00); // HS requires 2 extra bytes
    }
...
CPNewObjectInfo::CPNewObjectInfo() { InternalReset(); }
CPNewObjectInfo::CPNewObjectInfo(CItem &mItem, CChar &mChar) {
    InternalReset();
    CopyData(mItem, mChar);
}

void CPNewObjectInfo::Objects(CItem &mItem, CChar &mChar) { CopyData(mItem, mChar); }
...
//|						0x10 - FactionShip // unsure why client needs to
// know |						0x20 - Movable if normally not
//|						0x40 - War mode
//|						0x80 - Hidden
// o------------------------------------------------------------------------------------------------o
...
CPObjectInfo::CPObjectInfo() { InternalReset(); }
CPObjectInfo::CPObjectInfo(CItem &mItem, CChar &mChar) {
    InternalReset();
    CopyData(mItem, mChar);
}

void CPObjectInfo::Objects(CItem &mItem, CChar &mChar) { CopyData(mItem, mChar); }
...
//|						0x0F - Command Prompts
// o------------------------------------------------------------------------------------------------o
void CPUnicodeSpeech::Object(CBaseObject &talking) { CopyData(talking); }
...
void CPUnicodeSpeech::Name(std::string value) { pStream.WriteString(18, value, 30); }
void CPUnicodeSpeech::Message(const char *value) {
    size_t length = strlen(value);
    SetLength(static_cast<std::uint16_t>(48 + (2 * length) + 2));
    for (size_t i = 0; i < length; ++i) {
        pStream.WriteByte(48 + i * 2, value[i]);
    }
}
void CPUnicodeSpeech::SetLength(std::uint16_t value) {
...
CPUnicodeSpeech::CPUnicodeSpeech() { InternalReset(); }
...
void CPUnicodeSpeech::CopyData(CPITalkRequestUnicode &talking) {
    ...
    std::uint16_t length = talking.Length();
...
//|						0x0F - Command Prompts
// o------------------------------------------------------------------------------------------------o
CPUnicodeMessage::CPUnicodeMessage() { InternalReset(); }
...
void CPUnicodeMessage::Name(std::string value) { pStream.WriteString(18, value, 30); }
...
void CPUnicodeMessage::Message(const std::string msg) {
    ...
    if (wMsg != L"") {
        ...
        const std::uint16_t packetLen = static_cast<std::uint16_t>(48 + (stringLen * 2) + 2);
        SetLength(packetLen);
...
void CPUnicodeMessage::SetLength(std::uint16_t value) {
    pStream.ReserveSize(value);
    pStream.WriteShort(1, value);
}
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with gameserver list to display in client
// during login
// o------------------------------------------------------------------------------------------------o
...
//|							BYTE[4] pingIP
//|
//|					Note: System Info Flags: 0xCC - Do not send video card info. 0x64
//- Send Video card.
...
CPGameServerList::CPGameServerList() { InternalReset(); }
CPGameServerList::CPGameServerList(std::uint16_t numServers) {
    InternalReset();
    NumberOfServers(numServers);
}
void CPGameServerList::NumberOfServers(std::uint16_t numItems) {
    std::uint32_t tlen = 6 + (numItems * 40);
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with server response to secure trading
// request
// o------------------------------------------------------------------------------------------------o
...
CPSecureTrading::CPSecureTrading() { InternalReset(); }
...
void CPSecureTrading::Action(std::uint8_t value) { pStream.WriteByte(3, value); }
...
//|
//|						NOTE: Only 3D clients (used to) send this packet. Now both
//2D and 3D do! |						Server and client packet.
//|
//|						Client asks for name of object with ID x.
//|						Server has to reply with ID + name
//|						Client automatically knows names of items.
//|						Hence it only asks only for NPC/Player names nearby,
//but shows bars of items plus NPC’s.
//|
...
CPAllNames3D::CPAllNames3D() { InternalReset(); }
CPAllNames3D::CPAllNames3D(CBaseObject &obj) {
    InternalReset();
    CopyData(obj);
}
void CPAllNames3D::Object(CBaseObject &obj) { CopyData(obj); }

// o------------------------------------------------------------------------------------------------o
//| Function	-	CPBookPage()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with server response to request for book
// pages
// o------------------------------------------------------------------------------------------------o
...
//|						Note:
//|						server side: # of pages equals value given in
// 0x93/0xd4
//|						EACH page # given. If empty: # lines: 0 + terminator
//(=3 0’s)
//|						client side:  # of pages always 1. if 2 pages changed,
//client generates 2 packets.
// o------------------------------------------------------------------------------------------------o
void CPBookPage::IncLength(std::uint8_t amount) {
    bookLength += amount;
...
void CPBookPage::CopyData(CItem &obj) { Serial(obj.GetSerial()); }
CPBookPage::CPBookPage() { InternalReset(); }
CPBookPage::CPBookPage(CItem &obj) {
    InternalReset();
    CopyData(obj);
}
void CPBookPage::Object(CItem &obj) { CopyData(obj); }
void CPBookPage::NewPage(std::int16_t pNum) {
    ++pageCount; // 1 based counter
    std::uint16_t baseOffset = bookLength;
    IncLength(4);
    if (pNum == -1) {
...
void CPBookPage::AddLine(const std::string &line) {
    ...
    IncLength(static_cast<std::uint8_t>(strLen));
    pStream.WriteString(baseOffset, line, line.length());
}
// Why do we use pointersfor collections. References are wonder things
void CPBookPage::NewPage(std::int16_t pNum, const std::vector<std::string> *lines) {
    ++pageCount; // 1 based counter
    std::uint16_t baseOffset = bookLength;
    IncLength(4);
    if (pNum == -1) {
    ...
    pStream.WriteByte(static_cast<size_t>(baseOffset + 3),
                      static_cast<std::uint8_t>(lines->size())); // 8 lines per page
...
//|						BYTE[2] numTextLines
//|						BYTE[2] text length (in unicode (2 byte)
// characters.) |						BYTE[?] text (in unicode)
// o------------------------------------------------------------------------------------------------o
...
void CPSendGumpMenu::Finalize() {
    ...
    pStream.WriteShort(19, static_cast<std::uint16_t>(length - 20));
    
    std::uint32_t tlOff = length + 1;
    length += 3;
    std::uint32_t tlines = 0;
    ...
    for (auto &entry : text) {
        if (!entry.empty()) {
            ...
            for (std::uint16_t i = 0; i < lineLen; ++i) {
                pStream.WriteByte(static_cast<size_t>(length) + 3 + static_cast<size_t>(i) * 2,
                                  entry[i]);
            }
            length += increment;
            ++tlines;
    ...
    pStream.WriteShort(1, static_cast<std::uint16_t>(length));
...
CPNewSpellBook::CPNewSpellBook() { InternalReset(); }
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to show damage numbers above head of
// characters
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Packet: 0xBF (General Information Packet)
...
void CPDisplayDamage::CopyData(CChar &ourTarg, std::uint16_t ourDamage) {
    pStream.WriteLong(6, ourTarg.GetSerial());
    pStream.WriteByte(10, (ourDamage >> 8) + (ourDamage % 256));
}
CPDisplayDamage::CPDisplayDamage() { InternalReset(); }
CPDisplayDamage::CPDisplayDamage(CChar &ourTarg, std::uint16_t ourDamage) {
    InternalReset();
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with response to client request for item
// tooltip
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Packet: 0xBF (General Information Packet)
...
void CPQueryToolTip::CopyData(CBaseObject &mObj) { pStream.WriteLong(5, mObj.GetSerial()); }
CPQueryToolTip::CPQueryToolTip() { InternalReset(); }
...
//| Function	-	CPToolTip()
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with response to AoS client request for item
// tooltip
// o------------------------------------------------------------------------------------------------o
...
//|							BYTE[2] text length
//|							BYTE[text length] little endian Unicode text, not
        ...
        if (cItem.GetAmount() > 1 && !cItem.IsCorpse() && addAmount &&
            ...
            if (ServerConfig::shared().enabled(ServerSwitch::RANKSYSTEM) && cItem.GetRank() == 10) {
                ...
                if (cItem.IsCorpse()) {
                    ...
                    tempEntry.ourText = util::format(" \t<color=%s>%s</color>\t ",
                                                     iFlagColor.c_str(), cItemName.c_str());
                }
                else {
    ...
    if (ServerConfig::shared().enabled(ServerSwitch::DISPLAYGUILDTOOLTIP)) {
        if (!mChar.IsIncognito() && !mChar.IsDisguised()) {
            ...
            if (myGuild != nullptr) {
                ...
                if (guildTitle != "") {
                    tempEntry.ourText =
                    util::format("%s, %s", guildTitle.c_str(), myGuild->Name().c_str());
                }
                else {
...
void CPToolTip::CopyData(serial_t objSer, bool addAmount, bool playerVendor) {
    ...
    pStream.ReserveSize(packetLen);
    pStream.WriteShort(1, static_cast<std::int32_t>(packetLen));
    ...
    for (size_t i = 0; i < ourEntries.size(); ++i) {
        ...
        modifier += 4;
        pStream.WriteShort(modifier, static_cast<std::int32_t>(stringLen));
        modifier += 1;
...
auto CPSellList::CopyData(CChar &mChar, CChar &vendorId) -> void {
    ...
    pStream.WriteShort(1, static_cast<std::uint16_t>(packetLen));
    pStream.WriteLong(3, vendorId.GetSerial());
    pStream.WriteShort(7, numItems);
}

auto CPSellList::AddContainer(CTownRegion *tReg, CItem *spItem, CItem *ourPack, size_t &packetLen)
-> void {
...
void CPSellList::AddItem(CTownRegion *tReg, CItem *spItem, CItem *opItem, size_t &packetLen) {
    ...
    pStream.WriteShort(packetLen + 12, static_cast<std::int32_t>(stringLen));
    pStream.WriteString(packetLen + 14, itemname, stringLen);
    packetLen = newLen;
}

CPSellList::CPSellList() { InternalReset(); }
...
//|							BYTE[4] Board serial
//|							BYTE[22] board name (default is “bulletin board”,
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to open a bulletin board msg and/or msg
// summary
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Packet: 0x71 (Bulletin Board Message)
//|
...
//|							BYTE posterLen
//|							BYTE[posterLen] poster (null terminated
// string) |							BYTE subjectLen |
// BYTE[subjectLen] subject (null terminated string)
//|							BYTE timeLen
//|							BYTE[timeLen] time (null terminated string with time
//of posting) (“Day 1 @ 11:28”)
// o------------------------------------------------------------------------------------------------o
...
//|							BYTE timeLen
//|							BYTE[timeLen] time (null terminated string with time
//of posting) (“Day 1 @ 11:28”) |							BYTE[5]
//Unknown (01 90 03 F7 00) |							BYTE numlines |
...
void CPOpenMsgBoardPost::CopyData(CSocket *mSock, const MsgBoardPost_st &mbPost) {
    size_t totSize = 19 + static_cast<size_t>(mbPost.dateLen) +
    static_cast<size_t>(mbPost.posterLen) + static_cast<size_t>(mbPost.subjectLen);
    ...
    else if (bFullPost) // full post
    {
        ...
        totSize += 1;
        pStream.ReserveSize(totSize);
        pStream.WriteShort(1, static_cast<std::uint16_t>(totSize));      // packet size
        pStream.WriteLong(4, mSock->GetDWord(4));               // board serial
        pStream.WriteLong(8, (mbPost.serial + BASEITEMSERIAL)); // message serial
        size_t offset = 12;
        ...
        if (mChar->IsGM() && modboards.m_IntValue == 1) {
            ...
            pStream.WriteByte(offset, static_cast<std::uint8_t>(mChar->GetName().length() + 1));
        ...
        for (pIter = mbPost.msgBoardLine.begin(); pIter != mbPost.msgBoardLine.end(); ++pIter) {
            pStream.WriteByte(++offset, static_cast<std::uint8_t>((*pIter).size() + 2));
...
CPOpenMsgBoardPost::CPOpenMsgBoardPost(CSocket *mSock, const MsgBoardPost_st &mbPost,
                                       bool fullPost) {
    bFullPost = fullPost;
    InternalReset();
...
//|						Subcommand details
//|							BYTE subsubcommand (0x2 for 2D client, 0x5 for
//KR client) |							BYTE[4] serial
//|							BYTE unknown // always 0 ?
//|							BYTE lockBits
//|								Bits: XXSS DDII
//|									s=strength, d=dex, i=int
//|									0 = up, 1 = down, 2 = locked, FF =
//update mobile status animation (KR only)
//|							if subsubcommand is 0x5
//|								If Lock flags = 0xFF //Update mobile
//status animation
//|									BYTE[1] Status // Unveryfied if lock
//flags
//== FF the locks will be handled here
//|									BYTE[1] unknown (0x00)
//|									BYTE[1] Animation
//|									BYTE[1] unknown (0x00)
//|									BYTE[1] Frame
...
// o------------------------------------------------------------------------------------------------o
CPExtendedStats::CPExtendedStats() { InternalReset(); }
...
void CPExtendedStats::CopyData(CChar &mChar) {
    ...
    const std::uint8_t strength = static_cast<std::uint8_t>((mChar.GetSkillLock(STRENGTH) & 0x3) << 4);
    const std::uint8_t dexterity = static_cast<std::uint8_t>((mChar.GetSkillLock(DEXTERITY) & 0x3) << 2);
    const std::uint8_t intelligence = static_cast<std::uint8_t>(mChar.GetSkillLock(INTELLECT) & 0x3);
...
//|						BYTE[2] Packet Size
//|						BYTE[4] Serial
//|						BYTE[2] Extended (1 if status bar is green or yellow,
//else 0)
//|						BYTE[2] Status Color (0x01 = Green, 0x02 = Yellow, sends
//only if extended == 1)
//|						BYTE Flag (0x00 = Remove Status Color, 0x01 = Enable
//Status Color, sends only if extended == 1)
//|						If mobile is poisoned, flag value > 0x00 - poison
// level.
//|						Since 4.0.7.0/7.0.7.0, sends to both 2d and 3d but works
//only in 3d. Server |							sends it as response for
// 0x34 Mobile Status Query.
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE[2] 0x01
//|						BYTE[2] Status Color (0x01 = Green, 0x02 = Yellow, 0x03
//= Red)
//|						BYTE Flag (0x00 = Remove Status Color, 0x01 = Enable
//Status
// Color) |						If mobile is poisoned, flag value > 0x00 -
// poison level
// o------------------------------------------------------------------------------------------------o
CPHealthBarStatus::CPHealthBarStatus() { InternalReset(); }

CPHealthBarStatus::CPHealthBarStatus(CChar &mChar, CSocket &tSock, std::uint8_t healthBarColor) {
    InternalReset();
...
void CPHealthBarStatus::SetHBStatusData(CChar &mChar, CSocket &tSock, std::uint8_t healthBarColor) {
    if (tSock.clientType() == ClientType::UO3D || tSock.clientType() == ClientType::KR3D ||
        tSock.clientType() == ClientType::SA3D || tSock.clientType() == ClientType::HS3D) {
...
//|								BYTE[4] Number of static patches in
//this map
//|
...
void CPEnableMapDiffs::CopyData() {
    ...
    pStream.WriteShort(1, static_cast<std::int32_t>(pSize));
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet to respond to client request to change book
// title
// o------------------------------------------------------------------------------------------------o
...
//|						BYTE[2] length of author string (0 terminator
// included) |						BYTE[?] author (ASCII, 0 terminated) |
// BYTE[2] length of title string (0 terminator included) |
// BYTE[?] title string
//|
//|					Note1:
//|						server and client packet.
//|							server side: opening book writeable: flag1+flag2
//both
// 1
//|								(opening readonly book: flag1+flag2 0,
//unverified though)
...
//|							server sends 0x66 with all book data "beyond" title
//+ author
//|							if title + author changed: client side 0xd4
// send
//|							if other book pages changed: 0x66 client
//side send.
// o------------------------------------------------------------------------------------------------o
...
void CPNewBookHeader::Serial(std::uint32_t bookSer) { pStream.WriteLong(3, bookSer); }

void CPNewBookHeader::Flag1(std::uint8_t toSet) { pStream.WriteByte(7, toSet); }

void CPNewBookHeader::Flag2(std::uint8_t toSet) { pStream.WriteByte(8, toSet); }

void CPNewBookHeader::Pages(std::uint16_t numPages) { pStream.WriteShort(9, numPages); }

void CPNewBookHeader::Author(const std::string &newAuthor) { author = newAuthor; }
...
//|								Loop for each entry:
//|									BYTE[2] Entry Tag (this will be returned
//by the client on selection)
//|									BYTE[2] Cliloc ID (file number
//for intloc#.language e.g intloc6.enu and the index into that) | BYTE[2] Flags 0x00=enabled,
// 0x01=disabled, 0x02=arrow, 0x20 = color
//|									If (Flags &0x20)
//|										BYTE[2] color
//|											rgb 1555 color (ex, 0
//= transparent, 0x8000 = solid black,
//|											0x1F = blue, 0x3E0 =
//green,
// 0x7C00 = red) |							if subsubcommand is 0x02: |
// Loop for each entry:
//|									BYTE[4] Cliloc ID
//|									BYTE[2] Index of entry
//(entry tag)? |									BYTE[2]
...
void CPPopupMenu::CopyData(CChar &toCopy, CSocket &tSock) {
    ...
    if (toCopy.GetNpcWander() != WT_PATHFIND && toCopy.GetNpcWander() != WT_FOLLOW &&
        toCopy.GetNpcWander() != WT_FLEE) {
    ...
    // Open Backpack
    //	|| ( toCopy.IsTamed() && ValidateObject( toCopy.GetOwnerObj() ) && toCopy.GetOwnerObj() ==
    // mChar && !toCopy.CanBeHired() )))
    if (toCopy.GetQuestType() != QT_ESCORTQUEST &&
        ((toCopy.GetSerial() == tSock.CurrcharObj()->GetSerial() || toCopy.GetId() == 0x0123 ||
          toCopy.GetId() == 0x0124 || toCopy.GetId() == 0x0317) &&
         ValidateObject(toCopy.GetPackItem()))) {
        if (numEntries > 0) {
            offset += 2;
        }
    ...
    if (toCopy.IsNpc() && !toCopy.IsTamed() &&
        ...
        for (std::uint8_t i = 0; i < ALLSKILLS; ++i) {
            ...
            if (trainerSkillPoints > 600 && (mChar->GetSkill(i) < trainerSkillPoints / 3)) {
                ...
                switch (i) {
                    case ANATOMY:
                        pStream.WriteShort(offset, 0x0154);
                        break;
                    case PARRYING:
                        pStream.WriteShort(offset, 0x006d);
                        break;
                    case HEALING:
                        pStream.WriteShort(offset, 0x006e);
                        break;
                    case HIDING:
                        pStream.WriteShort(offset, 0x006f);
                        break;
                    case STEALING:
                        pStream.WriteShort(offset, 0x0070);
                        break;
                    case ALCHEMY:
                        pStream.WriteShort(offset, 0x0071);
                        break;
                    case ANIMALLORE:
                        pStream.WriteShort(offset, 0x0072);
                        break;
                    case ITEMID:
                        pStream.WriteShort(offset, 0x0073);
                        break;
                    case ARMSLORE:
                        pStream.WriteShort(offset, 0x0074);
                        break;
                    case BEGGING:
                        pStream.WriteShort(offset, 0x0075);
                        break;
                    case BLACKSMITHING:
                        pStream.WriteShort(offset, 0x0076);
                        break;
                    case BOWCRAFT:
                        pStream.WriteShort(offset, 0x0077);
                        break;
                    case PEACEMAKING:
                        pStream.WriteShort(offset, 0x0078);
                        break;
                    case CAMPING:
                        pStream.WriteShort(offset, 0x0079);
                        break;
                    case CARPENTRY:
                        pStream.WriteShort(offset, 0x007a);
                        break;
                    case CARTOGRAPHY:
                        pStream.WriteShort(offset, 0x007b);
                        break;
                    case COOKING:
                        pStream.WriteShort(offset, 0x007c);
                        break;
                    case DETECTINGHIDDEN:
                        pStream.WriteShort(offset, 0x007d);
                        break;
                    case ENTICEMENT:
                        pStream.WriteShort(offset, 0x007e);
                        break;
                    case EVALUATINGINTEL:
                        pStream.WriteShort(offset, 0x007f);
                        break;
                    case FISHING:
                        pStream.WriteShort(offset, 0x0080);
                        break;
                    case PROVOCATION:
                        pStream.WriteShort(offset, 0x0081);
                        break;
                    case LOCKPICKING:
                        pStream.WriteShort(offset, 0x0082);
                        break;
                    case MAGERY:
                        pStream.WriteShort(offset, 0x0083);
                        break;
                    case MAGICRESISTANCE:
                        pStream.WriteShort(offset, 0x0084);
                        break;
                    case TACTICS:
                        pStream.WriteShort(offset, 0x0085);
                        break;
                    case SNOOPING:
                        pStream.WriteShort(offset, 0x0086);
                        break;
                    case REMOVETRAP:
                        pStream.WriteShort(offset, 0x0087);
                        break;
                    case MUSICIANSHIP:
                        pStream.WriteShort(offset, 0x0088);
                        break;
                    case POISONING:
                        pStream.WriteShort(offset, 0x0089);
                        break;
                    case ARCHERY:
                        pStream.WriteShort(offset, 0x008a);
                        break;
                    case SPIRITSPEAK:
                        pStream.WriteShort(offset, 0x008b);
                        break;
                    case TAILORING:
                        pStream.WriteShort(offset, 0x008c);
                        break;
                    case TAMING:
                        pStream.WriteShort(offset, 0x008d);
                        break;
                    case TASTEID:
                        pStream.WriteShort(offset, 0x008e);
                        break;
                    case TINKERING:
                        pStream.WriteShort(offset, 0x008f);
                        break;
                    case VETERINARY:
                        pStream.WriteShort(offset, 0x0090);
                        break;
                    case FORENSICS:
                        pStream.WriteShort(offset, 0x0091);
                        break;
                    case HERDING:
                        pStream.WriteShort(offset, 0x0092);
                        break;
                    case TRACKING:
                        pStream.WriteShort(offset, 0x0093);
                        break;
                    case STEALTH:
                        pStream.WriteShort(offset, 0x0094);
                        break;
                    case INSCRIPTION:
                        pStream.WriteShort(offset, 0x0095);
                        break;
                    case SWORDSMANSHIP:
                        pStream.WriteShort(offset, 0x0096);
                        break;
                    case MACEFIGHTING:
                        pStream.WriteShort(offset, 0x0097);
                        break;
                    case FENCING:
                        pStream.WriteShort(offset, 0x0098);
                        break;
                    case WRESTLING:
                        pStream.WriteShort(offset, 0x0099);
                        break;
                    case LUMBERJACKING:
                        pStream.WriteShort(offset, 0x009a);
                        break;
                    case MINING:
                        pStream.WriteShort(offset, 0x009b);
                        break;
                    case MEDITATION:
                        pStream.WriteShort(offset, 0x009c);
                        break;
                    case NECROMANCY:
                        pStream.WriteShort(offset, 0x017c);
                        break;
                    case CHIVALRY:
                        pStream.WriteShort(offset, 0x017d);
                        break;
                    case FOCUS:
                        pStream.WriteShort(offset, 0x017e);
                        break;
                    default:
                        break;
                }
...
//|							Message Type
//|								0x00 = Regular, 0x01 = System, 0x02 = Emote,
//0x06 = Label, |								0x07 = Focus, 0x08
//= Whisper, 0x09 = Yell, 0x0A = Spell, |
//0x0D = Guild, 0x0E = Alliance, 0x0F = GM Request, 0x10 = GM Response, |
//0x20 = Special, 0xC0 = Encoded |						BYTE[2] hue |
//BYTE[2] font |						BYTE[4] Message number |
//BYTE[30] - speaker's name
//|						BYTE[?*2] - arguments // _little-endian_ unicode string,
//tabs
//('\t') seperate the arguments |							Argument
// example: |							take number 1042762: |
//"Only ~1_AMOUNT~ gold could be deposited. A check for ~2_CHECK_AMOUNT~ gold was returned to you."
//|							the arguments string may have "100
...
void CPClilocMessage::Type(std::uint8_t toSet) { pStream.WriteByte(9, toSet); }

void CPClilocMessage::Hue(std::uint16_t hueColor) { pStream.WriteShort(10, hueColor); }

void CPClilocMessage::Font(std::uint16_t fontType) { pStream.WriteShort(12, fontType); }

void CPClilocMessage::Message(std::uint32_t messageNum) { pStream.WriteLong(14, messageNum); }
...
void CPClilocMessage::ArgumentString(const std::string &arguments) {
    ...
    const std::uint16_t packetLen = static_cast<std::uint16_t>(pStream.GetShort(1) + (stringLen * 2) + 2);
    pStream.ReserveSize(packetLen);
    pStream.WriteShort(1, packetLen);
...
// o------------------------------------------------------------------------------------------------o
//| Purpose		-	Handles outgoing packet with response to client request to add
//someone to party
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Packet: 0xBF (General Information Packet)
...
//|							BYTE number of Members (total number of members in
...
void CPPartyMemberList::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Packet ID        : " << std::hex << static_cast<std::uint16_t>(pStream.GetByte(0))
    << std::dec << std::endl;
    outStream << "Subcommand       : " << pStream.GetShort(3) << std::endl;
    outStream << "Party Sub        : " << static_cast<std::uint16_t>(pStream.GetByte(5)) << std::endl;
    outStream << "Member Count     : " << static_cast<std::uint16_t>(pStream.GetByte(6)) << std::endl;
    for (std::int32_t i = 0; i < pStream.GetByte(6); ++i) {
        outStream << "    Member #" << i << "    : " << std::hex << "0x"
...
void CPPartyMemberRemove::log(std::ostream &outStream, bool fullHeader) {
    ...
    outStream << "Party Sub        : " << static_cast<std::uint16_t>(pStream.GetByte(5)) << std::endl;
    outStream << "Member Count     : " << static_cast<std::uint16_t>(pStream.GetByte(6)) << std::endl;
    outStream << "Member Removed   : 0x" << std::hex << static_cast<std::uint16_t>(pStream.GetLong(7))
    << std::dec << std::endl;
...
//|							BYTE[4] id (of target, from client, of source,
//from server) |							BYTE[n][2] Null terminated
// Unicode message.
// o------------------------------------------------------------------------------------------------o
CPPartyTell::CPPartyTell(CPIPartyCommand *removed, CSocket *talker) {
    ...
    std::uint8_t tellMode = talkSocket->GetByte(5);
    pStream.WriteByte(5, tellMode);
    pStream.WriteLong(6, talker->CurrcharObj()->GetSerial());
    
    std::uint16_t sizeModifier = 6;
    if (tellMode == 3) {
        sizeModifier = 10;
    }
    
    std::uint16_t messageLength = talkSocket->GetWord(1) - sizeModifier;
...
void CPPartyTell::log(std::ostream &outStream, bool fullHeader) {
    ...
    for (size_t j = 10; j < pStream.GetSize(); ++j) {
        outStream << static_cast<std::uint8_t>(pStream.GetByte(j));
    }
    outStream << std::endl;
    outStream << "  Raw dump     :" << std::endl;
    CPUOXBuffer::log(outStream, false);
...
CPClientVersion::CPClientVersion() { InternalReset(); }
