...
void CMagic::LoadScript( void )
{
	...
	for( auto &spellScp : FileLookup->ScriptListings[spells_def] )
	{
		...
		for( const auto &[spEntry, SpellLoad]:spellScp->collection() )
		{
			...
			if( ssecs[0] == "SPELL" )
			{
				i = static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 ));
				if( i <= SPELL_MAX )
				{
					++spellCount;
					...
					//Console::shared().Log( "Spell number: %i", "spell.log", i ); // Disabled for performance reasons
					for( const auto &sec : SpellLoad->collection() )
					{
						...
						//Console::shared().Log( "Tag: %s\tData: %s", "spell.log", UTag.c_str(), data.c_str() ); // Disabled for performance reasons
						switch(( UTag.data()[0] ))
						{
							case 'A':
								if( UTag == "ACTION" )
							...
							case 'F':
								if( UTag == "FLAGS" )
								{
									...
									if( ssecs.size() > 1 )
									{
										...
										spells[i].Flags((( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 16 ))) << 8 ) |
												    static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 16 )));
									}
									else
									{
							...
							case 'M':
								...
								else if( UTag == "MOVEFX" )
								{
									...
									if( ssecs.size() > 1 )
									{
										...
										mv->Effect( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 16 )), static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 16 )));
										mv->Speed( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[2], "//" )), nullptr, 16 )));
										mv->Loop( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[3], "//" )), nullptr, 16 )));
										mv->Explode( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[4], "//" )), nullptr, 16 )));
									}
								}
								break;
							case 'P':
							...
							case 'S':
								...
								else if( UTag == "SOUNDFX" )
								{
									...
									if( ssecs.size() > 1 )
									{
										...
										spells[i].Effect(((static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 16 )) << 8 ) |
													 static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 16 ))));
									}
									else
									{
								...
								else if( UTag == "STATFX" )
								{
									...
									if( ssecs.size() > 1 )
									{
										...
										stat->Effect( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 16 )), static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 16 )));
										stat->Speed( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[2], "//" )), nullptr, 16 )));
										stat->Loop( static_cast<UI08>( std::stoul( util::trim( util::strip( ssecs[3], "//" )), nullptr, 16 )));
									}
								}
								else if( UTag == "SCLO" )
								{
