...
//======================================================================
auto AccountEntry::writeOrphanHeader(const std::filesystem::path &path) const ->const AccountEntry& {
    if (!std::filesystem::exists(path)){
        auto output = std::ofstream(path) ;
        if (output.is_open()){
            //output << "//OI3.0" << "-UV" << CVersionClass::GetVersion() << "-BD" << CVersionClass::GetBuild() << "-DS" << time( nullptr ) << "-ED" << CVersionClass::GetRealBuild() << "\n";
        ...
        if (input.gcount()>0){
            ...
            if (!line.empty()){
                ...
                if (key == "name"){
                    ...
                    this->firstLoginTime = static_cast<std::uint32_t>(std::stoul(value,nullptr,0)) ;
                }
                else if (key == "lastip"){
...
//====================================================================
auto AccountEntry::test(AttributeFlag flag) const ->bool{
    return this->flag.test(flag) ;
}
//====================================================================
auto AccountEntry::set(AttributeFlag flag, bool state) ->void {
    this->flag[flag] = state ;
    
}

//====================================================================
auto AccountEntry::addCharacter(account::charnum_t characterSlot,const AccountCharacter &accountCharacter) ->bool{
...
auto Account::writeImportHeader(const std::filesystem::path &path) const->const Account& {
    auto output = std::ofstream(path) ;
    if (output.is_open()){
        //output << "//AIMP3.0" << "-UV" << CVersionClass::GetVersion() << "-BD" << CVersionClass::GetBuild() << "-DS" << time( nullptr ) << "-ED" << CVersionClass::GetRealBuild() << std::endl;
        output << "version version,"<<UOXVersion::version<<":build,"<<UOXVersion::build<<":realbuild,"<< UOXVersion::realBuild <<"\n";
        output << "//------------------------------------------------------------------------------\n" ;
...
//======================================================================
auto Account::version(std::ostream &output) const -> const Account& {
...
auto Account::delAccount(const std::string &username) ->bool {
    auto lname = util::lower(username);
    auto iter = std::find_if(accounts.begin(),accounts.end(),[lname](const std::pair<account::acctnum_t,AccountEntry> &entry){
                            ...
                            if (!exists(values[0])){
                                ...
                                switch(values.size()){
                                    ...
                                    case 4:
                                        ...
                                        [[fallthrough]];
                                    case 3:
                                        account.flag = std::bitset<AccountEntry::AttributeFlag::COUNT>(std::stoul(values[2],nullptr,0));
                                        [[fallthrough]];
                                    case 2:
...
//======================================================================
auto Account::allAccounts() -> std::map<account::acctnum_t,AccountEntry>&{
    return accounts ;
}
