...
//|						1.0		20th December, 2001 Initial
//implementation
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds speech entry of specified type, font, color, etc to the speech
//queue | Notes		-	Copied that here from sefunctions.cpp. Default paramters weren't
//working !?
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
JSBool Packet(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
...
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_Free(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                    [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_WriteByte(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
...
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_WriteShort(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_WriteLong(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Writes variable number of bytes at specified position in the packet
//stream
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_WriteString(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 3) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CPacket_ReserveSize(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_Send(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool Gump(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc, [[maybe_unused]] jsval *argv,
            [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_Free(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                  [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGumpData_Free(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                      [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddCheckbox(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc < 5 || argc > 6) {
    ...
    gList->one->push_back(
        util::format("checkbox %i %i %i %i %i %i", tL, tR, gImage, gImageChk, initState, relay));
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds noclose element to gump stream; specifies that gump cannot be
//closed by |					clicking the right mouse button
// o------------------------------------------------------------------------------------------------o
JSBool CGump_NoClose(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds nomove element to gump stream; specifies that gump cannot be
//moved
// o------------------------------------------------------------------------------------------------o
JSBool CGump_NoMove(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds nodispose element to gump stream; specifies that gump cannot be
//disposed |					of/closed using the Escape button | Notes
//-	It's also possible it is used to prevent gumps from being closed |
//using the "Close Dialogs" client macro
//|					No response is sent to server upon closing the Gump in this
//manner.
// o------------------------------------------------------------------------------------------------o
JSBool CGump_NoDispose(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//|	Date		-	14th February, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds noresize element to gump stream; specifies that gump cannot be
//resized
// o------------------------------------------------------------------------------------------------o
JSBool CGump_NoResize(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Possible that the mastergump command itself only has any effect with
//client versions between 4.0.4d and 5.0.5b?
// o------------------------------------------------------------------------------------------------o
JSBool CGump_MasterGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
//|	Prototype	-	void AddBackground( topHeight, topLeft, bottomHeight, bottomLeft,
//backImage )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds background gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddBackground(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 5) {
...
//|	Prototype	-	void AddButton( topHeight, topLeft, buttonImage,
//(buttonPressedImage), unk1, unk2, unk3 )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds button gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddButton(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc < 6 || argc > 7) {
    ...
    gList->one->push_back(
        util::format("button %i %i %i %i %i %i %i", tL, tR, gImage, gImage2, x1, x2, x3));
...
//|								buttonPressedImage, unk1, unk2, unk3, tileId,
//hue, tileX, tileY )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds button gump with tileart to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddButtonTileArt(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                              [[maybe_unused]] jsval *rval) {
    if (argc != 11) {
    ...
    gList->one->push_back(util::format("buttontileart %i %i %i %i %i %i %i %i %i %i %i", tL, tR,
                                       tileIdNorm, tileIdPush, buttonType, pageNum, buttonId,
                                       tileId, hue, tileX, tileY));
...
//|	Prototype	-	void AddPageButton( topHeight, topLeft, buttonImage,
//(buttonPressedImage), pageNum )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds button gump for navigating gump pages to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddPageButton(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc < 4 || argc > 5) {
    ...
    gList->one->push_back(
        util::format("button %i %i %i %i 0 %i", tL, tR, gImage, gImage2, pageNum));
...
//|	Date		-	14th February, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds transparent area gump to gump stream, turns underlying elements
//transparent
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddCheckerTrans(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddCroppedText(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 6) {
        ScriptError(
            cx, "AddCroppedText: Invalid number of arguments (takes 6 x y hue width height text)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddGroup(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_EndGroup(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 3 && argc != 4) {
        ScriptError(cx, "AddGump: Invalid number of arguments (takes 3 or 4 - topLeft, topRight, "
                        "imageID and hue (optional))");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddGumpColor(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds cliloc-based tooltip to previous gump (Gump, Image, Button) in
//gump stream
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Additional arguments (up to 10) can be provided if the cliloc in
//question needs |					them. This could also be used to show custom
//tooltips in the client, if using a
//|					cliloc (like 1042971) that only contain an argument and no
//additional text!
//|
//|					For example, if you use myGump.AddToolTip( 1042971, "My Custom
//Text" ) in JS |					UOX3 will send the following gump command to
//add a tooltip element to previous
//|					gump element: "tooltip 1042971 @My Custom Text@"
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddToolTip(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 11) {
...
// o--------------------------------------------------------------------------------
// ---------------o |	Function	-	CGump_AddHTMLGump() |	Prototype	-	void
//AddHTMLGump( topHeight, topLeft, width, height, background, scrollbar, HTMLText) |	Date
//-	16th February, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a new HTML gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddHTMLGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 7) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddPage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddPicture(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 3) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a tile image gump to gump stream, with additional parameter for
//hue
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddPictureColor(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
...
//|	Prototype	-	void AddPicInPic( topLeft, topHeight, gumpImage, spriteX, spriteY,
//width, height )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a picinpic gump to the gump stream
//|	Notes		-	Requires client v7.0.80.0 or above
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddPicInPic(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 7) {
    ...
    gList->one->push_back(util::format("picinpic %i %i %i %i %i %i %i", x, y, gImage, spriteX,
                                       spriteY, width, height));
...
//|	Date		-	6th July, 2020
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Attaches item property element to previous element added to JS Gump,
//which |					causes client to display the known properties of an
//object based on the
//|					object serial that was provided as a regular tooltip on cursor.
//Could be used to
//|					show item stats for items on a custom paperdoll, for
//instance.
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddItemProperty(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddRadio(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc < 5 || argc > 6) {
    ...
    gList->one->push_back(
        util::format("radio %i %i %i %i %i %i", tL, tR, gImage, gImageChk, initialState, relay));
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddText(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
...
//|	Prototype	-	void AddTextEntry( topHeight, topLeft, width, height, hue, unk4,
//textID, defaultText )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a text entry gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddTextEntry(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 8) {
    ...
    gList->one->push_back(util::format("textentry %i %i %i %i %i %i %i", tL, tR, width, height, hue,
                                       relay, initialTextIndex));
...
//|	Prototype	-	void AddTextEntryLimited( topHeight, topLeft, width, height, hue,
//unk4, textID, defaultText, maxLength )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a text entry gump with maximum length to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddTextEntryLimited(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                 [[maybe_unused]] jsval *rval) {
    if (argc != 9) {
    ...
    gList->one->push_back(util::format("textentrylimited %i %i %i %i %i %i %i %i", tL, tR, width,
                                       height, hue, relay, initialTextIndex, textEntrySize));
    gList->two->push_back(test);
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddTiledGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 5) {
        ScriptError(cx,
                    "AddTiledGump: Invalid number of arguments (takes 5 x y width height gump)");
        return JS_FALSE;
    }
...
//|	Prototype	-	void AddXMFHTMLGump( left, top, width, height, number, hasBorder,
//hasScrollbar ) |	Date		-	16th February, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a new XMFHTML gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddXMFHTMLGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 7) {
...
//|	Prototype	-	void AddXMFHTMLGumpColor( left, top, width, height, number,
//hasBorder, hasScrollbar, rgbColour ) |	Date		-	16th February, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a new cliloc based XMFHTML gump to gump stream
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddXMFHTMLGumpColor(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                 [[maybe_unused]] jsval *rval) {
    if (argc != 8) {
...
//|	Prototype	-	void AddXMFHTMLGumpColor( left, top, width, height, number,
//hasBorder, hasScrollbar, rgbColour )
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Adds a new cliloc based XMFHTML gump to gump stream, with optional
//cliloc arguments
// o------------------------------------------------------------------------------------------------o
JSBool CGump_AddXMFHTMLTok(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc < 8) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGump_Send(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
//|					void TextMessage( message, allHear, txtHue, speechTarget,
//speechTargetSerial, speechFontType ); |					void TextMessage(
//message, allHear, txtHue, speechTarget, speechTargetSerial, speechFontType, speechType );
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Causes character to say a message
// o------------------------------------------------------------------------------------------------o
JSBool CBase_TextMessage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 7) {
    ...
    if (myClass.ClassName() == "UOXItem") {
        ...
        MethodSpeech(*myItem, trgMessage, speechType, txtHue, speechFontType, speechTarget,
                     speechTargetSerial, useUnicode);
    }
    else if (myClass.ClassName() == "UOXChar") {
        ...
        if (myChar->GetNpcAiType() == AI_EVIL || myChar->GetNpcAiType() == AI_EVIL_CASTER) {
            ...
            MethodSpeech(*myChar, trgMessage, speechType, txtHue, speechFontType, speechTarget,
                         speechTargetSerial, useUnicode);
        }
        else {
            if (!txtHue) {
...
//|	Date		-	04/20/2002
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Kill all related timers that have been associated with an item or
//character
// o------------------------------------------------------------------------------------------------o
JSBool CBase_KillTimers(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Get JS timer with specific timerId association with an item or
//character, and return time left
// o------------------------------------------------------------------------------------------------o
JSBool CBase_GetJSTimer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 2) {
        ScriptError(cx, "GetJSTimer: Invalid count of arguments :%d, needs 2 (timerId, scriptId)",
                    argc);
        return JS_FALSE;
    }
    ...
    *rval = INT_TO_JSVAL(0); // Return value 0 by default, to indicate no valid timer found
    ...
    for (const auto &Effect : cwmWorldState->tempEffects.collection()) {
        ...
        if (myObjSerial == Effect->Destination() && Effect->More1() == timerId) {
            ...
            // provided scriptId...
            if (tScript != nullptr &&
                (scriptId == Effect->AssocScript() || scriptId == Effect->More2())) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Set expiration time for JS timer with specific timerId association
//with an item or character
// o------------------------------------------------------------------------------------------------o
JSBool CBase_SetJSTimer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 3) {
        ScriptError(cx,
                    "SetJSTimer: Invalid count of arguments :%d, needs 3 (timerId, "
                    "timeInMilliseconds, scriptId)",
                    argc);
        return JS_FALSE;
    }
    ...
    *rval =
    ...
    for (const auto &Effect : cwmWorldState->tempEffects.collection()) {
        ...
        if (myObjSerial == Effect->Destination() && Effect->More1() == timerId) {
            ...
            if (tScript != nullptr &&
                (scriptId == Effect->AssocScript() || scriptId == Effect->More2())) {
                // Set the timestamp for when the Effect timer expires to that specified in
                // parameters
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Kill JS timer on item or character based on specified scriptId and
//timerId
// o------------------------------------------------------------------------------------------------o
JSBool CBase_KillJSTimer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 2) {
        ScriptError(cx, "KillJSTimer: Invalid count of arguments :%d, needs 2 (timerId, scriptId)",
                    argc);
        return JS_FALSE;
    }
    auto myObj = static_cast<CBaseObject *>(JS_GetPrivate(cx, obj));
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_Delete(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                    [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Wander(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 3 && argc != 4) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Follow(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_DoAction(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 5) {
        ScriptError(cx,
                    "DoAction: Invalid number of arguments (takes 1 - (actionID), 2 (actionID, "
                    "subActionID), 3 (actionID, null, frameCount), 4 (actionID, null, frameCount, "
                    "frameDelay ) or 5 (actionID, null, frameCount, frameDelay, playBackwards)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_EmoteMessage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 5) {
        ScriptError(cx, "EmoteMessage: Invalid number of arguments (takes 1 - 5: text, allSay, "
                        "hue, speech target and speech serial)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Dismount(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_SysMessage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc == 0 || argc > 11) {
        ScriptError(cx,
                    "SysMessage: Invalid number of arguments (takes at least 1, and at most 11)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_Disconnect(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_Teleport(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    JSEncapsulate myClass(cx, obj);
    ...
    default:
        ScriptError(cx,
                    "Invalid number of arguments passed to Teleport, needs either 1, 2, 3, 4 or 5");
        break;
    }
    ...
    if (myClass.ClassName() == "UOXItem") {
        ...
        if ((world != myChar->WorldNumber() || instanceId != myChar->GetInstanceId()) &&
            !myChar->IsNpc()) {
            CSocket *mySock = myChar->GetSocket();
            if (mySock == nullptr)
                return JS_TRUE;
...
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_MakeMenu(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
        ScriptError(cx,
                    "MakeMenu: Invalid number of arguments (takes 2, number of menu, skill used)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_SoundEffect(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc < 2 || argc > 3) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_BuyFrom(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "SellTo: Invalid Number of Arguments: %d", argc);
        return JS_FALSE;
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Checks for specified spell in first spellbook found in player's
//inventory
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Removes specified spell from first spellbook found in player's
//inventory
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_RemoveSpell(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns value for tag with specified name, if tag has been stored on
//the object
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_SetTag(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if ((argc != 2) && (argc != 1)) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns value for temporary tag with specified name, if tag has been
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Stores temporary tag with specified name and value on object, does
//not persist |					across server restart (or character reconnect)
// o------------------------------------------------------------------------------------------------o
JSBool CBase_SetTempTag(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if ((argc != 2) && (argc != 1)) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_GetNumTags(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                        jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_GetTagMap(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       jsval *rval) {
    if (argc != 0) {
        ScriptError(cx, "Invalid Count of Parameters: %d, need: 0", argc);
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_GetTempTagMap(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                           jsval *rval) {
    if (argc != 0) {
        ScriptError(cx, "Invalid Count of Parameters: %d, need: 0", argc);
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_OpenBank(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    CChar *myChar = static_cast<CChar *>(JS_GetPrivate(cx, obj));
    if (!ValidateObject(myChar)) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_OpenContainer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    CSocket *mSock = static_cast<CSocket *>(JS_GetPrivate(cx, obj));
    if (mSock == nullptr) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_OpenLayer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    CChar *myChar = static_cast<CChar *>(JS_GetPrivate(cx, obj));
    if (!ValidateObject(myChar)) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_TurnToward(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    CChar *myChar = static_cast<CChar *>(JS_GetPrivate(cx, obj));
    ...
    if (argc == 1) {
        ...
        CBaseObject *myObj =
            static_cast<CBaseObject *>(JS_GetPrivate(cx, JSVAL_TO_OBJECT(argv[0])));
        if (!ValidateObject(myObj)) {
            ScriptError(cx, "(TurnToward) Invalid Object passed");
            return JS_FALSE;
        }
    ...
    if (newDir != myChar->GetDir()) {
        CSocket *mySock = myChar->GetSocket();
        if (mySock != nullptr) {
            // Update direction, but don't mark character as "dirty" to add them to update queue -
            // we want to send the update straight away
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Executes specified command (with cmd params as part of cmdString)
//for character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ExecuteCommand(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGuild_AcceptRecruit(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CGuild_IsAtPeace(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns the amount of the items of given ID, colour and moreVal
//character has in packs
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ResourceCount(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    ...
    if ((argc < 1) || (argc > 4)) {
        ScriptError(cx,
                    "(ResourceCount) Invalid count of parameters: %d, needs from 1 to 4 parameters",
                    argc);
        return JS_FALSE;
    }
...
//)
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Removes specified amount of items of given ID, colour and MORE value
//from |					char's packs, and returns amount deleted
// o------------------------------------------------------------------------------------------------o
JSBool CBase_UseResource(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    ...
    if ((argc < 2) || (argc > 5)) {
        ScriptError(cx,
                    "(UseResource) Invalid count of parameters: %d, needs from 2 to 5 parameters",
                    argc);
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_BoltEffect(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                        [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    CChar *myChar = static_cast<CChar *>(JS_GetPrivate(cx, obj));
    if (ValidateObject(myChar)) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Provides player with a custom target cursor that calls back into a
//specific |					function in script when a target has been selected.
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Very similar to PopUpTarget. It's a callback situation instead. tNum
//must be
//|					between 0 and 255. Says toSay, and shows a cursor. Reenters the
//script
//|					associated with the socket's player (ie player who gets the
//cursor).
//|					Enters function name based on the value of tNum. if tNum is 0, then
//the |					function would be onCallback0.
//|
//|					Useable with both sockets and characters.
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_CustomTarget(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    JSEncapsulate myClass(cx, obj);

    if ((argc > 3) || (argc < 1)) {
        ScriptError(cx,
                    "(CustomTarget) Invalid count of parameters: %d, either needs 1, 2 or 3 "
                    "(targetID, textToShow, cursorType",
                    argc);
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Provides a call to an inbuilt popup target. tNum must be between 0
//and 255
//|					inclusive. Says toSay, and shows a cursor. Note that this allows
//access |					potentially to GM functions.
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_PopUpTarget(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if ((argc > 2) || (argc < 1)) {
...
JSBool CBase_InRange(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 2) {
        ScriptError(cx,
                    "(InRange): Invalid count of parameters: %d needs 2 (Item/Char and distance)",
                    argc);
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	timerId must be between 0 and 100. Resulting onTimer event will be
//triggered.
//|					Which script it triggers in depends on the (optional, defaults to
//false) third |					parameter, which can be |
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_StartTimer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    CBaseObject *myObj = static_cast<CBaseObject *>(JS_GetPrivate(cx, obj));
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Performs a skillcheck for character based on specified skill.
//Returns true
//|					if result of skillcheck is between provided minimum and maximum
//values.
//|					isCraftSkill is a boolean flag to indicate if skill check is done
//for a craft skill;
//|					if true, an alternate skill check formula is used that gives player
//a minimum 50% chance
//|					if they at least meat the minimum requirements for crafting an
//item
// o------------------------------------------------------------------------------------------------o
JSBool CChar_CheckSkill(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 3 && argc != 4) {
        ScriptError(cx, "CheckSkill: Invalid number of arguments (takes 3 or 4, skillNum, "
                        "minSkill, maxSkill and isCraftSkill (optional))");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_OpenPlank(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Calls up the onSpeechInput event using specified ID, with the text
//the user types
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SpeechInput(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
    ...
    else if (argc == 2) // We got an "affected" item as well
    {
        ...
        ScriptError(cx, "(SpeechInput) Invalid Count of parameters: %d, needs: either 1 or 2",
                    argc);
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Sends update to client with specified stat (health, mana or stamina)
//for object
//| Notes		-	Can be used with any character, as well as with items/multis with
//damageable flag enabled
// o------------------------------------------------------------------------------------------------o
JSBool CBase_UpdateStats(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx,
                    "(UpdateStats) Invalid Number of Arguments %d, needs: 1 (stat type - 0, 1 or 2 "
                    "for Health, Mana or Stamina)",
                    argc);
        return JS_FALSE;
    }
    ...
    if (myObj->CanBeObjType(OT_MULTI) || myObj->CanBeObjType(OT_ITEM)) {
        ...
        if (!myObj->IsDamageable()) {
            ScriptError(cx, "UpdateStatus: Can only be used with characters, or Items/Multis with "
                            "damagable flag set to true");
            return JS_FALSE;
        }
    }
    ...
    case 2: // Stamina - only relevant for characters
        ...
        break;
    default:
        ScriptError(cx, "UpdateStats: Argument can only contain values 0, 1 or 2 for Health, Mana "
                        "or Stamina respectively");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Applies a specified level of poison to the character for a specified
//amount of |					time (in milliseconds).
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SetPoisoned(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc < 2 || argc > 3) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Deletes specified item by exploding it, dealing 5-10 dmg to nearby
//characters
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ExplodeItem(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Sets character to the specified visibility level for the specified
//amount of |					time (in milliseconds).
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SetInvisible(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_SetCont(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc, jsval *argv,
                     jsval *rval) {
    CItem *myItem = static_cast<CItem *>(JS_GetPrivate(cx, obj));
    JSObject *tObj = JSVAL_TO_OBJECT(argv[0]);
    CBaseObject *trgObj = static_cast<CBaseObject *>(JS_GetPrivate(cx, tObj));

    if (!ValidateObject(myItem) || !ValidateObject(trgObj) ||
        (trgObj->GetSerial() == INVALIDSERIAL)) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_IsMulti(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                     jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_IsBoat(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                    jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_RemoveFromBanList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_RemoveFromFriendList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                   jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_RemoveFromGuestList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                  jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_RemoveFromOwnerList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                  jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_ClearBanList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                           jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_ClearFriendList(JSContext *cx, JSObject *obj, uintN argc,
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_ClearGuestList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                             jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_ClearOwnerList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                             jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Randomly sets the placement/position of an item inside the container
//it is in.
//|					If second parameter is true, UOX3 will attempt to stack the item
//with an existing item |					instead.
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_OpenURL(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns value of a byte from the socket at offset, assumes 0 to 255
//as values
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns 1 byte of data from socket buffer at offset, assumes -127 to
//127 as values
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns 2 bytes of data from socket buffer at offset, assumes
//positive values
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns 2 bytes of data from socket buffer at offset, negative
//values accepted
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns 4 bytes of data from socket buffer at offset, positive
//values assumed
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns 4 bytes of data from socket buffer at offset, negative
//values accepted
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns data from socket buffer at offset with optional length
//param, string assumed |					If no length param is provided,
//reads until next null terminator
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_GetString(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 1 && argc != 2) {
        ScriptError(
            cx, "GetString: Invalid number of arguments. Takes 1 (offset) or 2 (offset, length)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_SetByte(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_SetWord(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_SetDWord(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_SetString(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_ReadBytes(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
//|					void WhoList()
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Opens a gump populated with all online or offline (if param is
//false) players
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_WhoList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    CSocket *mySock = static_cast<CSocket *>(JS_GetPrivate(cx, obj));
    if (mySock == nullptr) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_Music(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_YellMessage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_WhisperMessage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_OpenGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Closes specified generic gump based on its ID, and provides a button
//ID response
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_CloseGump(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
        ScriptError(cx, "CloseGump: Invalid number of arguments (takes 2 - gumpId to close, and "
                        "buttonId to send as response)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CRace_IsValidBeardColour(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_ApplySection(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_AddSpell(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
//|	Date		-	23 June, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Does the actions associated with spell failure, called after the
//failure happens
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SpellFail(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_Refresh(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//|	Date		-	23 June, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Modifies item properties based on item's rank (could be calculated
//based on
//|					player's crafting skill, but is always 10 if rank system not
//enabled. Higher
//|					values will increase base properties of item, lower will decrease
//base
//|					properties). maxRank is the maximum amount of ranks in the rank
//system (10 by default).
// o------------------------------------------------------------------------------------------------o
JSBool CItem_ApplyRank(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
JSBool CItem_IsOnFoodList(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 1 || argc > 7) {
        ScriptError(cx, "(IsOnFoodList) Invalid Number of Arguments %d, needs: 1 - foodlist name",
                    argc);
        return JS_TRUE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_GetAccount([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
...
//|	Purpose		-	NOT IMPLEMENTED
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_SetAccount([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                           [[maybe_unused]] uintN argc, [[maybe_unused]] jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    return JS_FALSE;
}
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Add an account through the JS. The calling script will be required
//to specifiy |					the username, password, flags, and comment/contact
//information.
// o------------------------------------------------------------------------------------------------o
//| Changes		-	Removed UOXAccountWrapper and exposed global var Accounts
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_AddAccount(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 4) {
        ScriptError(
            cx, "Account.AddAccount(user,pass,email,flags): Invalid number of arguments (takes 4)");
        return JS_FALSE;
    }
    // Ok get our object from the global context
    if (!JSVAL_IS_STRING(argv[0]) || !JSVAL_IS_STRING(argv[1]) || !JSVAL_IS_STRING(argv[2]) ||
        !(JSVAL_IS_INT(argv[3]) || JSVAL_IS_STRING(argv[3]))) {
        ScriptError(cx, "Account.AddAccount(user,pass,email,flags): Invalid parameter specifled, "
                        "please check param types.");
        return JS_FALSE;
    }
    ...
    if (lpszUsername.empty() || lpszPassword.empty() || lpszComment.empty() ||
        lpszUsername.length() == 0 || lpszPassword.length() == 0 || lpszComment.length() == 0)
        return JS_FALSE;

    if (!Account::shared().createAccount(lpszUsername, lpszPassword, u16Flags, lpszComment)) {
        ScriptError(cx, "Account.AddAccount( user, pass, email, flags ): Account was not added. "
                        "Duplicate expected.");
        return JS_FALSE;
    }
    return JS_TRUE;
...
/// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Remove an account from the memory. This is a permanent action that
//CAN NOT be
//|					undone. After a call to this function the account will be
//immidiatly removed
//|					from memory, and will not be written out to the accounts.adm file,
//and will |					have the associated directory tagged and renamed.
// o------------------------------------------------------------------------------------------------o
//| Changes		-	Removed UOXAccountWrapper and exposed global var Accounts
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_DelAccount(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
//|	Purpose		-	NOT IMPLEMENTED
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_ModAccount([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                           [[maybe_unused]] uintN argc, [[maybe_unused]] jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    return JS_FALSE;
}
...
//|	Purpose		-	NOT IMPLEMENTED
// o------------------------------------------------------------------------------------------------o
JSBool CAccount_SaveAccounts([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                             [[maybe_unused]] uintN argc, [[maybe_unused]] jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    return JS_FALSE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_Free(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_Open(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc < 2 || argc > 4) {
        ScriptError(cx, "Open: Invalid number of arguments (takes 2 to 4 - filename, file mode and "
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_Close(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                   [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//|	Date		-	23 June, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns a string of length numBytes, reading numBytes from the
//opened file
// o------------------------------------------------------------------------------------------------o
...
//|	Date		-	23 June, 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Reads a string until it encounters a newline or the string specified
//by delimeter.
// o------------------------------------------------------------------------------------------------o
JSBool CFile_ReadUntil(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    ...
    if (until[0] == '\\' && strlen(until) > 1) {
        ...
        case 'n':
            ...
            break;
        default:
            ScriptError(cx, "ReadUntil: Unsupported character escape sequence %s", until);
            break;
        }
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_Write(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                   [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_EOF(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                 jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CFile_Length(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                    jsval *rval) {
    if (argc != 0) {
    ...
    *rval = INT_TO_JSVAL(ftell(mFile->mWrap));
    ...
    if (fpos > -1) {
        [[maybe_unused]] int newFPos =
            fseek(mFile->mWrap, fpos,
                  SEEK_SET); // We don't care about return value, so suppress compiler warning
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_FirstItem(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_NextItem(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                      jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_FinishedItems(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                           jsval *rval) {
    if (argc != 0) {
...
//|	Date		-	06 Sep 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Begins pathfinding for a character, making them walk to target
//location,
//|					halting if max amount of steps is reached before reaching the
//location
// o------------------------------------------------------------------------------------------------o
JSBool CChar_WalkTo(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 2 && argc != 3) {
    ...
    Console::shared().Print(
        util::format("WalkTo: Moving character 0x%X to (%i,%i) with a maximum of %i steps\n",
...
//|	Date		-	06 Sep 2003
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Begins pathfinding for a character, making them run to target
//location,
//|					halting if max amount of steps is reached before reaching the
//location
// o------------------------------------------------------------------------------------------------o
JSBool CChar_RunTo(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                   [[maybe_unused]] jsval *rval) {
    if (argc != 2 && argc != 3) {
...
#if defined(UOX_DEBUG_MODE)
    Console::shared().Print(
        util::format("RunTo: Moving character %i to (%i,%i) with a maximum of %i steps",
                     cMove->GetSerial(), gx, gy, maxSteps));
#endif
    if (cMove->GetNpcWander() != WT_PATHFIND) {
        // Only update oldNpcWander if NPC is not already pathfinding
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Sets the specified timer with the amount of miliseconds until it
//expires
// o------------------------------------------------------------------------------------------------o
JSBool CMisc_SetTimer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_Glow(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_UnGlow(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    JSObject *mSock = JSVAL_TO_OBJECT(argv[0]);
    CSocket *mySock = static_cast<CSocket *>(JS_GetPrivate(cx, mSock));
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Opens a gate to the location marked on an item, or to a specified
//set of coords
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Gate(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 1 && argc != 4 && argc != 5) {
        ScriptError(cx, "Gate: Invalid number of arguments (takes 1: item/place; 4: x y z "
                        "worldNumber; or 5: x y z worldNumber instanceID)");
        return JS_FALSE;
    }
    ...
    SpawnGate(mChar, mChar->GetX(), mChar->GetY(), mChar->GetZ(), mChar->WorldNumber(), destX,
              destY, destZ, destWorld, destInstanceId);
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Recall(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Mark(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Sets the skill specified by name to the value specified (name must
//be the same |					as in skills.dfn, "ALLSKILLS" is also applicable.
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Kill(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Resurrect(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Dupe(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                  jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Jails character, either for ~27 hours or for specified amount of
//time in seconds
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Jail(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Release(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                     [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_Page(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_Print(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
    ...
    Console::shared().Print(arg0.toString());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_Log(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc != 1 && argc != 2) {
    ...
    JSEncapsulate arg1;
    if (argc == 1) {
        Console::shared().Log(arg0.toString());
    }
    else {
        arg1.SetContext(cx, &(argv[1]));
        Console::shared().Log(arg0.toString(), arg1.toString());
    }
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_Error(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
    ...
    JSEncapsulate arg0(cx, &(argv[0]));
    Console::shared().Error(arg0.toString());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_Warning(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
    ...
    JSEncapsulate arg0(cx, &(argv[0]));
    Console::shared().Warning(arg0.toString());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintSectionBegin([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                                  uintN argc, [[maybe_unused]] jsval *argv,
                                  [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//|	Purpose		-
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnYellow([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                           uintN argc, [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnRed([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc,
                        [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnGreen([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                          uintN argc, [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnBlue([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc,
                         [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
        ScriptError(cx, "TurnBlue: Invalid number of arguments (takes 0)");
        return JS_FALSE;
    }
    Console::shared().TurnBlue();
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnNormal([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                           uintN argc, [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_TurnBrightWhite([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                                uintN argc, [[maybe_unused]] jsval *argv,
                                [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
    ...
    Console::shared().TurnBrightWhite();
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintDone(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 0 && argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintFailed(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 0 && argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintPassed([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                            uintN argc, [[maybe_unused]] jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_ClearScreen([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                            uintN argc, [[maybe_unused]] jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintBasedOnVal(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc,
    ...
    JSEncapsulate arg0(cx, &(argv[0]));
    Console::shared().PrintBasedOnVal(arg0.toBool());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_MoveTo(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
    ...
    Console::shared().MoveTo(arg0.toInt(), arg1.toInt());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Prints a special tag in console similar to [done] and [failed], but
//with custom |					text and colour |	Notes		-
//Available colours: |					0 - Normal
//|					1 - Blue
//|					2 - Red
//|					3 - Green
//|					4 - Yellow
//|					5 - Bright White
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_PrintSpecial(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
    ...
    JSEncapsulate arg0(cx, &(argv[0]));
    JSEncapsulate arg1(cx, &(argv[1]));
    Console::shared().PrintSpecial(arg0.toInt(), arg1.toString().c_str());
    return JS_TRUE;
}
...
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_BeginShutdown([[maybe_unused]] JSContext *cx, [[maybe_unused]] JSObject *obj,
                              uintN argc, [[maybe_unused]] jsval *argv,
                              [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//|						8 Reload HTML
// o------------------------------------------------------------------------------------------------o
JSBool CConsole_Reload(JSContext *cx, [[maybe_unused]] JSObject *obj, uintN argc, jsval *argv,
                       [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Plays the MOVEFX effect of the specified spell in SPELLS.DFN, going
//from the |					character to the target
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SpellMoveEffect(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Plays the STATFX effect of the specified spell in SPELLS.DFN, on the
//character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_SpellStaticEffect(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                               [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Break a caster's concentration. Provide socket as extra argument for
//player chars
// o------------------------------------------------------------------------------------------------o
JSBool CChar_BreakConcentration(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_SendAddMenu(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                           [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_LockDown(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                      [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Initializes a WanderArea (10x10 box, or 10 radius circle) for the
//NPC.
//|					Will only work if they already have a wandermode set to box or
//circle.
// o------------------------------------------------------------------------------------------------o
JSBool CChar_InitWanderArea(JSContext *cx, JSObject *obj, [[maybe_unused]] uintN argc,
                            [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    CChar *mChar = static_cast<CChar *>(JS_GetPrivate(cx, obj));
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_Carve(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                   [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_GetTileName(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                         jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Gets coordinates for specified corner ("NW", "NE", "SW" or "SE") of
//multi
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_UnsecureContainer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CMulti_IsSecureContainer(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                jsval *rval) {
    if (argc != 1) {
...
JSBool CMulti_FirstChar(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    ...
    {
        firstChar = myObj->GetOwnersOfMultiList(true)
                        ->First(); // true to clear list before it is regenerated
    }
    else if (listType == "friend") // Friends
    {
        firstChar = myObj->GetFriendsOfMultiList(true)
                        ->First(); // true to clear list before it is regenerated
    }
    else if (listType == "guest") // Guests
    {
        firstChar = myObj->GetGuestsOfMultiList(true)
                        ->First(); // true to clear list before it is regenerated
    }
    else if (listType == "banned") // Banned
    {
        firstChar = myObj->GetBannedFromMultiList(true)
                        ->First(); // true to clear list before it is regenerated
    }
    else {
        ScriptError(cx, "FirstChar: Unknown listType provided. Supported listTypes: visitor "
                        "(default), owner, friend or banned");
        return JS_FALSE;
    }
...
JSBool CMulti_NextChar(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    ...
    else if (listType == "banned") // Banned
    {
        ...
        ScriptError(cx, "FinishedChars: Unknown listType provided. Supported listTypes: visitor "
                        "(default), owner, friend or banned");
        return JS_FALSE;
    }
...
JSBool CBase_CanSee(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 1 && argc != 3) {
        ScriptError(cx,
                    "CanSee: Invalid number of arguments (takes 1, a char/item or 3, an x/y/z)");
        return JS_FALSE;
    }
    ...
    if (argc == 1) // we've been passed an item, character, or socket
    {
        ...
        if (myClass.ClassName() == "UOXSocket") {
            ...
            if (tChar->WorldNumber() != mChar->WorldNumber() ||
                tChar->GetInstanceId() != mChar->GetInstanceId()) {
                *rval = JSVAL_FALSE;
                return JS_TRUE;
            }
            x = tChar->GetX();
            ...
            if (tObj->WorldNumber() != mChar->WorldNumber() ||
                tObj->GetInstanceId() != mChar->GetInstanceId()) {
                *rval = JSVAL_FALSE;
                return JS_TRUE;
            }
    ...
    if (ValidateObject(mChar)) {
        *rval = BOOLEAN_TO_JSVAL(LineOfSight(
            mSock, mChar, x, y, z, WALLS_CHIMNEYS + DOORS + FLOORS_FLAT_ROOFING, false, zTop));
    }
    else {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_DisplayDamage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                             [[maybe_unused]] jsval *rval) {
    if (argc != 2) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Deals damage of a specified damageType to a character, with optional
//parameters to include attacker and
//|					whether or not attacker should be flagged as a criminal
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Damage(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                    [[maybe_unused]] jsval *rval) {
    if (argc < 1 || argc > 4) {
        ScriptError(cx,
                    "(CChar_Damage) Invalid Number of Arguments %d, needs: 1 (amount), 2 (amount, "
                    "damageType), 3 (amount, damageType and attacker) or 4 (amount, damageType, "
                    "attacker and doRepsys)",
                    argc);
        return JS_TRUE;
    }
...
JSBool CChar_InitiateCombat(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "(InitiateCombat) Invalid Number of Arguments %d, takes: 1 (targetChar))",
                    argc);
        return JS_TRUE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_InvalidateAttacker(JSContext *cx, JSObject *obj, uintN argc,
                                [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_AddAggressorFlag(JSContext *cx, JSObject *obj, uintN argc,
                              [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_RemoveAggressorFlag(JSContext *cx, JSObject *obj, uintN argc,
                                 [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_CheckAggressorFlag(JSContext *cx, JSObject *obj, uintN argc,
                                [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Updates the expiry timestamp of character's aggressor flag towards
//target character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_UpdateAggressorFlagTimestamp(JSContext *cx, JSObject *obj, uintN argc,
                                          [[maybe_unused]] jsval *argv,
                                          [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "(UpdateAggressorFlagTimestamp) Invalid Number of Arguments %d, takes: 1)",
                    argc);
        return JS_TRUE;
    }
    ...
    if (!ValidateObject(ourTarget)) {
        ScriptError(cx,
                    "(UpdateAggressorFlagTimestamp): Operating on an invalid Character (arg 0)");
        return JS_TRUE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ClearAggressorFlags(JSContext *cx, JSObject *obj, uintN argc,
                                 [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns true/false depending on whether character has any active
//aggressor flags
//|					Optional parameter supported to check only flags towards players
//and ignore NPCs
// o------------------------------------------------------------------------------------------------o
JSBool CChar_IsAggressor(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
//|	Purpose		-	Adds aggressor flag for character towards target character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_AddPermaGreyFlag(JSContext *cx, JSObject *obj, uintN argc,
                              [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_RemovePermaGreyFlag(JSContext *cx, JSObject *obj, uintN argc,
                                 [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Check if character has an active permagrey flag towards target
//character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_CheckPermaGreyFlag(JSContext *cx, JSObject *obj, uintN argc,
                                [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Updates the expiry timestamp of character's permagrey flag towards
//target character
// o------------------------------------------------------------------------------------------------o
JSBool CChar_UpdatePermaGreyFlagTimestamp(JSContext *cx, JSObject *obj, uintN argc,
                                          [[maybe_unused]] jsval *argv,
                                          [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "(UpdatePermaGreyFlagTimestamp) Invalid Number of Arguments %d, takes: 1)",
                    argc);
        return JS_TRUE;
    }
    ...
    if (!ValidateObject(ourTarget)) {
        ScriptError(cx,
                    "(UpdatePermaGreyFlagTimestamp): Operating on an invalid Character (arg 0)");
        return JS_TRUE;
    }
...
//|	Purpose		-	Clears all the character's aggressor flags towards other characters
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ClearPermaGreyFlags(JSContext *cx, JSObject *obj, uintN argc,
                                 [[maybe_unused]] jsval *argv, [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
//and ignore NPCs
// o------------------------------------------------------------------------------------------------o
JSBool CChar_IsPermaGrey(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                         [[maybe_unused]] jsval *rval) {
    if (argc > 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Heals a character, with optional argument to provide character who
//healed
// o------------------------------------------------------------------------------------------------o
JSBool CChar_Heal(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                  [[maybe_unused]] jsval *rval) {
    if (argc != 1 && argc != 2) {
        ScriptError(cx,
                    "(CChar_Heal) Invalid Number of Arguments %d, needs: 1 (amount) or 2 (amount "
                    "and healer)",
                    argc);
        return JS_TRUE;
    }
...
JSBool CBase_Resist(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 1 && argc != 2) {
        ScriptError(cx, "Resist: Invalid number of arguments (takes 1, the resist type or 2, the "
                        "resist type and value to set)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns the defense value against damageType of the armor item at
//the location |					hitLoc and does armor damage if needed
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Valid moreVarName values: "more", "more0", "more1", "more2",
//"morex", "morey", "morez" |					Valid moreVarPart values: 1, 2, 3, 4
// o------------------------------------------------------------------------------------------------o
JSBool CItem_GetMoreVar(JSContext *cx, JSObject *obj, uintN argc, jsval *argv, jsval *rval) {
    if (argc != 2) {
        ScriptError(cx,
                    "GetMoreVar: Invalid number of arguments (takes 2, the moreVarName (more, "
                    "more0, more1, more2, morex, morey or morez - and the moreVarPart (1 to 4))");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Sets the value of the specified moreVar's moreVarPart to
//moreVarValue
// o------------------------------------------------------------------------------------------------o
//|	Notes		-	Valid moreVarName values: "more", "more0", "more1", "more2",
//"morex", "morey", "morez" |					Valid moreVarPart values: 1, 2, 3, 4
...
// o------------------------------------------------------------------------------------------------o
JSBool CItem_SetMoreVar(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                        [[maybe_unused]] jsval *rval) {
    if (argc != 3) {
        ScriptError(cx, "SetMoreVar: Invalid number of arguments (takes 3, the moreVarName (more, "
                        "more0, more1, more2, morex, morey or morez); the moreVarPart (1 to 4) and "
                        "the moreVarValue (0-255)");
        return JS_FALSE;
    }
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_AddScriptTrigger(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                              [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CBase_RemoveScriptTrigger(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                 [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Checks if object has a specific scriptTrigger in it's list of
//scriptTriggers
// o------------------------------------------------------------------------------------------------o
...
//|	Purpose		-	Adds a scriptTrigger to an object's list of scriptTriggers
// o------------------------------------------------------------------------------------------------o
JSBool CRegion_AddScriptTrigger(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "AddScriptTrigger: Invalid number of arguments (takes 1)");
        return JS_FALSE;
...
// o------------------------------------------------------------------------------------------------o
JSBool CRegion_RemoveScriptTrigger(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                   [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
        ScriptError(cx, "RemoveScriptTrigger: Invalid number of arguments (takes 1)");
        return JS_FALSE;
...
// o------------------------------------------------------------------------------------------------o
JSBool CRegion_GetOrePref(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                          [[maybe_unused]] jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CRegion_GetOreChance(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                            [[maybe_unused]] jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_GetFriendList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                           jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_ClearFriendList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                             jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_GetPetList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                        jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns whether character pChar has previously owned the pet (is on
//pet owner list)
// o------------------------------------------------------------------------------------------------o
...
// o------------------------------------------------------------------------------------------------o
//|	Purpose		-	Returns chance (in values ranging from 0 to 1000) of pChar (player)
//successfully controlling mChar (pet)
// o------------------------------------------------------------------------------------------------o
JSBool CChar_CalculateControlChance(JSContext *cx, JSObject *obj, uintN argc, jsval *argv,
                                    jsval *rval) {
    if (argc != 1) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CChar_GetFollowerList(JSContext *cx, JSObject *obj, uintN argc, [[maybe_unused]] jsval *argv,
                             jsval *rval) {
    if (argc != 0) {
                ...
                *rval = BOOLEAN_TO_JSVAL(false);
                ...
                if (targSock != nullptr) {
                    ...
                    if (leaderSock != nullptr) {
                        ...
                        targSock->SysMessage(
                            9002); // You have been invited to join a party, type /accept or
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_FirstTriggerWord(JSContext *cx, JSObject *obj, uintN argc,
                                [[maybe_unused]] jsval *argv, jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_NextTriggerWord(JSContext *cx, JSObject *obj, uintN argc,
                               [[maybe_unused]] jsval *argv, jsval *rval) {
    if (argc != 0) {
...
// o------------------------------------------------------------------------------------------------o
JSBool CSocket_FinishedTriggerWords(JSContext *cx, JSObject *obj, uintN argc,
                                    [[maybe_unused]] jsval *argv, jsval *rval) {
    if (argc != 0) {
