	...
	switch( cmd.data()[0] )
	{
		...
		case 'G':
			if( cmd == "GMMENU" )
			{
				...
				CPIHelpRequest toHandle( s, static_cast<UI16>( std::stoul( util::trim( util::strip( data, "//" )), nullptr, 0 )));
				toHandle.Handle();
			}
			else if( cmd == "GMPAGE" )
			{
			...
			else if( cmd == "GOPLACE" )
			{
				...
				UI16 placeNum = static_cast<UI16>( std::stoul( util::trim( util::strip( data, "//" )), nullptr, 0 ));
		...
		case 'I':
			if( cmd == "ITEMMENU" )
			{
				...
				BuildAddMenuGump( s, static_cast<UI16>( std::stoul( util::trim( util::strip( data, "//" )), nullptr, 0 )));
			}
		...
		case 'M':
			if( cmd == "MAKEMENU" )
			{
				...
				Skills->NewMakeMenu( s, std::stoi( util::trim( util::strip( data, "//" )), nullptr, 0 ), static_cast<UI08>( s->AddId() ));
			}
			break;
		case 'N':
			if( cmd == "NPC" )
