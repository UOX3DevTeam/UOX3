		...
		if( toFind )
		{
			for( const auto &sec : toFind->collection() )
			{
				...
				if( util::upper( tag ) == "ADDITEM" )
				{
					...
					if( csecs.size() > 1 )
					{
						if( !CreateBodyPart( mChar, i, util::trim( util::strip( csecs[0], "//" )), static_cast<UI16>( std::stoul( util::trim( util::strip( csecs[1], "//" )), nullptr, 0 ))))
						{
							return false;
						}
					}
		...
		if( toFind )
		{
			for( const auto &sec : toFind->collection() )
			{
				...
				if( util::upper( tag ) == "ADDITEM" )
				{
					...
					if( csecs.size() > 1 )
					{
						Items->CreateScriptItem( s, mChar, util::trim( util::strip( csecs[0], "//" )), static_cast<UI16>( std::stoul( util::trim( util::strip( csecs[1], "//" )), nullptr, 0 )), OT_ITEM, true );
					}
					else
					{
