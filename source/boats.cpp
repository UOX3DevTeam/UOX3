...
bool CreateBoat(CSocket *s, CBoatObj *b, std::uint8_t id2, std::uint8_t boattype) {
    ...
    CItem *tiller = worldItem.CreateItem(nullptr, mChar, 0x3E4E, 1, 0, CBaseObject::OT_ITEM, false, true, worldNumber, instanceId, x, y, z);
    if (tiller == nullptr)
        return false;
    ...
    CItem *p2 = worldItem.CreateItem(nullptr, mChar, 0x3EB2, 1, 0, CBaseObject::OT_ITEM, false, true, worldNumber, instanceId, x, y, z); // Plank2 is on the RIGHT side of the boat
    if (p2 == nullptr)
        return false;
    ...
    CItem *p1 = worldItem.CreateItem(nullptr, mChar, 0x3EB1, 1, 0, CBaseObject::OT_ITEM, false, true, worldNumber, instanceId, x, y, z); // Plank1 is on the LEFT side of the boat
    if (p1 == nullptr)
        return false;
    ...
    CItem *hold = worldItem.CreateItem(nullptr, mChar, 0x3EAE, 1, 0, CBaseObject::OT_ITEM, false, true, worldNumber, instanceId, x, y, z);
    if (hold == nullptr)
        return false;
    ...
    for (const auto &bItem : itemList->collection()) {
        if (ValidateObject(bItem)) {
            if (!(bItem == tiller || bItem == p1 || bItem == p2 || bItem == hold)) {
                ...
                // Remember to remove any items on board from sight of nearby players if boat got
                // teleported
                if (teleportBoat) {
...
void CBoatResponse::Handle(CSocket *mSock, CChar *mChar) {
    ...
    switch (trigWord) {
        ...
        case TW_SETNAME: {
            ...
            // Trim spaces from start and end of player provided name, and see if there's actually a
...
void ModelBoat(CSocket *s, CBoatObj *i) {
    ...
    if (i->GetOwnerObj() == mChar) {
        ...
        // this is a high-value item, so let's prevent the action instead of dropping the item on
        // the ground!
        ...
        CItem *model = worldItem.CreateItem(s, mChar, 0x14f3, 1, 0, CBaseObject::OT_ITEM, true);
        if (model == nullptr)
            return;
