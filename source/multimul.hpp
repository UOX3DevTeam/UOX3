...
class MultiCollection : public UopFile {
    ...
    auto virtual ProcessEntry(std::size_t entry, std::size_t index, std::vector<std::uint8_t> &data)
        -> bool final;
    auto virtual ProcessHash(std::uint64_t hash, std::size_t entry, std::vector<std::uint8_t> &data)
        -> bool final;
  ...
  public:
    ...
    auto LoadMultiCollection(const std::filesystem::path &uodir, const std::string &housingbin,
                             TileInfo *info = nullptr) -> bool;
