	...
	for( Script *ourScript = FileLookup->FirstScript( create_def ); !FileLookup->FinishedScripts( create_def ); ourScript = FileLookup->NextScript( create_def ))
	{
		...
		for( const auto &[eName1, toSearch] : ourScript->collection() )
		{
			...
			if( "SUBMENU" == eName.substr( 0, 7 )) // is it a menu? (not really SUB, just to avoid picking up MAKEMENUs)
			{
				...
				ourEntry = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 ));
				...
				for( const auto &sec : toSearch->collection() )
				{
					...
					else if( UTag == "RESOURCE" )
					{
						...
						if( ssecs.size() > 1 )
						{
							if( ssecs.size() == 4 )
							{
								tmpResource.moreVal = static_cast<UI32>( std::stoul( util::trim( util::strip( ssecs[3], "//" )), nullptr, 0 ));
							}
							if( ssecs.size() >= 3 )
							{
								tmpResource.colour = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[2], "//" )), nullptr, 0 ));
							}
							if( ssecs.size() >= 2 )
							{
								tmpResource.amountNeeded = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 ));
							}
						}
						...
						{
							tmpResource.idList.push_back( static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 0 )));
						}
					...
					else if( UTag == "SKILL" )
					{
						...
						else
						{
							...
							{
								tmpResource.maxSkill = 1000;
								tmpResource.minSkill =  static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 ));
							}
							else
							{
								tmpResource.minSkill = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[1], "//" )), nullptr, 0 ));
								tmpResource.maxSkill = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[2], "//" )), nullptr, 0 ));
							}
							tmpResource.skillNumber = static_cast<UI16>( std::stoul( util::trim( util::strip( ssecs[0], "//" )), nullptr, 0 ));
						}
