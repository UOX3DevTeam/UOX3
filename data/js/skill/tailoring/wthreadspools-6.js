...
function onUse( pUser, iUsed ) 
{
	...
	CustomTarget( srcSock, 0, "Where do you want to spin the cotton on?" );
}
...
function onCallback0( tSock, targSerial )
{
	...
	if( iID == 0x1015 || iID == 0x1019 || iID == 0x101c || iID == 0x10a4 )
	{
		...
		// remove one bale of cotton
		var iMakeResource = GetResourceCount( tChar, 0x0df9, 0 );	// is there enough resources to use up to make it
		if( iMakeResource < 1 )
		{
			SysMessage( tSock, "You dont seem to have any cotton!" );
			return;
		}
		UseResource( tChar, 0x0df9, 0, 1); // uses up a resource (character, item ID, item colour, amount)
	...
	SysMessage( tSock, "That is not a thing to use cotton on." );
