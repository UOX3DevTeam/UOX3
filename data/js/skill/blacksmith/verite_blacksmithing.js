...
function VeriteSmithGump( socket, pUser )
{
    if ( socket == null )
        return;
    ...
    bsGump.AddHTMLGump( 178, 4, 160, 25, 0, 0, GetDictionaryEntry( 2274, socket.language ) );       // Verite Blacksmithy Selection Menu
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + "16 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 19.4 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 14 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 16.9 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 18 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 21.9 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 20 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 39.1 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 12 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 58.9 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 25 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 75.0 <\center>" )
    ...
    bsGump.AddToolTip( 1114778, socket, "<center> <basefont color=#FFFF00>" + GetDictionaryEntry( 2214, socket.language ) + "</basefont> \n" + GetDictionaryEntry( 2215, socket.language ) + " 15 Verite Ingots \n" + GetDictionaryEntry( 2216, socket.language ) + " 8.3 <\center>" )
	...
	switch ( pButton )
	{
		...
		case 1:
            if ( anvil > 0 )
            {
				...
				{
					case 2:
                        MakeItem( pSock, pUser, 1006 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Ringmail Gloves
					case 3:
                        MakeItem( pSock, pUser, 1008 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Ringmail Legs
					case 4:
                        MakeItem( pSock, pUser, 1007 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Ringmail Arms
					case 5:
                        MakeItem( pSock, pUser, 1009 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Ringmail Chest
					case 6:
                        MakeItem( pSock, pUser, 1010 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Chain Coif
					case 7:
                        MakeItem( pSock, pUser, 1011 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Chain Legs
					case 8:
                        MakeItem( pSock, pUser, 1012 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Chain Chest
					case 9:
                        MakeItem( pSock, pUser, 1015 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Arms
					case 10:
                        MakeItem( pSock, pUser, 1014 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Gloves
					case 11:
                        MakeItem( pSock, pUser, 1013 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Gorget
					case 12:
                        MakeItem( pSock, pUser, 1016 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Legs
					case 13:
                        MakeItem( pSock, pUser, 1017 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Chest
					case 14:
                        MakeItem( pSock, pUser, 1018 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Female Plate Chest
					case 15:
                        MakeItem( pSock, pUser, 1020 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Bascinet
					case 16:
                        MakeItem( pSock, pUser, 1022 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Close Helm
					case 17:
                        MakeItem( pSock, pUser, 1019 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Helmet
					case 18:
                        MakeItem( pSock, pUser, 1021 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Norse Helm
					case 19:
                        MakeItem( pSock, pUser, 1023 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Plate Helm
					case 20:
                        MakeItem( pSock, pUser, 1000 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Buckler
					case 21:
                        MakeItem( pSock, pUser, 1001 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Bronze Shield
					case 22:
                        MakeItem( pSock, pUser, 1005 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Heater Shield
					case 23:
                        MakeItem( pSock, pUser, 1002 );
                        VeriteSmithGump( pSock, pUser );
              			break;//Metal Shield
					case 24:
                        MakeItem( pSock, pUser, 1004 );
                        VeriteSmithGump( pSock, pUser );
