...
// Provides Harbor Master context-menu services for locating, towing and
// abandoning player-owned ships.

// Area callbacks cannot carry arbitrary context, so these values temporarily
// communicate the active owner and result while FindOwnedShip runs synchronously.
let harborSearchOwner = 0;
let harborSearchResult = 0;
...
function onContextMenuRequest( socket, harborMaster )
{
	...
	let entries = [ { id : 43, text : 1149570, flags : 0x0000, hue : 0x03E0 } ];
...
function onIterate( candidate )
{
	...
	let owner = CalcCharFromSer( harborSearchOwner );
...
function CreateReplacementShipToken( socket, user, harborMaster )
{
	...
	let token = CreateDFNItem( socket, user, IsHighSeasHarborShip( boat ) ? "usedrecallrune" : "boat_key", 1, "ITEM", true );
	...
	token.Refresh();
	harborMaster.TextMessage( IsHighSeasHarborShip( boat ) ? "A recall rune to your ship has been placed in your backpack."
														   : "A replacement ship key has been placed in your backpack.",
		true, 0x03B2 );
}
...
function onGumpPress( socket, button, gumpData )
{
	...
	let boat = CalcItemFromSer( parseInt( user.GetTempTag( "harborAbandonBoat" ) ) );
...
function onTimer( boat, timerID )
{
	...
	let owner = CalcCharFromSer( parseInt( boat.GetTempTag( "harborAbandonOwner" ) ) );
	...
	let itemSerials = [];
	for( let item = boat.FirstItem(); !boat.FinishedItems(); item = boat.NextItem() )
	{
	...
	{
		let shipItem = CalcItemFromSer( itemSerials[i] );
