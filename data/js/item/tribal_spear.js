...
function onPickup( iPickedUp, pGrabber )
{
	...
	if( pSocket && pSocket.pickupSpot == 3 )
	{
		...
		if( ValidateObject( iCont ) && iCont.corpse && ( iCont.amount == 0x00b9 || iCont.amount == 0x00ba ))
		{
			if( RandomNumber( 1, 100 ) <= 95 ) // 95% chance of breaking when being looted
			{
				...
				pGrabber.TextMessage( GetDictionaryEntry( 1682, pSocket.language ), false, 0x096a ); // The Tribal Spear breaks apart in your hands as you pick it up!
				iPickedUp.Delete();
				return false;
			}
			else
