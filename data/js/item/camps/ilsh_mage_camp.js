...
function onCreateDFN( baseCampItem, objType )
{
	...
	baseCampItem.StartTimer( 10, 1, 5503 );
}

function onTimer( baseCampItem, timerID )
{
	var multiID = baseCampItem.morex;
	var campMulti = CreateBaseMulti( 0x1f5, baseCampItem.x, baseCampItem.y, baseCampItem.z, baseCampItem.worldnumber, baseCampItem.instanceID );
	...
	// Second, add the mage sign
	var mageSign = CreateDFNItem( null, null, "0x0bae", 1, "ITEM", false, 0, baseCampItem.worldnumber, baseCampItem.instanceID );
	if( !ValidateObject( mageSign ))
	{
	...
	baseCampItem.SetTag( "mageSignSer", mageSign.serial.toString() );

	// Position the sign correctly relative to baseCampItem's position
	mageSign.x = baseCampItem.x - 5;
	mageSign.y = baseCampItem.y + 5;
	mageSign.z = baseCampItem.z -4;
	mageSign.movable = 2;
}
...
function onDelete( baseCampItem, objType )
{
	...
	// Delete the mage sign
	var mageSign = CalcItemFromSer( parseInt( baseCampItem.GetTag( "mageSignSer" )));
	if( ValidateObject( mageSign ))
	{
		mageSign.Delete();
	}
