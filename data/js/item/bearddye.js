...
function display_dyes( dyes_arr, page_num, color, pUser ) 
{
	...
	var myBeardDye = new Gump;

	beardDye( myBeardDye, pUser );
	for ( var i = 0; i < dyes_arr.length; i++ )
	{
		...
		myBeardDye.AddPage( page_num );
		myBeardDye.AddRadio( col_num, 30 + (row_offset * 20 ), 0xa92, 0, dyes_arr[0] + i );
		myBeardDye.AddText( col_num + COL_SPACING, 30 + (row_offset * 20), dyes_arr[i], color );
		row_offset++;
	}
	myBeardDye.Send( pUser );
	myBeardDye.Free();
}

function beardDye( myBeardDye, pUser)
{
	var socket = pUser.socket;
	myBeardDye.AddPage(0);
	myBeardDye.AddBackground( 0, 0, 420, 400, 5054 );
	myBeardDye.AddBackground( 10, 10, 400, 380, 3000 );
	myBeardDye.AddText( 91, 10, 1000, GetDictionaryEntry( 17117, socket.language ) ); // Beard Color Selection Menu
	myBeardDye.AddText( 81, 361, 1000, GetDictionaryEntry( 17118, socket.language ) ); // Dye my beard this color!
	myBeardDye.AddBackground( 28, 30, 120, 315, 5054 );
	myBeardDye.AddButton( 251, 360, 0xf7, 1, 0, 1 );
	...
	{
		myBeardDye.AddButton( 40, 40 + ( i * 20 ), 0x0a9a, 1, 0, 2 + i );
		myBeardDye.AddText( 60, 40 + ( i * 20 ), menuDye[i], GetDictionaryEntry( menuNames[i], socket.language ) );
	}
}
