...
function onUseChecked( pUser, iUsed )
{
	...
	if( pSocket && iUsed && iUsed.isItem )
	{
		...
		if( pUser.GetTempTag( "snowpile" ) == null || pUser.GetTempTag( "snowpile" ) == 0 )
		{
			...
			pSocket.SysMessage( GetDictionaryEntry( 5512, pSocket.language )); //You carefully pack the snow into a ball...
			pUser.CustomTarget(0);
			pUser.StartTimer( 5000, 0, true );
		}
		else if( pUser.GetTempTag( "snowpile" ) == 1 )
		{
			pSocket.SysMessage( GetDictionaryEntry( 5511, pSocket.language )); // The snow is not ready to be packed yet.  Keep trying.
		}
		else
		{
			return true;
...
function onCallback0( socket, myTarget )
{
	...
	if( !socket.GetWord( 1 ) && ValidateObject( myTarget ) && myTarget.isChar && myTarget.socket )
	{
		pUser.DoAction( 0x9 );
