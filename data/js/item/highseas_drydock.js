...
/** @type { ( socket: Socket, user: Character, boat: Multi ) => any } */
function ValidateDryDockOwnerAndRange( socket, user, boat )
{
	if( !ValidateObject( boat ) || !boat.IsBoat() )
	{
		socket.SysMessage( "That is not a High Seas vessel." );
		return false;
	}
	...
	if( user.dead )
	{
		socket.SysMessage( "You cannot dry dock a vessel while dead." );
		return false;
	}
	if( user.multi == boat )
	{
...
/** @type { ( socket: Socket, result: number ) => any } */
function ReportDryDockResult( socket, result )
{
	...
	if( result == 1 )
	{
		socket.SysMessage( "The ship must be fully repaired before it can be docked." );
	}
	else if( result == 2 )
	{
		socket.SysMessage( "Make sure the ship's hold is empty and try again." );
	}
	else if( result == 3 )
	{
		socket.SysMessage( "You cannot dock the ship with beings aboard." );
	}
	else if( result == 4 )
	{
		socket.SysMessage( "You cannot dock the ship with a cluttered deck." );
	}
	else if( result == 5 )
	{
		socket.SysMessage( "You cannot dock the ship with loaded weapons on deck." );
	}
	else
	{
		socket.SysMessage( "That vessel cannot be dry docked." );
	}
	return false;
}
...
function DryDockDeedSection( boat )
{
	...
	if( multiID >= 0x24 && multiID <= 0x27 )
	{
		return "gargishgalleondeed";
	}
	if( multiID >= 0x30 && multiID <= 0x33 )
	{
		return "tokunogalleondeed";
	}
	if( multiID >= 0x40 && multiID <= 0x43 )
	{
		return "britannianshipdeed";
	}
	if( multiID >= 0x50 && multiID <= 0x53 )
	{
		return "pumpkinrowboatdeed";
	}
	return "";
}
