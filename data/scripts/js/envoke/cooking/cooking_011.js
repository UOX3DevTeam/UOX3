...
function onUse( pUser, iUsed ) 
{
	...
	CustomTarget( srcSock, 0, "What do you want to use the eggs with?" );
}
...
function onCallback0( tSock, targSerial )
{
	...
	if( iID == 0x0461 || iID == 0x0462 || iID == 0x046b || iID == 0x046f || iID == 0x0475 || iID == 0x047b || iID == 0x092b || iID == 0x092c || iID == 0x0930 || iID == 0x0931 || iID == 0x0937 || iID == 0x0945 || iID == 0x0953 || iID == 0x0961 || iID == 0x0de3 || iID == 0x0fac || iID == 0x0fb1 || iID == 0x0e31 || iID == 0x019bb )
	{
		...
		// remove one raw bird
		var iMakeResource = GetResourceCount( tChar, 0x09b5, 0 );	// is there enough resources to use up to make it
		if( iMakeResource < 1 )
		{
			SysMessage( tSock, "You dont seem to have any raw eggs!" );
			return;
		}
		UseResource( tChar, 0x09b5, 0, 1); // uses up a resource (character, item ID, item colour, amount)
		DoSoundEffect( tItem, 1, 0x0021, true);
		// check the skill
		if( GetSkill( srcChar, 13 ) < 10 ) 
		{
			SysMessage( tSock, "You are not skilled enough to do that." );
			return;
		}
		if( !CheckSkill( tChar, 13, 10, 400 ) )	// character to check, skill #, minimum skill, and maximum skill
		{
			SysMessage( tSock, "You burnt the eggs crisp." );
			return;
		}
		// add fried eggs if skill is ok
		var itemMade = SpawnItem( tSock, tChar, 0x09b6, false );	// makes an item and puts in tChar's pack
		SysMessage( tSock, "You cook the eggs." );
		return;
	}

	SysMessage( tSock, "That is not a thing to use raw eggs on." );
